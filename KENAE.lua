...
if not database:get( 'KENAE:'..bot_id.."fshar:"..msg.chat_id_) then
local id = msg.id_
local msgs = { [0] = id}
local chat = msg.chat_id_
delete_msg(chat, msgs)
...
send(msg.chat_id_, msg.id_, 1, '🚦⁞ تم قفل الفشار بالتقييد •', 1, 'md')   
 database:del("fshar"..msg.chat_id_)
    end
 if text == 'فتح الفشار بالتقيد' then
 send(msg.chat_id_, msg.id_, 1, '🚦⁞ تم فتح الفشار بالتقييد •', 1, 'md')   
 database:set("fshar"..msg.chat_id_, true)
  end
...
local ex = database:ttl( 'KENAE:'..bot_id.."charge:"..msg.chat_id_)
  ...
  local nakali = redis:get('KENAE:'..bot_id..'nakali')
    ...
    sendContact(msg.chat_id_, msg.id_, 0, 1, nil, (nkali or 9647806672865), (nakali or "KENAE"), "", bot_id)
  end
   end
    for k,v in pairs(sudo_users) do
