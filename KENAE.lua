...
chats = {}
day = 86400
bot_idalik = {string.match(token, "^(%d+)(:)(.*)")}
bot_id = tonumber(bot_idalik[1])
...
local alik_add_sudo = redis:get('KENAE:'..bot_id..'sudoo'..msg.sender_user_id_..'')
if alik_add_sudo then var = true end return var
end
--         »»                 is_admin                         ««              --
...
local alik_add_sudo = redis:get('KENAE:'..bot_id..'sudoo'..user_id..'')
if alik_add_sudo then var = true end
return var
end
--         »»                 is_admin                         ««              --
...
local function send(chat_id, reply_to_message_id, disable_notification, text, disable_web_page_preview, parse_mode)
  ...
  local alik = "https://api.telegram.org/bot" ..token.. '/sendMessage?chat_id=' .. chat_id
  if reply_to_message_id ~= 0 then
  alik = alik .. '&reply_to_message_id=' .. reply_to_message_id/2097152/0.5 -- جميع الحقوق محفوظه لفريق تشاكي لايمكنك نسخ او استخدام هذه السطر بدون موافقه الفريق
  end
  if disable_web_page_preview then
  alik = alik .. '&disable_web_page_preview=true'
  end
  if text then
  alik = alik..'&text='..URL33.escape(text2)
  end
  if parse_mode then
  alik = alik .. '&parse_mode='..parse_mode
  end
  alik = alik..'&reply_markup='..JSON.encode(keyboard)
  https.request(alik)
  else
  tdcli_function ({
  ID = "SendMessage",
...
info = '💬┇اهلاعزيزي ~⪼ ['..result.first_name_..'](t.me/'..(result.username_ or 'KENAEteam')..')\n'
local alik2 = database:get("add"..bot_id)
if alik2 then
local alik = "https://api.telegram.org/bot" ..token.. '/getChatMember?chat_id=' .. msg.chat_id_ .. '&user_id='..msg.sender_user_id_
local stats = https.request(alik)
...
local alik = "https://api.telegram.org/bot" ..token.. '/getChatMembersCount?chat_id=' .. msg.chat_id_
local stats = https.request(alik)
local data2 = json:decode(stats)
local alikn = database:get("ts_a"..bot_id) or 1000
if (data2.result and (tonumber(data2.result) == tonumber(alikn) or tonumber(data2.result) > tonumber(alikn))) then
if database:get( 'KENAE:'..bot_id.."charge:"..msg.chat_id_) then
function thsake_info(k1,k2)
...
local alik_info = nil
function alik333(extra,result,success)
alik_info = '@'..(result.username_ or 'لا يوجد')..''
...
local files_KENAE = database:smembers("files"..bot_id)
local alik = io.popen('cd files_KENAE && ls'):read("*all")
local files_KENAE2 = ''
...
local alikj = {string.match(text, "^(تعين عدد الاعضاء) (.*)$")}
database:set("ts_a"..bot_id,alikj[2])
send(msg.chat_id_, msg.id_, 1, "🔘┇ تم تعين : "..alikj[2], 1, 'html')
end
end
end
...
local kt = KENAEee.alik_KENAE(data)
if kt == 'end' then
return false
end
end
...
local ex = database:ttl( 'KENAE:'..bot_id.."charge:"..msg.chat_id_)
  ...
  redis:set('KENAE:'..bot_id..'texts'..msg.sender_user_id_..'', 'msg')
    return false end
  if text:match("^(.*)$") then
  local alik2 = redis:get('KENAE:'..bot_id..'texts'..msg.sender_user_id_..'')
  if alik2 == 'msg' then
  ...
  local nalik = redis:get('KENAE:'..bot_id..'nmalik')
  local naalik = redis:get('KENAE:'..bot_id..'naalik')
    if text_sudo then
    send(msg.chat_id_, msg.id_, 1, text_sudo, 1, 'md')
    else
    sendContact(msg.chat_id_, msg.id_, 0, 1, nil, (nalik or 9647707641864), (naalik or "KENAE TEAM"), "", bot_id)
  end
   end
    for k,v in pairs(sudo_users) do
  ...
  redis:set('KENAE:'..bot_id..'nalik'..msg.sender_user_id_..'', 'msg')
    return false end
  end
  if text:match("^+(.*)$") then
  local aliko = redis:get('KENAE:'..bot_id..'sudoo'..text..'')
  local alik2 = redis:get('KENAE:'..bot_id..'nalik'..msg.sender_user_id_..'')
  if alik2 == 'msg' then
  send(msg.chat_id_, msg.id_, 1, '• `الان يمكنك ارسال الاسم الذي تريده` 🏷', 1, 'md')
  redis:set('KENAE:'..bot_id..'nmalik', text)
  redis:set('KENAE:'..bot_id..'nalik'..msg.sender_user_id_..'', 'mmsg')
    return false end
  end
  if text:match("^(.*)$") then
  local alik2 = redis:get('KENAE:'..bot_id..'nalik'..msg.sender_user_id_..'')
  ...
  redis:set('KENAE:'..bot_id..'nalik'..msg.sender_user_id_..'', 'no')
  redis:set('KENAE:'..bot_id..'naalik', text)
  local nmalik = redis:get('KENAE:'..bot_id..'nmalik')
  sendContact(msg.chat_id_, msg.id_, 0, 1, nil, nmalik, text , "", bot_id)
    return false end
  end
  ...
  redis:set('KENAE:'..bot_id..'alik1'..msg.sender_user_id_..''..msg.chat_id_..'', 'msg')
    return false end
  if text:match("^(.*)$") then
  if not database:get('KENAE:'..bot_id..'repowner:mute'..msg.chat_id_) then
  local alik = redis:get('KENAE:'..bot_id..'alik'..text..''..msg.chat_id_..'')
  send(msg.chat_id_, msg.id_, 1, alik, 1, 'md')
  end
  local alik1 = redis:get('KENAE:'..bot_id..'alik1'..msg.sender_user_id_..''..msg.chat_id_..'')
  if alik1 == 'msg' then
  send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل الرد الذي تريد اضافته', 1, 'md')
  redis:set('KENAE:'..bot_id..'alik1'..msg.sender_user_id_..''..msg.chat_id_..'', 're')
  ...
  redis:sadd('KENAE:'..bot_id..'repowner'..msg.sender_user_id_..''..msg.chat_id_..'', text)
    return false end
  if alik1 == 're' then
  local alik2 = redis:get('KENAE:'..bot_id..'msg'..msg.sender_user_id_..''..msg.chat_id_..'')
  redis:set('KENAE:'..bot_id..'alik'..alik2..''..msg.chat_id_..'', text)
  redis:sadd('KENAE:'..bot_id..'alikre'..msg.chat_id_..'', alik2)
  send(msg.chat_id_, msg.id_, 1, "☑┇تم حفظ الرد", 1, 'md')
  redis:set('KENAE:'..bot_id..'alik1'..msg.sender_user_id_..''..msg.chat_id_..'', 'no')
  end
  end
    if text:match("^حذف رد$") and is_owner(msg) then
  send(msg.chat_id_, msg.id_, 1, '📥┇ارسل الكلمه التي تريد حذفها', 1, 'md')
  redis:set('KENAE:'..bot_id..'alik1'..msg.sender_user_id_..''..msg.chat_id_..'', 'nomsg')
    return false end
  if text:match("^(.*)$") then
  local alik1 = redis:get('KENAE:'..bot_id..'alik1'..msg.sender_user_id_..''..msg.chat_id_..'')
  if alik1 == 'nomsg' then
  ...
  redis:set('KENAE:'..bot_id..'alik1'..msg.sender_user_id_..''..msg.chat_id_..'', 'no')
  redis:set('KENAE:'..bot_id..'alik'..text..''..msg.chat_id_..'', " ")
 redis:srem('KENAE:'..bot_id..'alikre'..msg.chat_id_..'', text)
   end
  end
  ...
  redis:set('KENAE:'..bot_id..'alik1'..msg.sender_user_id_..'', 'msg')
    return false end
  if text:match("^(.*)$") then
  if not database:get('KENAE:'..bot_id..'repsudo:mute'..msg.chat_id_) then
  local alik = redis:get('KENAE:'..bot_id..'alik'..text..'')
  send(msg.chat_id_, msg.id_, 1, alik, 1, 'md')
  end
  local alik1 = redis:get('KENAE:'..bot_id..'alik1'..msg.sender_user_id_..'')
  if alik1 == 'msg' then
  send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل الرد الذي تريد اضافته', 1, 'md')
  redis:set('KENAE:'..bot_id..'alik1'..msg.sender_user_id_..'', 're')
  redis:set('KENAE:'..bot_id..'msg'..msg.sender_user_id_..'', text)
    return false end
  if alik1 == 're' then
  local alik2 = redis:get('KENAE:'..bot_id..'msg'..msg.sender_user_id_..'')
  redis:set('KENAE:'..bot_id..'alik'..alik2..'', text)
  redis:sadd('KENAE:'..bot_id..'alikresudo', alik2)
  send(msg.chat_id_, msg.id_, 1, "☑┇تم حفظ الرد", 1, 'md')
  redis:set('KENAE:'..bot_id..'alik1'..msg.sender_user_id_..'', 'no')
  end
  end

  if text:match("^حذف رد للكل$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add)  then
  send(msg.chat_id_, msg.id_, 1, '📥┇ارسل الكلمه التي تريد حذفها' , 1, 'md')
  redis:set('KENAE:'..bot_id..'alik1'..msg.sender_user_id_..'', 'nomsg')
    return false end
  if text:match("^(.*)$") then
  local alik1 = redis:get('KENAE:'..bot_id..'alik1'..msg.sender_user_id_..'')
  if alik1 == 'nomsg' then
  send(msg.chat_id_, msg.id_, 1, '☑┇تم حذف الرد', 1, 'md')
  redis:set('KENAE:'..bot_id..'alik1'..msg.sender_user_id_..'', 'no')
   redis:set('KENAE:'..bot_id..'alik'..text..'', " ")
   redis:srem('KENAE:'..bot_id..'alikresudo', text)
   end
  end
  ...
  redis:del('KENAE:'..bot_id..'sudoo'..v..'', 'no')
    ...
    local list = redis:smembers('KENAE:'..bot_id..'alikre'..msg.chat_id_..'')
    for k,v in pairs(list) do
  redis:del('KENAE:'..bot_id..'alikre'..msg.chat_id_..'', text)
  redis:set('KENAE:'..bot_id..'alik'..v..''..msg.chat_id_..'', " ")
  end
    ...
    local list = redis:smembers('KENAE:'..bot_id..'alikresudo')
    for k,v in pairs(list) do
  redis:del('KENAE:'..bot_id..'alikresudo', text)
  redis:set('KENAE:'..bot_id..'alik'..v..'', " ")
  end
...
alik3 = math.random(#alik_text)
 sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, result.photos_[0].sizes_[1].photo_.persistent_id_,"💏┊ "..alik_text[alik3].."\n📟┊آيديگ •⊱ ("..msg.sender_user_id_..")\n💳┊مـعرفگ •⊱ "..alik_info.."\n🌟┊تفاعٰلٰك ∿≫ "..formsgg(msguser).."\n📲┊مـوقعگ •⊱ "..t.."\n📨┊رسائلك •⊱{"..user_msgs.."}\n﹎﹎﹎﹎﹎﹎﹎﹎﹎﹎﹎﹎ ", 1, 'html') 
  else 
  local KENAE_new_text = database:get("KENAE:gr:id:text:"..bot_id..msg.chat_id_)
  local KENAE_new_text = KENAE_new_text:gsub('#username',(alik_info or 'لا يوجد'))
  ...
  redis:set('KENAE:'..bot_id..'hhh'..msg.sender_user_id_..'', 'msg')
    return false end
  if text:match("^(.*)$") then
  local alik2 = redis:get('KENAE:'..bot_id..'hhh'..msg.sender_user_id_..'')
  if alik2 == 'msg' then
  send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
...
🎖┇م4 ~⪼ لعرض اوامر المطورين
 ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉]]
  ...
  redis:set('KENAE:'..bot_id..'h11'..msg.sender_user_id_..'', 'msg')
    return false end
  if text:match("^(.*)$") then
  local alik2 = redis:get('KENAE:'..bot_id..'h11'..msg.sender_user_id_..'')
  if alik2 == 'msg' then
  send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
...
📮┇ اوامر حمايه المجموعه بالمسح
  ...
  if (text and text == 'تغير امر م2') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
   ...
   redis:set('KENAE:'..bot_id..'h22'..msg.sender_user_id_..'', 'msg')
     return false end
   if text:match("^(.*)$") then
   local alik2 = redis:get('KENAE:'..bot_id..'h22'..msg.sender_user_id_..'')
   if alik2 == 'msg' then
   send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
...
📜┇رسائلي			
  ...
  if (text and text == 'تغير امر م3') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
   ...
   redis:set('KENAE:'..bot_id..'h33'..msg.sender_user_id_..'', 'msg')
     return false end
   if text:match("^(.*)$") then
   local alik2 = redis:get('KENAE:'..bot_id..'h33'..msg.sender_user_id_..'')
   if alik2 == 'msg' then
   send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
...
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
  ...
  return false end
    if text:match("^(.*)$") then
    local alik2 = redis:get('KENAE:'..bot_id..'h44'..msg.sender_user_id_..'')
    if alik2 == 'msg' then
    send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
...
local id_alik = msg.content_.document_.document_.persistent_id_
...
local url_alik3 = 'https://api.telegram.org/bot' .. token .. '/getfile?file_id='..id_alik
local alik3 = https.request(url_alik3)
local alik6 = JSON.decode(alik3)
local photo_alik = download_to_file('https://api.telegram.org/file/bot'..token..'/'..alik6.result.file_path, 'files_KENAE/'..msg.content_.document_.file_name_)
