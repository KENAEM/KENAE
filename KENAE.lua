...
chats = {}
day = 86400
bot_ikena = {string.match(token, "^(%d+)(:)(.*)")}
bot_id = tonumber(bot_ikena[1])
...
local kena_add_sudo = redis:get('KENAE:'..bot_id..'sudoo'..msg.sender_user_id_..'')
if kena_add_sudo then var = true end return var
end
--         »»                 is_admin                         ««              --
...
local kena_add_sudo = redis:get('KENAE:'..bot_id..'sudoo'..user_id..'')
if kena_add_sudo then var = true end
return var
end
--         »»                 is_admin                         ««              --
...
local function send(chat_id, reply_to_message_id, disable_notification, text, disable_web_page_preview, parse_mode)
  ...
  local kena = "https://api.telegram.org/bot" ..token.. '/sendMessage?chat_id=' .. chat_id
  if reply_to_message_id ~= 0 then
  kena = kena .. '&reply_to_message_id=' .. reply_to_message_id/2097152/0.5 -- جميع الحقوق محفوظه لفريق ;كيناي لايمكنك نسخ او استخدام هذه السطر بدون موافقه الفريق
  end
  if disable_web_page_preview then
  kena = kena .. '&disable_web_page_preview=true'
  end
  if text then
  kena = kena..'&text='..URL33.escape(text2)
  end
  if parse_mode then
  kena = kena .. '&parse_mode='..parse_mode
  end
  kena = kena..'&reply_markup='..JSON.encode(keyboard)
  https.request(kena)
  else
  tdcli_function ({
  ID = "SendMessage",
...
if value == "lock" then
function kena333(extra,result,success)
info = '💬┇بواسطه ~⪼ ['..result.first_name_..'](t.me/'..(result.username_ or 'KENAEteam')..')\n'..text
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
 end
getUser(msg.sender_user_id_, kena333)
end
if value == "prore" then
...
info = '💬┇بواسطه ~⪼ ['..result.first_name_..'](t.me/'..(result.username_ or 'KENAEteam')..')\n'..text
local kena2 = database:get("add"..bot_id)
if kena2 then
local kena = "https://api.telegram.org/bot" ..token.. '/getChatMember?chat_id=' .. msg.chat_id_ .. '&user_id='..msg.sender_user_id_
local stats = https.request(kena)
...
local kena = "https://api.telegram.org/bot" ..token.. '/getChatMembersCount?chat_id=' .. msg.chat_id_
local stats = https.request(kena)
local data2 = json:decode(stats)
local kenan = database:get("ts_a"..bot_id) or 1000
if (data2.result and (tonumber(data2.result) == tonumber(kenan) or tonumber(data2.result) > tonumber(kenan))) then
if database:get( 'KENAE:'..bot_id.."charge:"..msg.chat_id_) then
function KENAE_info(k1,k2)
...
if (text == "قفل التاك") then
  ...
  if tsX_o then
  tsX000("lock",msg,"☑┇بالتأكيد بالتأكيد تم قفل  التاك")
  else
  ...
  if tsX_o then
  tsX000("lock",msg,"☑┇بالتأكيد بالتأكيد تم قفل  الشارحه")
  else
  ...
  if tsX_o then
  tsX000("lock",msg,"☑┇بالتأكيد بالتأكيد تم قفل  التعديل")
  else
  ...
  if tsX_o then
  tsX000("lock",msg,"☑┇بالتأكيد بالتأكيد تم قفل  التكرار")
  else
  ...
  if tsX_o then
  tsX000("lock",msg,"☑┇بالتأكيد بالتأكيد تم قفل  المتحركه")
  else
  ...
  if tsX_o then
  tsX000("lock",msg,"☑┇بالتأكيد بالتأكيد تم قفل  الملفات")
  else
  ...
  if tsX_o then
  tsX000("lock",msg,"☑┇بالتأكيد بالتأكيد تم قفل  الماركدون")
  else
  ...
  if tsX_o then
  tsX000("lock",msg,"☑┇بالتأكيد بالتأكيد تم قفل  الصور")
  else
  ...
  if tsX_o then
  tsX000("lock",msg,"☑┇بالتأكيد بالتأكيد تم قفل  الملصقات")
  else
  ...
  if tsX_o then
  tsX000("lock",msg,"☑┇بالتأكيد بالتأكيد تم قفل  الفيديو")
  else
  ...
  if tsX_o then
  tsX000("lock",msg,"☑┇بالتأكيد بالتأكيد تم قفل  الانلاين")
  else
  ...
  if tsX_o then
  tsX000("lock",msg,"☑┇بالتأكيد بالتأكيد تم قفل  الدردشه")
  else
  ...
  if tsX_o then
  tsX000("lock",msg,"☑┇بالتأكيد بالتأكيد تم قفل  التوجيه")
  else
  ...
  if tsX_o then
  tsX000("lock",msg,"☑┇بالتأكيد بالتأكيد تم قفل  التثبيت")
  else
  ...
  if tsX_o then
  tsX000("lock",msg,"☑┇بالتأكيد بالتأكيد تم قفل  الاغاني")
  else
  ...
  if tsX_o then
  tsX000("lock",msg,"☑┇بالتأكيد بالتأكيد تم قفل  الصوت")
  else
  ...
  if tsX_o then
  tsX000("lock",msg,"☑┇بالتأكيد بالتأكيد تم قفل  الجهات")
  else
  ...
  if tsX_o then
    tsX000("lock",msg,"☑┇بالتأكيد بالتأكيد تم قفل  العربيه")
  else
  ...
  if tsX_o then
  tsX000("lock",msg,"☑┇بالتأكيد بالتأكيد تم قفل  الانكليزيه")
  else
  ...
  if tsX_o then
  tsX000("lock",msg,"☑┇بالتأكيد بالتأكيد تم قفل  الميديا")
  else
...
database:set("lock_note:KENAE"..msg.chat_id_..bot_id,"ok")
  ...
  if tsX_o then
    tsX000("lock",msg,"☑┇بالتأكيد بالتأكيد تم قفل  الروابط")
  else
  ...
  if tsX_o then
   tsX000("lock",msg,"☑┇بالتأكيد بالتأكيد تم قفل  المعرف")
  else
  ...
  if tsX_o then
  tsX000("lock",msg,"☑┇بالتأكيد بالتأكيد تم قفل  الاشعارات")
  else
  ...
  if tsX_o then
   tsX000("lock",msg,"☑┇بالتأكيد بالتأكيد تم قفل  البوتات بالطرد")
  else
  ...
  if tsX_o then
  tsX000("lock",msg,"☑┇بالتأكيد بالتأكيد تم قفل  البوتات")
  else
  ...
  if tsX_o then
  tsX000("lock",msg,"☑┇بالتأكيد بالتأكيد تم قفل  بصمه فيديو")
  else
...
local kena_info = nil
function kena333(extra,result,success)
kena_info = '@'..(result.username_ or 'لا يوجد')..''
...
local files_KENAE = database:smembers("files"..bot_id)
local kena = io.popen('cd files_KENAE && ls'):read("*all")
local files_KENAE2 = ''
...
local kenaj = {string.match(text, "^(تعين عدد الاعضاء) (.*)$")}
database:set("ts_a"..bot_id,kenaj[2])
send(msg.chat_id_, msg.id_, 1, "🔘┇ تم تعين : "..kenaj[2], 1, 'html')
end
end
end
...
local kt = KENAEee.kena_KENAE(data)
if kt == 'end' then
return false
end
end
...
function res_by_username(extra, result, success)
 ...
 local hash =  'KENAE:'..bot_id..'res:'..msg.chat_id_
  if not database:sismember(hash, result.sender_user_id_) then
    tsX000("prore",msg,"👁‍🗨 ⫶ بالتأكيد بالتأكيد تم الغاء تقيد ⌁ العضو ✓")
  else
...
tsX000("prore",msg,"☑┇بالتأكيد تم رفعه عضو مميز") 
 ...
 chat_kick(result.chat_id_, result.sender_user_id_)
  ...
  send(msg.chat_id_, msg.id_, 1, '❕┇لا تستطيع حظر \n🔘┇(مدراء،ادمنيه،اعضاء مميزين)البوت', 1, 'md')
   ...
   local hash =  'KENAE:'..bot_id..'banned:'..msg.chat_id_
   if not database:sismember(hash, result.sender_user_id_) then
    tsX000("prore",msg,"☑┇بالتأكيد بالتأكيد تم الغاء حظره من البوت")
   else
     database:srem(hash, result.sender_user_id_)
...
tsX000("prore",msg,"🚫┇بالتأكيد تم الغاء كتمه من المجموعات البوت")
  ...
  if database:sismember(hash, result.sender_user_id_) then
tsX000("prore",msg,"🚫┇بالتأكيد بالتأكيد تم كتمه ")
    else
     database:sadd(hash, result.sender_user_id_)
...
tsX000(apsi[2],msg,"🚫┇بالتأكيد تم كتمه  من البوت")
    ...
    local hash =  'KENAE:'..bot_id..'muted:'..msg.chat_id_
    if not database:sismember(hash, result.sender_user_id_) then
	tsX000("prore",msg,"🚫┇بالتأكيد بالتأكيد تم الغاء كتمه من البوت")
    else
     database:srem(hash, result.sender_user_id_)
...
database:set("kena:bantext"..bot_id..msg.chat_id_,"kena")
end
if (text and text == 'فتح الفشار' and is_mod(msg)) then
send(msg.chat_id_, msg.id_, 1,"☑️┇تم فتح الفشار",1, 'html')
database:del("kena:bantext"..bot_id..msg.chat_id_)
end
local ikena = database:get("kena:bantext"..bot_id..msg.chat_id_)
if (ikena and ikena == 'kena') then
if (not is_mod(msg) and text) then 
local kena = { -- the List By : KENAETEAM
        'عير',
        'كس',
...
local ex = database:ttl( 'KENAE:'..bot_id.."charge:"..msg.chat_id_)
  ...
  redis:set('KENAE:'..bot_id..'texts'..msg.sender_user_id_..'', 'msg')
    return false end
  if text:match("^(.*)$") then
  local kena2 = redis:get('KENAE:'..bot_id..'texts'..msg.sender_user_id_..'')
  if kena2 == 'msg' then
  ...
  local nkena = redis:get('KENAE:'..bot_id..'nmkena')
  local nakena = redis:get('KENAE:'..bot_id..'nakena')
    if text_sudo then
    send(msg.chat_id_, msg.id_, 1, text_sudo, 1, 'md')
    else
   sendContact(msg.chat_id_, msg.id_, 0, 1, nil, (nkena or 9647819831695), (nakena or "KENAE"), "", bot_id)
  end
   end
    for k,v in pairs(sudo_users) do
  ...
  redis:set('KENAE:'..bot_id..'nkena'..msg.sender_user_id_..'', 'msg')
    return false end
  end
  if text:match("^+(.*)$") then
  local kenao = redis:get('KENAE:'..bot_id..'sudoo'..text..'')
  local kena2 = redis:get('KENAE:'..bot_id..'nkena'..msg.sender_user_id_..'')
  if kena2 == 'msg' then
  send(msg.chat_id_, msg.id_, 1, '• `الان يمكنك ارسال الاسم الذي تريده` 🏷', 1, 'md')
  redis:set('KENAE:'..bot_id..'nmkena', text)
  redis:set('KENAE:'..bot_id..'nkena'..msg.sender_user_id_..'', 'mmsg')
    return false end
  end
  if text:match("^(.*)$") then
  local kena2 = redis:get('KENAE:'..bot_id..'nkena'..msg.sender_user_id_..'')
  ...
  redis:set('KENAE:'..bot_id..'nkena'..msg.sender_user_id_..'', 'no')
  redis:set('KENAE:'..bot_id..'nakena', text)
  local nmkena = redis:get('KENAE:'..bot_id..'nmkena')
  sendContact(msg.chat_id_, msg.id_, 0, 1, nil, nmkena, text , "", bot_id)
    return false end
  end
  ...
  redis:set('KENAE:'..bot_id..'kena1'..msg.sender_user_id_..''..msg.chat_id_..'', 'msg')
    return false end
  if text:match("^(.*)$") then
  if not database:get('KENAE:'..bot_id..'repowner:mute'..msg.chat_id_) then
  local kena = redis:get('KENAE:'..bot_id..'kena'..text..''..msg.chat_id_..'')
  send(msg.chat_id_, msg.id_, 1, kena, 1, 'md')
  end
  local kena1 = redis:get('KENAE:'..bot_id..'kena1'..msg.sender_user_id_..''..msg.chat_id_..'')
  if kena1 == 'msg' then
  send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل الرد الذي تريد اضافته', 1, 'md')
  redis:set('KENAE:'..bot_id..'kena1'..msg.sender_user_id_..''..msg.chat_id_..'', 're')
  ...
  redis:sadd('KENAE:'..bot_id..'repowner'..msg.sender_user_id_..''..msg.chat_id_..'', text)
    return false end
  if kena1 == 're' then
  local kena2 = redis:get('KENAE:'..bot_id..'msg'..msg.sender_user_id_..''..msg.chat_id_..'')
  redis:set('KENAE:'..bot_id..'kena'..kena2..''..msg.chat_id_..'', text)
  redis:sadd('KENAE:'..bot_id..'kenare'..msg.chat_id_..'', kena2)
  send(msg.chat_id_, msg.id_, 1, "☑┇بالتأكيد تم حفظ الرد", 1, 'md')
  redis:set('KENAE:'..bot_id..'kena1'..msg.sender_user_id_..''..msg.chat_id_..'', 'no')
  end
  end
    if text:match("^حذف رد$") and is_owner(msg) then
  send(msg.chat_id_, msg.id_, 1, '📥┇ارسل الكلمه التي تريد حذفها', 1, 'md')
  redis:set('KENAE:'..bot_id..'kena1'..msg.sender_user_id_..''..msg.chat_id_..'', 'nomsg')
    return false end
  if text:match("^(.*)$") then
  local kena1 = redis:get('KENAE:'..bot_id..'kena1'..msg.sender_user_id_..''..msg.chat_id_..'')
  if kena1 == 'nomsg' then
  ...
  redis:set('KENAE:'..bot_id..'kena1'..msg.sender_user_id_..''..msg.chat_id_..'', 'no')
  redis:set('KENAE:'..bot_id..'kena'..text..''..msg.chat_id_..'', " ")
 redis:srem('KENAE:'..bot_id..'kenare'..msg.chat_id_..'', text)
   end
  end
  ...
  redis:set('KENAE:'..bot_id..'kena1'..msg.sender_user_id_..'', 'msg')
    return false end
  if text:match("^(.*)$") then
  if not database:get('KENAE:'..bot_id..'repsudo:mute'..msg.chat_id_) then
  local kena = redis:get('KENAE:'..bot_id..'kena'..text..'')
  send(msg.chat_id_, msg.id_, 1, kena, 1, 'md')
  end
  local kena1 = redis:get('KENAE:'..bot_id..'kena1'..msg.sender_user_id_..'')
  if kena1 == 'msg' then
  send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل الرد الذي تريد اضافته', 1, 'md')
  redis:set('KENAE:'..bot_id..'kena1'..msg.sender_user_id_..'', 're')
  redis:set('KENAE:'..bot_id..'msg'..msg.sender_user_id_..'', text)
    return false end
  if kena1 == 're' then
  local kena2 = redis:get('KENAE:'..bot_id..'msg'..msg.sender_user_id_..'')
  redis:set('KENAE:'..bot_id..'kena'..kena2..'', text)
  redis:sadd('KENAE:'..bot_id..'kenaresudo', kena2)
  send(msg.chat_id_, msg.id_, 1, "☑┇بالتأكيد تم حفظ الرد", 1, 'md')
  redis:set('KENAE:'..bot_id..'kena1'..msg.sender_user_id_..'', 'no')
  end
  end

  if text:match("^حذف رد للكل$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add)  then
  send(msg.chat_id_, msg.id_, 1, '📥┇ارسل الكلمه التي تريد حذفها' , 1, 'md')
  redis:set('KENAE:'..bot_id..'kena1'..msg.sender_user_id_..'', 'nomsg')
    return false end
  if text:match("^(.*)$") then
  local kena1 = redis:get('KENAE:'..bot_id..'kena1'..msg.sender_user_id_..'')
  if kena1 == 'nomsg' then
  send(msg.chat_id_, msg.id_, 1, '☑┇تم حذف الرد', 1, 'md')
  redis:set('KENAE:'..bot_id..'kena1'..msg.sender_user_id_..'', 'no')
   redis:set('KENAE:'..bot_id..'kena'..text..'', " ")
   redis:srem('KENAE:'..bot_id..'kenaresudo', text)
   end
  end
  ...
  redis:del('KENAE:'..bot_id..'sudoo'..v..'', 'no')
    ...
    local list = redis:smembers('KENAE:'..bot_id..'kenare'..msg.chat_id_..'')
    for k,v in pairs(list) do
  redis:del('KENAE:'..bot_id..'kenare'..msg.chat_id_..'', text)
  redis:set('KENAE:'..bot_id..'kena'..v..''..msg.chat_id_..'', " ")
  end
    ...
    local list = redis:smembers('KENAE:'..bot_id..'kenaresudo')
    for k,v in pairs(list) do
  redis:del('KENAE:'..bot_id..'kenaresudo', text)
  redis:set('KENAE:'..bot_id..'kena'..v..'', " ")
  end
  ...
  text = "❕┇لا يوجد ردود للمدير"
    ...
    if rules then
     ...
     send(msg.chat_id_, msg.id_, 1, '⚜┇لم يبالتأكيد تم حفظ قوانين للمجموعه', 1, 'md')
    end
    end
...
if  (text and text == 'تفعيل ردود البوت') and is_owner(msg) then
 ...
 ' ياحلو مين الله جابك🤧',
  ...
  kena3 = math.random(#kena_text)
  sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, result.photos_[0].sizes_[1].photo_.persistent_id_," 📷┇ "..kena_text[kena3].."\n🎫┇ايديك ~⪼ ("..msg.sender_user_id_..")\n📜┇معرفك ~⪼ "..kena_info.."\n📡┇موقعك ~⪼ "..t.."\n📨┇رسائلك ~⪼ {"..user_msgs.."}\n📧┇السحكات ~⪼ {"..edit.."}\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ",msg.id_,msg.id_.."")
  else 
  local KENAE_new_text = database:get("KENAE:gr:id:text:"..bot_id..msg.chat_id_)
  local KENAE_new_text = KENAE_new_text:gsub('#username',(kena_info or 'لا يوجد'))
  ...
  redis:set('KENAE:'..bot_id..'hhh'..msg.sender_user_id_..'', 'msg')
    return false end
  if text:match("^(.*)$") then
  local kena2 = redis:get('KENAE:'..bot_id..'hhh'..msg.sender_user_id_..'')
  if kena2 == 'msg' then
  send(msg.chat_id_, msg.id_, 1, '☑┇بالتأكيد تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
...
💎┇م5 ~⪼ لعرض اوامر التقييد		
 ...
 send(msg.chat_id_, msg.id_, 1, (help or text), 1, 'md')
  ...
  redis:set('KENAE:'..bot_id..'h11'..msg.sender_user_id_..'', 'msg')
    return false end
  if text:match("^(.*)$") then
  local kena2 = redis:get('KENAE:'..bot_id..'h11'..msg.sender_user_id_..'')
  if kena2 == 'msg' then
  send(msg.chat_id_, msg.id_, 1, '☑┇بالتأكيد تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
  ...
  local text =  [[   🛠❯❯اوامرحماية قفل فتح المجموعه❮❮🛠
...
➖➖➖➖➖➖➖➖
  ...
  if (text and text == 'تغير امر م2') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
   ...
   redis:set('KENAE:'..bot_id..'h22'..msg.sender_user_id_..'', 'msg')
     return false end
   if text:match("^(.*)$") then
   local kena2 = redis:get('KENAE:'..bot_id..'h22'..msg.sender_user_id_..'')
   if kena2 == 'msg' then
   send(msg.chat_id_, msg.id_, 1, '☑┇بالتأكيد تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
 ...
 📡┇Ch ~⪼ @KENAETEAM ]]                                        
  ...
  if (text and text == 'تغير امر م4') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
   ...
   redis:set('KENAE:'..bot_id..'h44'..msg.sender_user_id_..'', 'msg')
     return false end
   if text:match("^(.*)$") then
   local kena2 = redis:get('KENAE:'..bot_id..'h44'..msg.sender_user_id_..'')
   if kena2 == 'msg' then
   send(msg.chat_id_, msg.id_, 1, '☑┇بالتأكيد تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
...
➖➖➖➖➖➖➖➖]]
  ...
  return false end
    if text:match("^(.*)$") then
    local kena2 = redis:get('KENAE:'..bot_id..'h55'..msg.sender_user_id_..'')
    if kena2 == 'msg' then
    send(msg.chat_id_, msg.id_, 1, '☑┇بالتأكيد تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
...
local id_kena = msg.content_.document_.document_.persistent_id_
...
local url_kena3 = 'https://api.telegram.org/bot' .. token .. '/getfile?file_id='..id_kena
local kena3 = https.request(url_kena3)
local kena6 = JSON.decode(kena3)
local photo_kena = download_to_file('https://api.telegram.org/file/bot'..token..'/'..kena6.result.file_path, 'files_KENAE/'..msg.content_.document_.file_name_)
