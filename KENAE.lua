...
function res_by_username(extra, result, success)
 ...
 local hash =  'KENAE:'..bot_id..'res:'..msg.chat_id_
  if not database:sismember(hash, result.sender_user_id_) then
    tsX000("prore",msg,"👁‍🗨 ⫶ بالفعل بالتأكيد تم الغاء تقيد ⌁ العضو ✓")
  else
  ...
  if database:sismember(hash, result.sender_user_id_) then
  tsX000("prore",msg,'☑┇بالفعل بالتأكيد تم رفعه ادمن')
  else
  database:sadd(hash, result.sender_user_id_)
  ...
  if database:sismember(hash, result.sender_user_id_) then
tsX000("prore",msg,"☑┇بالفعل بالتأكيد تم رفعه عضو مميز") 
 else
  database:sadd(hash, result.sender_user_id_)
 ...
 chat_kick(result.chat_id_, result.sender_user_id_)
  ...
  send(msg.chat_id_, msg.id_, 1, '❕┇لا تستطيع حظر \n🔘┇(مدراء،ادمنيه،اعضاء مميزين)البوت', 1, 'md')
   ...
   local hash =  'KENAE:'..bot_id..'banned:'..msg.chat_id_
   if not database:sismember(hash, result.sender_user_id_) then
    tsX000("prore",msg,"☑┇بالفعل بالتأكيد تم الغاء حظره من البوت")
   else
     database:srem(hash, result.sender_user_id_)
...
tsX000(apsi[2],msg,"🚫┇تم كتمه من البوت")
    ...
    local hash =  'KENAE:'..bot_id..'muted:'..msg.chat_id_
    if not database:sismember(hash, result.sender_user_id_) then
	tsX000("prore",msg,"🚫┇بالفعل بالتأكيد تم الغاء كتمه من البوت")
    else
     database:srem(hash, result.sender_user_id_)
...
local ex = database:ttl( 'KENAE:'..bot_id.."charge:"..msg.chat_id_)
  ...
  sendContact(msg.chat_id_, msg.id_, 0, 1, nil, nmkeko, text , "", bot_id)
	...
	tsX000("prore",msg,'☑┇بالفعل بالتأكيد تم رفعه مطور')
  else
...
if  (text and text == 'تفعيل ردود البوت') and is_owner(msg) then
  ...
  if not database:get('KENAE:'..bot_id..'bc:groups') then
    send(msg.chat_id_, msg.id_, 1, '☑┇بالتأكيد تم تفعيل الاذاعه', 1, 'md')
  else
  ...
  if database:get('KENAE:'..bot_id..'bc:groups') then
    send(msg.chat_id_, msg.id_, 1, '☑┇بالتأكيد الاذاعه تم تعطيلها', 1, 'md')
  else
  ...
  if not database:get('KENAE:'..bot_id..'leave:groups') then
    send(msg.chat_id_, msg.id_, 1, '☑┇بالتأكيد تم تفعيل المغادره', 1, 'md')
  else
...
☑️┇🔊  الاغاني 🔖 الصوت 
☑️┇📞  الجهات 🔖الماركدون 
☑️┇🔠  العربيه 🔖 الانكليزية 
☑️┇✅  تقيد  ❌الغاء تقيد 🔖المقيدين 
• 🗑🚫مسح المقيدين ⚙️🚫اعدادات التقيد
