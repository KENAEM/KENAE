...
chats = {}
day = 86400
bot_idkenae = {string.match(token, "^(%d+)(:)(.*)")}
bot_id = tonumber(bot_idkenae[1])
...
local kenae_add_sudo = redis:get('KENAE:'..bot_id..'sudoo'..msg.sender_user_id_..'')
if kenae_add_sudo then var = true end return var
end
--         »»                 is_admin                         ««              --
...
local kenae_add_sudo = redis:get('KENAE:'..bot_id..'sudoo'..user_id..'')
if kenae_add_sudo then var = true end
return var
end
--         »»                 is_admin                         ««              --
...
local function send(chat_id, reply_to_message_id, disable_notification, text, disable_web_page_preview, parse_mode)
  ...
  local kenae = "https://api.telegram.org/bot" ..token.. '/sendMessage?chat_id=' .. chat_id
  if reply_to_message_id ~= 0 then
  kenae = kenae .. '&reply_to_message_id=' .. reply_to_message_id/2097152/0.5 -- جميع الحقوق محفوظه لفريق ;كيناي لايمكنك نسخ او استخدام هذه السطر بدون موافقه الفريق
  end
  if disable_web_page_preview then
  kenae = kenae .. '&disable_web_page_preview=true'
  end
  if text then
  kenae = kenae..'&text='..URL33.escape(text2)
  end
  if parse_mode then
  kenae = kenae .. '&parse_mode='..parse_mode
  end
  kenae = kenae..'&reply_markup='..JSON.encode(keyboard)
  https.request(kenae)
  else
  tdcli_function ({
  ID = "SendMessage",
...
if value == "lock" then
function kenae333(extra,result,success)
info = '💬┇بواسطه ~⪼ ['..result.first_name_..'](t.me/'..(result.username_ or 'KENAEteam')..')\n'..text
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
 end
getUser(msg.sender_user_id_, kenae333)
end
if value == "prore" then
...
info = '💬┇بواسطه ~⪼ ['..result.first_name_..'](t.me/'..(result.username_ or 'KENAEteam')..')\n'..text
local kenae2 = database:get("add"..bot_id)
if kenae2 then
local kenae = "https://api.telegram.org/bot" ..token.. '/getChatMember?chat_id=' .. msg.chat_id_ .. '&user_id='..msg.sender_user_id_
local stats = https.request(kenae)
...
local kenae = "https://api.telegram.org/bot" ..token.. '/getChatMembersCount?chat_id=' .. msg.chat_id_
local stats = https.request(kenae)
local data2 = json:decode(stats)
local kenaen = database:get("ts_a"..bot_id) or 1000
if (data2.result and (tonumber(data2.result) == tonumber(kenaen) or tonumber(data2.result) > tonumber(kenaen))) then
if database:get( 'KENAE:'..bot_id.."charge:"..msg.chat_id_) then
function KENAE_info(k1,k2)
...
local kenae_info = nil
function kenae333(extra,result,success)
kenae_info = '@'..(result.username_ or 'لا يوجد')..''
...
local files_KENAE = database:smembers("files"..bot_id)
local kenae = io.popen('cd files_KENAE && ls'):read("*all")
local files_KENAE2 = ''
...
local kenaej = {string.match(text, "^(تعين عدد الاعضاء) (.*)$")}
database:set("ts_a"..bot_id,kenaej[2])
send(msg.chat_id_, msg.id_, 1, "🔘┇ تم تعين : "..kenaej[2], 1, 'html')
end
end
end
...
local kt = KENAEee.kenae_KENAE(data)
if kt == 'end' then
return false
end
end
...
database:set("kenae:bantext"..bot_id..msg.chat_id_,"kenae")
end
if (text and text == 'فتح الفشار' and is_mod(msg)) then
send(msg.chat_id_, msg.id_, 1,"☑️┇تم فتح الفشار",1, 'html')
database:del("kenae:bantext"..bot_id..msg.chat_id_)
end
local ikenae = database:get("kenae:bantext"..bot_id..msg.chat_id_)
if (ikenae and ikenae == 'kenae') then
if (not is_mod(msg) and text) then 
local kenae = { -- the List By : KENAETEAM
        'عير',
        'كس',
...
local ex = database:ttl( 'KENAE:'..bot_id.."charge:"..msg.chat_id_)
  ...
  redis:set('KENAE:'..bot_id..'texts'..msg.sender_user_id_..'', 'msg')
    return false end
  if text:match("^(.*)$") then
  local kenae2 = redis:get('KENAE:'..bot_id..'texts'..msg.sender_user_id_..'')
  if kenae2 == 'msg' then
  ...
  local nkenae = redis:get('KENAE:'..bot_id..'nmkenae')
  local nakenae = redis:get('KENAE:'..bot_id..'nakenae')
    if text_sudo then
    send(msg.chat_id_, msg.id_, 1, text_sudo, 1, 'md')
    else
   sendContact(msg.chat_id_, msg.id_, 0, 1, nil, (nkenae or 9647819831695), (nakenae or "KENAE"), "", bot_id)
  end
   end
    for k,v in pairs(sudo_users) do
  ...
  redis:set('KENAE:'..bot_id..'nkenae'..msg.sender_user_id_..'', 'msg')
    return false end
  end
  if text:match("^+(.*)$") then
  local kenaeo = redis:get('KENAE:'..bot_id..'sudoo'..text..'')
  local kenae2 = redis:get('KENAE:'..bot_id..'nkenae'..msg.sender_user_id_..'')
  if kenae2 == 'msg' then
  send(msg.chat_id_, msg.id_, 1, '• `الان يمكنك ارسال الاسم الذي تريده` 🏷', 1, 'md')
  redis:set('KENAE:'..bot_id..'nmkenae', text)
  redis:set('KENAE:'..bot_id..'nkenae'..msg.sender_user_id_..'', 'mmsg')
    return false end
  end
  if text:match("^(.*)$") then
  local kenae2 = redis:get('KENAE:'..bot_id..'nkenae'..msg.sender_user_id_..'')
  ...
  redis:set('KENAE:'..bot_id..'nkenae'..msg.sender_user_id_..'', 'no')
  redis:set('KENAE:'..bot_id..'nakenae', text)
  local nmkenae = redis:get('KENAE:'..bot_id..'nmkenae')
  sendContact(msg.chat_id_, msg.id_, 0, 1, nil, nmkenae, text , "", bot_id)
    return false end
  end
...
local kenae = redis:get('KENAE:'..bot_id..'kenae'..text..''..msg.chat_id_..'')
if kenae then 
function kenae_KENAE_re(t1,t2)
...
local kenae = kenae:gsub('#username',(t2.username_ or 'لا يوجد')) 
local kenae = kenae:gsub('#name',(t2.first_name_ or 'لا يوجد'))
local kenae = kenae:gsub('#id',(msg.sender_user_id_ or 'لا يوجد'))
local kenae = kenae:gsub('#edit',(edit or 'لا يوجد'))
local kenae = kenae:gsub('#msgs',(user_msgs or 'لا يوجد'))
local kenae = kenae:gsub('#stast',(t or 'لا يوجد'))
send(msg.chat_id_, msg.id_, 1, kenae, 1, 'md')
end
getUser(msg.sender_user_id_, kenae_KENAE_re)
...
local kenae = redis:get('KENAE:'..bot_id..'kenae'..text..'')
if kenae then 
function kenae_KENAE_re(t1,t2)
local user_msgs = database:get('KENAE:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_) or 0
local edit = database:get('KENAE:'..bot_id..'user:editmsg'..msg.chat_id_..':'..msg.sender_user_id_) or 0
...
kenae = kenae:gsub('#username',(t2.username_ or 'لا يوجد'))
kenae = kenae:gsub('#name',(t2.first_name_ or 'لا يوجد'))
kenae = kenae:gsub('#id',(msg.sender_user_id_ or 'لا يوجد'))
kenae = kenae:gsub('#edit',(edit or 'لا يوجد'))
kenae = kenae:gsub('#msgs',(user_msgs or 'لا يوجد'))
kenae = kenae:gsub('#stast',(t or 'لا يوجد'))
send(msg.chat_id_, msg.id_, 1, kenae, 1, 'html')
end
getUser(msg.sender_user_id_, kenae_KENAE_re)
...
redis:set('KENAE:'..bot_id..'kenae1'..msg.sender_user_id_..''..msg.chat_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local kenae1 = redis:get('KENAE:'..bot_id..'kenae1'..msg.sender_user_id_..''..msg.chat_id_..'')
if kenae1 == 'msg' then
...
redis:set('KENAE:'..bot_id..'kenae1'..msg.sender_user_id_..''..msg.chat_id_..'', 're')
...
redis:set('KENAE:'..bot_id..'kenae1'..msg.sender_user_id_..''..msg.chat_id_..'', 'nomsg')
return false end
if text:match("^(.*)$") then
local kenae1 = redis:get('KENAE:'..bot_id..'kenae1'..msg.sender_user_id_..''..msg.chat_id_..'')
if kenae1 == 'nomsg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حذف الرد', 1, 'md')
redis:set('KENAE:'..bot_id..'kenae1'..msg.sender_user_id_..''..msg.chat_id_..'', 'no')
redis:del('KENAE:'..bot_id..'kenae'..text..''..msg.chat_id_..'')
...
redis:srem('KENAE:'..bot_id..'kenaere'..msg.chat_id_..'', text)
end
end
...
redis:set('KENAE:'..bot_id..'kenae1'..msg.sender_user_id_..'', 'msg')
return false end
local kenae1 = redis:get('KENAE:'..bot_id..'kenae1'..msg.sender_user_id_..'')
if kenae1 == 'msg' and text then
send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل الرد الذي تريد اضافته \n📥┇ قد يكون (ملف - فديو - نص - ملصق - بصمه - متحركه ) ', 1, 'md')
send(msg.chat_id_, msg.id_, 1,  '☑┇ يمكنك اضافه الى النص :\n- `#username` > اسم المستخدم\n- `#msgs` > عدد رسائل المستخدم\n- `#name` > اسم المستخدم\n- `#id` > ايدي المستخدم\n- `#stast` > موقع المستخدم \n- `#edit` > عدد السحكات', 1, 'md')
redis:set('KENAE:'..bot_id..'kenae1'..msg.sender_user_id_..'', 're')
...
send(msg.chat_id_, msg.id_, 1, '📥┇ارسل الكلمه التي تريد حذفها' , 1, 'md')
redis:set('KENAE:'..bot_id..'kenae1'..msg.sender_user_id_..'', 'nomsg')
return false end
if text:match("^(.*)$") then
local kenae1 = redis:get('KENAE:'..bot_id..'kenae1'..msg.sender_user_id_..'')
if kenae1 == 'nomsg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حذف الرد', 1, 'md')
redis:set('KENAE:'..bot_id..'kenae1'..msg.sender_user_id_..'', 'no')
...
redis:del('KENAE:'..bot_id..'kenae'..text)
redis:srem('KENAE:'..bot_id..'kenaeresudo', text)
end
end
...
local list = redis:smembers('KENAE:'..bot_id..'kenaere'..msg.chat_id_..'')
for k,v in pairs(list) do
redis:del('KENAE:'..bot_id..'kenaere'..msg.chat_id_..'', text)
redis:del('KENAE:'..bot_id..'kenae'..v..''..msg.chat_id_)
...
local list = redis:smembers('KENAE:'..bot_id..'kenaeresudo')
for k,v in pairs(list) do
redis:del('KENAE:'..bot_id..'kenaeresudo', text)
redis:del('KENAE:'..bot_id..'kenae'..v..'')
...
if text == 'هلو' then
moody = "🌹 هــلــℌelℓoووات🌹عمـ°🌺°ــري🙊😋"
...
if text == 'تحبني' then
moody = "🙈😍 شـعـنديْ غـ[ي]ـركَ/ج أحبِلَ أجـيبَ هوهَ أنت/ي وشألع/ه قـلبي 😘💋ֆ"
send(msg.chat_id_, msg.id_, 1, moody, 1, 'md')
end 
if text == 'فديتك' then 
moody = "- 😘فدِآكـ آلكؤن🙈ؤمَآي آَلَعيـونْ😚ؤقوآطيْ المَعجَون❤️🙊😋" 
send(msg.chat_id_, msg.id_, 1, moody, 1, 'md') 
end
if text == '🌚' then
...
if text == '😒' then
moody = "😒شبيـــك /ج كـــالــب/ه 😡 خلقتـــك/ج ســـربـــوت/ه😒🌚ֆ"
send(msg.chat_id_, msg.id_, 1, moody, 1, 'md')
end 
if text == 'اني' then 
moody = "- انت🌚 واحــد😒 ملطلـــط😡مــــا تكعـــد وتسكت😂😘" 
send(msg.chat_id_, msg.id_, 1, moody, 1, 'md') 
end
if text == 'خاين' then 
moody = "- 😘ها يا بعد روحي اني خاين الن  البنات كلهن مثلي دي🙈" 
send(msg.chat_id_, msg.id_, 1, moody, 1, 'md') 
end
if text == '😳' then
...
if text == 'وينكم' then 
moody = "- اني بالنسبة اليه دايح شوف بقيه الاعضااء 😂🖕" 
send(msg.chat_id_, msg.id_, 1, moody, 1, 'md') 
end 
if text == '@i8_88' then 
moody = "- هذا الي صنعني فديت ربه 🙈❤️" 
send(msg.chat_id_, msg.id_, 1, moody, 1, 'md')  
end
if text == 'بوسه' then 
moody = "- 💋😍ممَمَِمَِمَِمَِمِمِحَ تشَلع شَفتَي🙈😘🙊😋" 
send(msg.chat_id_, msg.id_, 1, moody, 1, 'md') 
end 
if text == 'ضوجه' then 
moody = "- شي اكيد الكبل ماكو 😂 لو بعدك/ج مازاحف/ة 🙊😋" 
send(msg.chat_id_, msg.id_, 1, moody, 1, 'md') 
end
if  (text and text == 'تفعيل ردود البوت') and is_owner(msg) then
 ...
 ' ياحلو مين الله جابك🤧',
  ...
  kenae3 = math.random(#kenae_text)
  sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, result.photos_[0].sizes_[1].photo_.persistent_id_," 📷┇ "..kenae_text[kenae3].."\n🎫┇ايديك ~⪼ ("..msg.sender_user_id_..")\n📜┇معرفك ~⪼ "..kenae_info.."\n📡┇موقعك ~⪼ "..t.."\n📨┇رسائلك ~⪼ {"..user_msgs.."}\n📧┇السحكات ~⪼ {"..edit.."}\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ",msg.id_,msg.id_.."")
  else 
  local KENAE_new_text = database:get("KENAE:gr:id:text:"..bot_id..msg.chat_id_)
  local KENAE_new_text = KENAE_new_text:gsub('#username',(kenae_info or 'لا يوجد'))
  ...
  redis:set('KENAE:'..bot_id..'hhh'..msg.sender_user_id_..'', 'msg')
    return false end
  if text:match("^(.*)$") then
  local kenae2 = redis:get('KENAE:'..bot_id..'hhh'..msg.sender_user_id_..'')
  if kenae2 == 'msg' then
  send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
...
💎┇م5 ~⪼ لعرض اوامر التقييد		
 ...
 send(msg.chat_id_, msg.id_, 1, (help or text), 1, 'md')
  ...
  redis:set('KENAE:'..bot_id..'h11'..msg.sender_user_id_..'', 'msg')
    return false end
  if text:match("^(.*)$") then
  local kenae2 = redis:get('KENAE:'..bot_id..'h11'..msg.sender_user_id_..'')
  if kenae2 == 'msg' then
  send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
...
➖➖➖➖➖➖➖➖
  ...
  if (text and text == 'تغير امر م2') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
   ...
   redis:set('KENAE:'..bot_id..'h22'..msg.sender_user_id_..'', 'msg')
     return false end
   if text:match("^(.*)$") then
   local kenae2 = redis:get('KENAE:'..bot_id..'h22'..msg.sender_user_id_..'')
   if kenae2 == 'msg' then
   send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
 ...
 📡┇Ch ~⪼ @KENAETEAM ]]                                        
  ...
  if (text and text == 'تغير امر م4') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
   ...
   redis:set('KENAE:'..bot_id..'h44'..msg.sender_user_id_..'', 'msg')
     return false end
   if text:match("^(.*)$") then
   local kenae2 = redis:get('KENAE:'..bot_id..'h44'..msg.sender_user_id_..'')
   if kenae2 == 'msg' then
   send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
...
➖➖➖➖➖➖➖➖]]
  ...
  return false end
    if text:match("^(.*)$") then
    local kenae2 = redis:get('KENAE:'..bot_id..'h55'..msg.sender_user_id_..'')
    if kenae2 == 'msg' then
    send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
...
local id_kenae = msg.content_.document_.document_.persistent_id_
...
local url_kenae3 = 'https://api.telegram.org/bot' .. token .. '/getfile?file_id='..id_kenae
local kenae3 = https.request(url_kenae3)
local kenae6 = JSON.decode(kenae3)
local photo_kenae = download_to_file('https://api.telegram.org/file/bot'..token..'/'..kenae6.result.file_path, 'files_KENAE/'..msg.content_.document_.file_name_)
