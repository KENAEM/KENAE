...
localKENAE = 'https://api.telegram.org/bot' .. token .. '/sendDocument'
...
local files_KENAE = database:smembers("files"..bot_id)
for i=1,#files_KENAE do
local KENAE = dofile("files_KENAE/"..files_KENAE[i])
local kt = KENAE.keko_KENAE(data)
if kt == 'end' then
return false
end
end
...
return false end
  ...
  for k,v in pairs(sudo_users) do
    ...
    if not database:sismember( 'KENAE:'..bot_id.."userss",msg.chat_id_) then
  ...
  if text:match("^رفع عضو مميز$")  and is_owner(msg) and msg.reply_to_message_id_ then
  function promote_by_reply(extra, result, success)
...
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'KENAE)..')\n☑┇تم رفعه عضو مميز'
    ...
    if text:match("^تنزيل عضو مميز$") and is_owner(msg) and msg.reply_to_message_id_ then
    function demote_by_reply(extra, result, success)
    local hash =  'KENAE:'..bot_id..'vipgp:'..msg.chat_id_
    if not database:sismember(hash, result.sender_user_id_) then
...
local ex = database:ttl( 'KENAE:'..bot_id.."charge:"..msg.chat_id_)
  ...
  local nakeko = redis:get('KENAE:'..bot_id..'nakeko')
    ...
    sendContact(msg.chat_id_, msg.id_, 0, 1, nil, (nkeko or+9647816856066), (nakeko or "KENAE KENAE"), "", bot_id)
  end
   end
    for k,v in pairs(sudo_users) do
