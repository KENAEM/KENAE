...
local hash = 'flood:KENAE:'..bot_id..msg.chat_id_
if not database:get(hash) then
floodKENAE = 10
else
floodKENAE = tonumber(database:get(hash))
end
...
local ph = pronumb[2] - 1
  ...
  limit_ = pronumb[2]
    ...
    local floodKENAE = {string.match(text, "^(وضع تكرار) (%d+)$")}
    if tonumber(floodKENAE[2]) < 2 then
  send(msg.chat_id_, msg.id_, 1, '🔘┊ضع التكرار من *{2}* الى  *{99999}*', 1, 'md')
    else
  database:set('KENAE:'..bot_id..'flood:KENAE:'..msg.chat_id_,floodKENAE[2])
  send(msg.chat_id_, msg.id_, 1, '☑┊تم  وضع التكرار بالطرد للعدد ~⪼  *{'..floodKENAE[2]..'}*', 1, 'md')
    end
  end
  ...
  send(msg.chat_id_, msg.id_, 1, moody, 1, 'html')
   ...
   bot.channel_get_admins(msg.chat_id_,cb)
    ...
    if database:get("lock_media:KENAE"..msg.chat_id_..bot_id) then
       ...
       ------------
       if not database:get('KENAE:'..bot_id..'flood:KENAE:'..msg.chat_id_) then
       flood_m = 10
       else
       flood_m = database:get('KENAE:'..bot_id..'flood:KENAE:'..msg.chat_id_)
     end
