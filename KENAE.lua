...
chats = {}
day = 86400
bot_idi8_88 = {string.match(token, "^(%d+)(:)(.*)")}
bot_id = tonumber(bot_idi8_88[1])
sudo_users = {sudo_add,bot_id}
URL33 = require('socket.url')
---------- {Show Files} -----------
red = '\27[31m' reset = '\27[m' Blue = "\27[34m" Green = "\27[32m"
local files_i8_88E = database:smembers("files"..bot_id) 
print(Green.."\nFiles Now Started : \n "..reset)
for i,v in pairs(files_i8_88E) do
print(Blue..i..red..' - \27[10;33m'..v..',\27[m')  end
print(Green.."\nThes all Files.\n\n\n"..reset)
io.popen("mkdir files_i8_88E")
...
local i8_88_add_sudo = redis:get('i8_88E:'..bot_id..'sudoo'..msg.sender_user_id_..'')
if i8_88_add_sudo then var = true end return var
end
--         »»                 is_admin                         ««              --
function is_admin(msg)
user_id = msg.sender_user_id_
local var = false 
local admin = database:sismember('i8_88E:'..bot_id..'admins:', user_id)
if admin then var = true end
for k,v in pairs(sudo_users) do
if user_id == v then var = true end
end
local i8_88_add_sudo = redis:get('i8_88E:'..bot_id..'sudoo'..user_id..'')
if i8_88_add_sudo then var = true end
return var
end
--         »»                 is_admin                         ««              --
...
local var = false
local creator = database:sismember('i8_88E:'..bot_id..'creator:'..chat_id, user_id) 
local admin = database:sismember('i8_88E:'..bot_id..'admins:', user_id)
...
chat_id = msg.chat_id_
local var = false
local mod = database:sismember('i8_88E:'..bot_id..'mods:'..chat_id, user_id)  
local admin = database:sismember('i8_88E:'..bot_id..'admins:', user_id)  
local owner = database:sismember('i8_88E:'..bot_id..'owners:'..chat_id, user_id)
local creator = database:sismember('i8_88E:'..bot_id..'creator:'..chat_id, user_id)  
local vip = database:sismember('i8_88E:'..bot_id..'vipgp:'..chat_id, user_id)
...
local var = false
local banned = database:sismember('i8_88E:'..bot_id..'banned:'..chat_id, user_id)
...
local var = false
local banned = database:sismember('i8_88E:'..bot_id..'gbanned:', user_id)
if banned then var = true end
return var
end
--         »»                 is_muted                         ««              --
...
local var = false
local banned = database:sismember('i8_88E:'..bot_id..'muted:'..chat_id, user_id)
if banned then var = true end
return var
end
--         »»                 is_gmuted                         ««              --
...
local var = false 
local banned = database:sismember('i8_88E:'..bot_id..'gmuted:', user_id)
if banned then var = true end
return var
end
--         »»                 getMessage                         ««              --
...
local hash =  'i8_88E:'..bot_id..'filters:'..msg.chat_id_
if hash then
...
local function send(chat_id, reply_to_message_id, disable_notification, text, disable_web_page_preview, parse_mode)
  ...
  {
  {text = ''..(channel_ts or "i8_88E TEAM")..'', url=''..(channel_user_ts or 't.me/i8_88ETEAM')..''},
  },
  }
  local i8_88 = "https://api.telegram.org/bot" ..token.. '/sendMessage?chat_id=' .. chat_id
  if reply_to_message_id ~= 0 then
  i8_88 = i8_88 .. '&reply_to_message_id=' .. reply_to_message_id/2097152/0.5 -- جميع الحقوق محفوظه لفريق كيناي لايمكنك نسخ او استخدام هذه السطر بدون موافقه الفريق
  end
  if disable_web_page_preview then
  i8_88 = i8_88 .. '&disable_web_page_preview=true'
  end
  if text then
  i8_88 = i8_88..'&text='..URL33.escape(text2)
  end
  if parse_mode then
  i8_88 = i8_88 .. '&parse_mode='..parse_mode
  end
  i8_88 = i8_88..'&reply_markup='..JSON.encode(keyboard)
  https.request(i8_88)
  else
  tdcli_function ({
  ID = "SendMessage",
...
if value == "lock" then
function i8_88333(extra,result,success)
info = '💬┇بواسطه ~⪼ ['..result.first_name_..'](t.me/'..(result.username_ or 'i8_88Eteam')..')\n'..text
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
 end
getUser(msg.sender_user_id_, i8_88333)
end
if value == "prore" then
function get_i8_88EX(i8_88Ex1,i8_88Ex2,i8_88Ex3)
local id_i8_88Ex = i8_88Ex2.sender_user_id_
function i8_88333(extra,result,success)
info = '👤┇العضو ~⪼ ['..result.first_name_..'](t.me/'..(result.username_ or 'i8_88Eteam')..')\n'..text
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
 end
getUser(id_i8_88Ex, i8_88333)
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,get_i8_88EX)
end
if value ~= "prore" and value~= "lock"  then
function i8_88333(extra,result,success)
info = '👤┇العضو ~⪼ ['..(result.first_name_ or value)..'](t.me/'..(result.username_ or 'i8_88Eteam')..')\n'..text
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
getUser(value, i8_88333)
end
end -- end fun
--         »»                 End Functions                         ««              --
...
info = '💬┇بواسطه ~⪼ ['..result.first_name_..'](t.me/'..(result.username_ or 'i8_88Eteam')..')\n'
local i8_882 = database:get("add"..bot_id)
if i8_882 then
local i8_88 = "https://api.telegram.org/bot" ..token.. '/getChatMember?chat_id=' .. msg.chat_id_ .. '&user_id='..msg.sender_user_id_
local stats = https.request(i8_88)
local data = json:decode(stats)
if (data.result and data.result.status == 'creator') then
if not database:get( 'i8_88E:'..bot_id.."charge:"..msg.chat_id_) then
...
if database:get( 'i8_88E:'..bot_id.."charge:"..msg.chat_id_) then
database:del( 'i8_88E:'..bot_id.."charge:"..msg.chat_id_)
function thsake_info(k1,k2)
...
database:set('i8_88E:'..bot_id.."group:link"..msg.chat_id_ ,link.result) 
link = link.result 
else 
link = "(لا يوجد)"
end
send(tostring((database:get("i8_88E"..bot_id..":sudo:gr") or sudo_add)), 0, 1, "🔘┇قام بتعطيل مجموعه \n🎫┇ايدي المطور ~⪼ ("..msg.sender_user_id_..")\n📜┇معرف المطور ~⪼ @"..(result.username_ or "لا يوجد").."\n🌐┇معلومات المجموعه \n\n🎫┇ايدي المجموعه ~⪼ ("..msg.chat_id_..")\nⓂ️┇اسم المجموعه ~⪼ ("..k2.title_..")\n📎┇رابط المجموعه ~⪼ ["..link.."]" , 1, 'html')
end
...
local i8_88 = "https://api.telegram.org/bot" ..token.. '/getChatMembersCount?chat_id=' .. msg.chat_id_
local stats = https.request(i8_88)
local data2 = json:decode(stats)
local i8_88n = database:get("ts_a"..bot_id) or 1000
if (data2.result and (tonumber(data2.result) == tonumber(i8_88n) or tonumber(data2.result) > tonumber(i8_88n))) then
if database:get( 'i8_88E:'..bot_id.."charge:"..msg.chat_id_) then
function thsake_info(k1,k2)
...
if not database:get( 'i8_88E:'..bot_id.."charge:"..msg.chat_id_) then
database:set( 'i8_88E:'..bot_id.."charge:"..msg.chat_id_,true)
function thsake_info(k1,k2)
...
send(tostring((database:get("i8_88E"..bot_id..":sudo:gr") or sudo_add)), 0, 1, "🔘┇قام بتفعيل مجموعه \n🎫┇ايدي المطور ~⪼ ("..msg.sender_user_id_..")\n📜┇معرف المطور ~⪼ @"..(result.username_ or "لا يوجد").."\n🌐┇معلومات المجموعه \n\n🎫┇ايدي المجموعه ~⪼ ("..msg.chat_id_..")\nⓂ️┇اسم المجموعه ~⪼ ("..k2.title_..")\n📎┇رابط المجموعه ~⪼ ["..link.."]" , 1, 'html')
end
openChat(msg.chat_id_,thsake_info2) 
--
if data.result.can_promote_members  then
database:sadd('i8_88E:'..bot_id..'owners:'..msg.chat_id_,msg.sender_user_id_)
end
database:set( 'i8_88E:'..bot_id.."enable:"..msg.chat_id_,true)
if data.result.status == 'creator' then
database:sadd('i8_88E:'..bot_id..'creator:'..msg.chat_id_, msg.sender_user_id_)
end
end
else
...
if (text == "قفل التاك") then
  local tsX_o = database:get("lock_tag:i8_88E"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل التاك")
  else
  tsX000("lock",msg,"☑┇تم قفل التاك")
  database:set("lock_tag:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الشارحه") then
  local tsX_o = database:get("lock_sarha:i8_88E"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل الشارحه")
  else
  tsX000("lock",msg,"☑┇تم قفل الشارحه")
  database:set("lock_sarha:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل التعديل") then
  local tsX_o = database:get("lock_edit:i8_88E"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل التعديل")
  else
  tsX000("lock",msg,"☑┇تم قفل التعديل")
  database:set("lock_edit:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل التكرار") then
  local tsX_o = database:get("lock_lllll:i8_88E"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل التكرار")
  else
  tsX000("lock",msg,"☑┇تم قفل التكرار")
  database:set("lock_lllll:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل المتحركه") then
  local tsX_o = database:get("lock_gif:i8_88E"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل المتحركه")
  else
  tsX000("lock",msg,"☑┇تم قفل المتحركه")
  database:set("lock_gif:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الملفات") then
  local tsX_o = database:get("lock_files:i8_88E"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل الملفات")
  else
  tsX000("lock",msg,"☑┇تم قفل الملفات")
  database:set("lock_files:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الماركدون") then
  local tsX_o = database:get("lock_mark:i8_88E"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل الماركدون")
  else
  tsX000("lock",msg,"☑┇تم قفل الماركدون")
  database:set("lock_mark:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الصور") then
  local tsX_o = database:get("lock_photo:i8_88E"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل الصور")
  else
  tsX000("lock",msg,"☑┇تم قفل الصور")
  database:set("lock_photo:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الملصقات") then
  local tsX_o = database:get("lock_stecker:i8_88E"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل الملصقات")
  else
  tsX000("lock",msg,"☑┇تم قفل الملصقات")
  database:set("lock_stecker:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الفيديو") then
  local tsX_o = database:get("lock_video:i8_88E"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل الفيديو")
  else
  tsX000("lock",msg,"☑┇تم قفل الفيديو")
  database:set("lock_video:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الانلاين") then
  local tsX_o = database:get("lock_inline:i8_88E"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل الانلاين")
  else
  tsX000("lock",msg,"☑┇تم قفل الانلاين")
  database:set("lock_inline:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الدردشه") then
  local tsX_o = database:get("lock_chat:i8_88E"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل الدردشه")
  else
  tsX000("lock",msg,"☑┇تم قفل الدردشه")
  database:set("lock_chat:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل التوجيه") then
  local tsX_o = database:get("lock_fwd:i8_88E"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل التوجيه")
  else
  tsX000("lock",msg,"☑┇تم قفل التوجيه")
  database:set("lock_fwd:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل التثبيت") then
  local tsX_o = database:get("lock_pin:i8_88E"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل التثبيت")
  else
  tsX000("lock",msg,"☑┇تم قفل التثبيت")
  database:set("lock_pin:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الاغاني") then
  local tsX_o = database:get("lock_audeo:i8_88E"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل الاغاني")
  else
  tsX000("lock",msg,"☑┇تم قفل الاغاني")
  database:set("lock_audeo:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الصوت") then
  local tsX_o = database:get("lock_voice:i8_88E"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل الصوت")
  else
  tsX000("lock",msg,"☑┇تم قفل الصوت")
  database:set("lock_voice:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الجهات") then
  local tsX_o = database:get("lock_contact:i8_88E"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل الجهات")
  else
  tsX000("lock",msg,"☑┇تم قفل الجهات")
  database:set("lock_contact:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل العربيه") then
  local tsX_o = database:get("lock_ar:i8_88E"..msg.chat_id_..bot_id)
  if tsX_o then
    tsX000("lock",msg,"☑┇بالفعل تم قفل العربيه")
  else
  tsX000("lock",msg,"☑┇تم قفل العربيه")
  database:set("lock_ar:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الانكليزيه") then
  local tsX_o = database:get("lock_en:i8_88E"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل الانكليزيه")
  else
  tsX000("lock",msg,"☑┇تم قفل الانكليزيه")
  database:set("lock_en:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الميديا") then
  local tsX_o = database:get("lock_media:i8_88E"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل الميديا")
  else
  tsX000("lock",msg,"☑┇تم قفل الميديا")
  database:set("lock_media:i8_88E"..msg.chat_id_..bot_id,"ok")
database:set("lock_audeo:i8_88E"..msg.chat_id_..bot_id,"ok")
database:set("lock_video:i8_88E"..msg.chat_id_..bot_id,"ok")
database:set("lock_photo:i8_88E"..msg.chat_id_..bot_id,"ok")
database:set("lock_stecker:i8_88E"..msg.chat_id_..bot_id,"ok")
database:set("lock_voice:i8_88E"..msg.chat_id_..bot_id,"ok")
database:set("lock_gif:i8_88E"..msg.chat_id_..bot_id,"ok")
database:set("lock_note:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الروابط") then
  local tsX_o = database:get("lock_link:i8_88E"..msg.chat_id_..bot_id)
  if tsX_o then
    tsX000("lock",msg,"☑┇بالفعل تم قفل الروابط")
  else
  tsX000("lock",msg,"☑┇تم قفل الروابط")
  database:set("lock_link:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل المعرف") then
  local tsX_o = database:get("lock_username:i8_88E"..msg.chat_id_..bot_id)
  if tsX_o then
   tsX000("lock",msg,"☑┇بالفعل تم قفل المعرف")
  else
  tsX000("lock",msg,"☑┇تم قفل المعرف")
  database:set("lock_username:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الاشعارات") then
  local tsX_o = database:get("lock_new:i8_88E"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل الاشعارات")
  else
  tsX000("lock",msg,"☑┇تم قفل الاشعارات")
  database:set("lock_new:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل البوتات بالطرد") then
  local tsX_o = database:get("lock_botAndBan:i8_88E"..msg.chat_id_..bot_id)
  if tsX_o then
   tsX000("lock",msg,"☑┇بالفعل تم قفل البوتات بالطرد")
  else
  tsX000("lock",msg,"☑┇تم قفل البوتات بالطرد")
  database:set("lock_botAndBan:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل البوتات") then
  local tsX_o = database:get("lock_bot:i8_88E"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل البوتات")
  else
  tsX000("lock",msg,"☑┇تم قفل البوتات")
  database:set("lock_bot:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل بصمه الفيديو") then
  local tsX_o = database:get("lock_note:i8_88E"..msg.chat_id_..bot_id)
  if tsX_o then
  ...
  database:del("lock_new:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_tag:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_sarha:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_lllll:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_edit:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_gif:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_files:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_mark:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_photo:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_stecker:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_video:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_inline:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_chat:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_fwd:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_pin:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_audeo:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_voice:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_botAndBan:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_contact:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_ar:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_en:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_media:i8_88E"..msg.chat_id_..bot_id,"ok")
database:del("lock_audeo:i8_88E"..msg.chat_id_..bot_id,"ok")
database:del("lock_video:i8_88E"..msg.chat_id_..bot_id,"ok")
database:del("lock_photo:i8_88E"..msg.chat_id_..bot_id,"ok")
database:del("lock_stecker:i8_88E"..msg.chat_id_..bot_id,"ok")
database:del("lock_voice:i8_88E"..msg.chat_id_..bot_id,"ok")
database:del("lock_gif:i8_88E"..msg.chat_id_..bot_id,"ok")
database:del("lock_note:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_username:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_link:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_bot:i8_88E"..msg.chat_id_..bot_id,"ok")  
if is_mod(msg) then
if (text == "قفل التاك بالتقيد") then
  local tsX_o = database:get("lock_tag.note:i8_88E"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ التاك بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل  ⌁ التاك بالتقيد ✓")
  database:set("lock_tag.note:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الشارحه بالتقيد") then
  local tsX_o = database:get("lock_sarha.note:i8_88E"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ الشارحه بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل  ⌁ الشارحه بالتقيد ✓")
  database:set("lock_sarha.note:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل المتحركه بالتقيد") then
  local tsX_o = database:get("lock_gif.note:i8_88E"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ المتحركه بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل  ⌁ المتحركه بالتقيد ✓")
  database:set("lock_gif.note:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الملفات بالتقيد") then
  local tsX_o = database:get("lock_files.note:i8_88E"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ الملفات بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل  ⌁ الملفات بالتقيد ✓")
  database:set("lock_files.note:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الماركدون بالتقيد") then
  local tsX_o = database:get("lock_mark.note:i8_88E"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ الماركدون بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل  ⌁ الماركدون بالتقيد ✓")
  database:set("lock_mark.note:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الصور بالتقيد") then
  local tsX_o = database:get("lock_photo.note:i8_88E"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ الصور بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل  ⌁ الصور بالتقيد ✓")
  database:set("lock_photo.note:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الملصقات بالتقيد") then
  local tsX_o = database:get("lock_stecker.note:i8_88E"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ الملصقات بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل  ⌁ الملصقات بالتقيد ✓")
  database:set("lock_stecker.note:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الفيديو بالتقيد") then
  local tsX_o = database:get("lock_video.note:i8_88E"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ الفيديو بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل  ⌁ الفيديو بالتقيد ✓")
  database:set("lock_video.note:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الانلاين بالتقيد") then
  local tsX_o = database:get("lock_inline.note:i8_88E"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ الانلاين بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل  ⌁ الانلاين بالتقيد ✓")
  database:set("lock_inline.note:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الدردشه بالتقيد") then
  local tsX_o = database:get("lock_chat.note:i8_88E"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ الدردشه بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل  ⌁ الدردشه بالتقيد ✓")
  database:set("lock_chat.note:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل التوجيه بالتقيد") then
  local tsX_o = database:get("lock_fwd.note:i8_88E"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ التوجيه بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل  ⌁ التوجيه بالتقيد ✓")
  database:set("lock_fwd.note:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الاغاني بالتقيد") then
  local tsX_o = database:get("lock_audeo.note:i8_88E"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ الاغاني بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل  ⌁ الاغاني بالتقيد ✓")
  database:set("lock_audeo.note:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الصوت بالتقيد") then
  local tsX_o = database:get("lock_voice.note:i8_88E"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ الصوت بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل  ⌁ الصوت بالتقيد ✓")
  database:set("lock_voice.note:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الجهات بالتقيد") then
  local tsX_o = database:get("lock_contact.note:i8_88E"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ الجهات بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل  ⌁ الجهات بالتقيد ✓")
  database:set("lock_contact.note:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل العربيه بالتقيد") then
  local tsX_o = database:get("lock_ar.note:i8_88E"..msg.chat_id_..bot_id)
  if tsX_o then
    tsX000("lock",msg,"👁‍🗨 ⫶ العربيه بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل  ⌁ العربيه بالتقيد ✓")
  database:set("lock_ar.note:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الانكليزيه بالتقيد") then
  local tsX_o = database:get("lock_en.note:i8_88E"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ الانكليزيه بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل  ⌁ الانكليزيه بالتقيد ✓")
  database:set("lock_en.note:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الروابط بالتقيد") then
  local tsX_o = database:get("lock_link.note:i8_88E"..msg.chat_id_..bot_id)
  if tsX_o then
    tsX000("lock",msg,"👁‍🗨 ⫶ الروابط بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل  ⌁ الروابط بالتقيد ✓")
  database:set("lock_link.note:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل المعرف بالتقيد") then
  local tsX_o = database:get("lock_username.note:i8_88E"..msg.chat_id_..bot_id)
  if tsX_o then
   tsX000("lock",msg,"👁‍🗨 ⫶ المعرف بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل  ⌁ المعرف بالتقيد ✓")
  database:set("lock_username.note:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل بصمه الفيديو بالتقيد") then
  local tsX_o = database:get("lock_note.note:i8_88E"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ بصمه الفيديو بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل  ⌁ بصمه الفيديو بالتقيد ✓")
  database:set("lock_note.note:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
end
if is_mod(msg) then
  ...
  database:del("lock_tag.note:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_sarha.note:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_gif.note:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_files.note:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_mark.note:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_photo.note:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_stecker.note:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_video.note:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_inline.note:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_chat.note:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_fwd.note:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_audeo.note:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_voice.note:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_contact.note:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_ar.note:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_en.note:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_username.note:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_note.note:i8_88E"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_link.note:i8_88E"..msg.chat_id_..bot_id,"ok") 
end
  end  --         »»               End i8_88E unlock                       ««              --
  end 
end 
...
if database:get("lock_link:i8_88E"..msg.chat_id_..bot_id) then
...
if database:get("lock_username:i8_88E"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
end
...
if database:get("lock_chat:i8_88E"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "i8_88E"
end
if text and text:match("(.*)(/)(.*)")  then
if database:get("lock_sarha:i8_88E"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if text and text:match("(.*)(#)(.*)")  then
if database:get("lock_tag:i8_88E"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.ID == "MessageChatAddMembers" then
if msg.content_.members_[0].type_.ID == 'UserTypeBot' then
if database:get("lock_bot:i8_88E"..msg.chat_id_..bot_id) then
changeChatMemberStatus(msg.chat_id_, msg.content_.members_[0].id_, "Kicked")
end
if database:get("lock_botAndBan:i8_88E"..msg.chat_id_..bot_id) then
changeChatMemberStatus(msg.chat_id_, msg.content_.members_[0].id_, "Kicked")
...
if database:get("lock_fwd:i8_88E"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
end
if msg.content_.ID == "MessageSticker" then
if database:get("lock_stecker:i8_88E"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.ID == "MessageChatJoinByLink" or msg.content_.ID == "MessageChatAddMembers" then
if database:get("lock_new:i8_88E"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end

if msg.content_.ID == "MessageChatAddMembers" then
database:incr('i8_88E:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)
end
if msg.content_.ID == "MessageUnsupported" then
if database:get("lock_note:i8_88E"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.ID == "MessagePhoto" then
if database:get("lock_photo:i8_88E"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.ID == "MessageAudio" then
if database:get("lock_audeo:i8_88E"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.ID == "MessageVoice" then
if database:get("lock_voice:i8_88E"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.ID == "MessageVideo" then
if database:get("lock_video:i8_88E"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.ID == "MessageAnimation" then
if database:get("lock_gif:i8_88E"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.ID == "MessageContact" then
if database:get("lock_contact:i8_88E"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if text and text:match("[\216-\219][\128-\191]") then
if database:get("lock_ar:i8_88E"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.ID == "MessageDocument" then
if database:get("lock_files:i8_88E"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if text and text:match("[ASDFGHJKLQWERTYUIOPZXCVBNMasdfghjklqwertyuiopzxcvbnm]") then
if database:get("lock_en:i8_88E"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
...
if database:get("lock_mark:i8_88E"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
end
...
if database:get("lock_lllll:i8_88E"..msg.chat_id_..bot_id) then
...
local hash = 'i8_88E:'..bot_id..'flood:time:'..msg.chat_id_
if not database:get(hash) then
floodTime = 1
else
...
database:sadd('i8_88E:'..bot_id..'muted:'..msg.chat_id_, msg.sender_user_id_)
end
end
...
if not database:get('i8_88E:'..bot_id..'leave:groups') then
...
local hash =  'i8_88E:'..bot_id..'creator:'..msg.chat_id_
...
if result.id_ then
database:sadd('i8_88E:'..bot_id..'creator:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apow[2] or 'i8_88Eteam')..')\n☑┇تم رفع منشئ في البوت'
else
texts = '✖┇خطاء'
end
...
database:sadd('i8_88E:'..bot_id..'creator:'..msg.chat_id_, apow[2])
...
local user_info = database:hgetall('i8_88E:'..bot_id..'user:'..v)
...
if not database:get('i8_88E:'..bot_id..'bc:groups') then
local gps = database:scard( 'i8_88E:'..bot_id.."groups") or 0
local gpss = database:smembers( 'i8_88E:'..bot_id.."groups") or 0
for i=1, #gpss do
if not database:sismember('i8_88E:'..bot_id..'pro:groups', gpss[i]) then
send(gpss[i], 0, 1, ssss[2], 1, 'html')		
end					
end
local pro = database:scard('i8_88E:'..bot_id..'pro:groups') or 0
...
local gps = database:scard('i8_88E:'..bot_id.."groups") or 0
local user = database:scard('i8_88E:'..bot_id.."userss") or 0
local gps2 = database:scard("thsake:gog"..bot_id) or 0
local gps3 = database:scard('i8_88E:'..bot_id..'pro:groups') or 0
...
local gps = database:smembers('i8_88E:'..bot_id.."groups") or 0
local gps2 = database:smembers("thsake:gog"..bot_id) or 0
for i=1,#gps do
database:sadd("thsake:gog"..bot_id, gps[i])
database:set('i8_88E:'..bot_id.."enable:"..gps[i],true)
database:set( 'i8_88E:'..bot_id.."charge:"..gps[i],true)
end
...
database:del('i8_88E:'..bot_id.."enable:"..gps[i])
database:del( 'i8_88E:'..bot_id.."charge:"..gps[i])
end
...
database:del("thsake:gog"..bot_id)
database:del('i8_88E:'..bot_id.."groups")
end
end --
if text:match("^روابط الكروبات$") then
local gpss = database:smembers("thsake:"..bot_id.."groups") or 0
local gps = database:scard('i8_88E:'..bot_id.."groups")
text = '📊┇روابط الكروبات\n\n'
for i=1, #gpss do
local link = database:get('i8_88E:'..bot_id.."group:link"..gpss[i])
text = text.."|"..i.."| ~⪼ "..gpss[i].."\n ~⪼ "..(link or  "لا يوجد رابط").."\n"
end
 local f = io.open('i8_88E.txt', 'w')
 f:write(text)
 f:close()
 local i8_88Ee = 'https://api.telegram.org/bot' .. token .. '/sendDocument'
 local curl = 'curl "' .. i8_88Ee .. '" -F "chat_id=' .. msg.chat_id_ .. '" -F "document=@' .. 'i8_88E.txt' .. '"'
 io.popen(curl)
 end
if text:match("^تحديث السورس$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
send(msg.chat_id_, msg.id_, 1, '☑┇تم التحديث', 1, 'md')
os.execute('rm -rf ./libs/utils.lua')
os.execute('cd libs && wget https://raw.githubusercontent.com/i8_88ETEAM/i8_88E/master/libs/utils.lua')
os.execute('rm -rf i8_88E.lua')
os.execute('wget https://raw.githubusercontent.com/i8_88ETEAM/i8_88E/master/i8_88E.lua')
os.exit()
return false
end
...
local time = a[2] * day
database:setex( 'i8_88E:'..bot_id.."charge:"..msg.chat_id_,time,true)
database:set( 'i8_88E:'..bot_id.."enable:"..msg.chat_id_,true)
...
local ex = database:ttl( 'i8_88E:'..bot_id.."charge:"..txt[2])
if ex == -1 then
...
local i8_88_info = nil
function i8_88333(extra,result,success)
i8_88_info = '@'..(result.username_ or 'لا يوجد')..''
local timeplan1 = 2592000
database:setex( 'i8_88E:'..bot_id.."charge:"..txt[2],timeplan1,true)
...
database:setex( 'i8_88E:'..bot_id.."charge:"..txt[2],timeplan2,true)
...
function i8_88333(extra,result,success)
i8_88_info = '@'..(result.username_ or 'لا يوجد')..''
database:set( 'i8_88E:'..bot_id.."charge:"..txt[2],true)
...
local files_i8_88E = database:smembers("files"..bot_id)
local i8_88 = io.popen('cd files_i8_88E && ls'):read("*all")
local files_i8_88E2 = ''
for i=1,#files_i8_88E do
files_i8_88E2 = files_i8_88E2..'{'..files_i8_88E[i]..'}\n'
end
send(msg.chat_id_, msg.id_, 1, '☑┇جميع الملفات : \n '..i8_88..'\n ---------------------- \n\n✔┇الملفات المفعله \n'..files_i8_88E2..'', 1, 'html')
end
text = msg.content_.text_
if text then
...
local name_t = {string.match(text, "^(حذف ملف) (.*)(.lua)$")}
io.popen("rm -fr files_i8_88E/"..name_t[2]..'.lua')
database:srem("files"..bot_id,name_t[2]..'.lua')
...
local i8_88Ee = 'https://api.telegram.org/bot' .. token .. '/sendDocument'
local curl = 'curl "' .. i8_88Ee .. '" -F "chat_id=' .. msg.chat_id_ .. '" -F "document=@' .. 'files_i8_88E/'..name_t[2]..'.lua' .. '"'
io.popen(curl)
end
end
end -- end if sudo
...
local i8_88j = {string.match(text, "^(تعين عدد الاعضاء) (.*)$")}
database:set("ts_a"..bot_id,i8_88j[2])
send(msg.chat_id_, msg.id_, 1, "🔘┇ تم تعين : "..i8_88j[2], 1, 'html')
end
end
end
end -- end function sudo

function i8_88E_run_file(data)
local files_i8_88E = database:smembers("files"..bot_id)
for i=1,#files_i8_88E do
local i8_88Eee = dofile("files_i8_88E/"..files_i8_88E[i])
local kt = i8_88Eee.i8_88_i8_88E(data)
if kt == 'end' then
return false
end
end
...
local photo = database:get('i8_88E:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_)
if photo then
local idPh = nil
...
database:del('i8_88E:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_)
end end
...
return false end
  ...
  if not database:get( 'i8_88E:'..bot_id.."charge:"..msg.chat_id_) then
  if database:get( 'i8_88E:'..bot_id.."enable:"..msg.chat_id_) then
  database:del( 'i8_88E:'..bot_id.."enable:"..msg.chat_id_)
  for k,v in pairs(sudo_users) do
  end
  end
  end
    ...
    if not database:sismember( 'i8_88E:'..bot_id.."groups",msg.chat_id_) then
    database:sadd( 'i8_88E:'..bot_id.."groups",msg.chat_id_)
    end
    elseif id:match('^(%d+)') then
    database:sadd('i8_88E:'..bot_id.."userss",msg.chat_id_)
    else
    if not database:sismember( 'i8_88E:'..bot_id.."groups",msg.chat_id_) then
    database:sadd( 'i8_88E:'..bot_id.."groups",msg.chat_id_)
    end
  ...
  database:incr('i8_88E:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
  if database:get('i8_88E:'..bot_id..'viewget'..msg.sender_user_id_) then
  if not msg.forward_info_ then
  send(msg.chat_id_, msg.id_, 1, '❕┇قم بارسال المنشور من القناة', 1, 'md')
  database:del('i8_88E:'..bot_id..'viewget'..msg.sender_user_id_)
  else
  ...
  local hash =  'i8_88E:'..bot_id..'mods:'..msg.chat_id_
  if database:sismember(hash, result.sender_user_id_) then
  ...
  if result.id_ then
  database:sadd('i8_88E:'..bot_id..'mods:'..msg.chat_id_, result.id_)
  texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'i8_88Eteam')..')\n☑┇تم رفعه ادمن'
  else
  texts = '✖┇خطاء'
  end
  send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
  ...
  database:sadd('i8_88E:'..bot_id..'mods:'..msg.chat_id_, apmd[2])
  ...
  function promote_by_reply(extra, result, success)
  local hash =  'i8_88E:'..bot_id..'vipgp:'..msg.chat_id_
  if database:sismember(hash, result.sender_user_id_) then
tsX000("prore",msg,"☑┇بالفعل تم رفعه عضو مميز") 
  ...
  function promote_by_username(extra, result, success)
  if result.id_ then
  database:sadd('i8_88E:'..bot_id..'vipgp:'..msg.chat_id_, result.id_)
  texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'i8_88Eteam')..')\n☑┇تم رفعه عضو مميز'
  else
  texts = '✖┇خطاء'
  end
  ...
  database:sadd('i8_88E:'..bot_id..'vipgp:'..msg.chat_id_, apmd[2])
  ...
  database:set("tsahke:set:id:"..bot_id..msg.chat_id_..msg.sender_user_id_,'i8_88E')
  return "i8_88E"
  end
  ...
  database:set("i8_88E:gr:id:text:"..bot_id..msg.chat_id_,text)
  end
  if text and text == "مسح الايدي" and is_owner(msg) then
  send(msg.chat_id_, msg.id_, 1,  '✖┇ تم المسح بنجاح', 1, 'md')
  database:del("i8_88E:gr:id:text:"..bot_id..msg.chat_id_)
  end
  ...
  tsX000(apmd[2],msg,"☑┇تم تنزيله من اعضاء الممزين البوت")
    ...
    local hash =  'i8_88E:'..bot_id..'banned:'..msg.chat_id_
 ...
 chat_kick(result.chat_id_, result.sender_user_id_)
  ...
  send(msg.chat_id_, msg.id_, 1, '❕┇لا تستطيع حظر \n🔘┇(مدراء،ادمنيه،اعضاء مميزين)البوت', 1, 'md')
      ...
      database:sadd('i8_88E:'..bot_id..'banned:'..msg.chat_id_, result.id_)
      texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apba[2] or 'i8_88Eteam')..')\n☑┇تم حظره من المجموعه'
  ...
  send(msg.chat_id_, msg.id_, 1, '❕┇لا تستطيع حظر \n🔘┇(مدراء،ادمنيه،اعضاء مميزين)البوت', 1, 'md')
      ...
      database:sadd('i8_88E:'..bot_id..'banned:'..msg.chat_id_, apba[2])
  ...
  if database:get('i8_88E:'..bot_id.."group:link"..msg.chat_id_) == 'Waiting For Link!\nPls Send Group Link' and is_mod(msg) then 
  ...
  local hash = 'i8_88E:'..bot_id.."group:link"..msg.chat_id_ 
  ...
  send(msg.chat_id_, 0, 1, '↙️┇رابط المجموعه الجديد\n'..glink, 1, 'html')
   ...
   if result.id_ then
     database:srem('i8_88E:'..bot_id..'banned:'..msg.chat_id_, result.id_)
  texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apba[2] or 'i8_88Eteam')..')\n☑┇تم الغاء حظره من البوت' 
   ...
   local apba = {string.match(text, "^(الغاء حظر) (%d+)$")}
     database:srem('i8_88E:'..bot_id..'banned:'..msg.chat_id_, apba[2])
  ...
  function gban_by_reply(extra, result, success)
    local hash =  'i8_88E:'..bot_id..'gbanned:'
  ...
  send(msg.chat_id_, msg.id_, 1, '❕┇لا تستطيع حظر عام \n🔘┇(مدراء،ادمنيه،اعضاء مميزين)البوت', 1, 'md')
    ...
    local hash =  'i8_88E:'..bot_id..'gbanned:'
  texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apbll[2] or 'i8_88Eteam')..')\n🚫┇تم حظره من المجموعات البوت'
  ...
  function ungban_by_username(extra, result, success)
    local hash =  'i8_88E:'..bot_id..'gbanned:'
    if result.id_ then
  texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apid[2] or 'i8_88Eteam')..')\n🚫┇تم الغاء حظره من المجموعات البوت'
  database:srem(hash, result.id_)
    else
    texts = '✖┇خطاء'
    end
  ...
  function gmute_by_reply(extra, result, success)
    local hash =  'i8_88E:'..bot_id..'gmuted:'
    if is_admin(result) then
...
tsX000("prore",msg,"🚫┇تم الغاء كتمه من المجموعات البوت")
  ...
  tsX000(apbll[2],msg,"🚫┇تم الغاء كتمه من المجموعات البوت")
    ...
    local hash =  'i8_88E:'..bot_id..'muted:'..msg.chat_id_
    if ck_mod(result.sender_user_id_, msg.chat_id_) then
...
tsX000("prore",msg,"🚫┇بالفعل تم كتمه")
  ...
  send(msg.chat_id_, msg.id_, 1, '✖┇لا تستطيع كتم \n🔘┇(مدراء،ادمنيه،اعضاء مميزين)البوت', 1, 'md')
      ...
      database:sadd('i8_88E:'..bot_id..'muted:'..msg.chat_id_, result.id_)
    texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apsi[2] or 'i8_88Eteam')..')\n🚫┇تم كتمه من البوت'
    end
  else
    texts = '✖┇خطاء'
  end
...
database:sadd('i8_88E:'..bot_id..'muted:'..msg.chat_id_, apsi[2])
tsX000(apsi[2],msg,"🚫┇تم كتمه من البوت")
    ...
    if result.id_ then
     database:srem('i8_88E:'..bot_id..'muted:'..msg.chat_id_, result.id_)
   texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apsi[2] or 'i8_88Eteam')..')\n🚫┇تم الغاء كتمه من البوت'
    ...
    local apsi = {string.match(text, "^(الغاء كتم) (%d+)$")}
      database:srem('i8_88E:'..bot_id..'muted:'..msg.chat_id_, apsi[2])
  ...
  send(msg.chat_id_, msg.id_, 1, '✖┇لا تستطيع طرد \n🔘┇(مدراء،ادمنيه،اعضاء مميزين)البوت', 1, 'md')
    ...
    texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apki[2] or 'i8_88Eteam')..')\n🚫┇تم طرده من المجموعه'
       chat_kick(msg.chat_id_, result.id_)
    end
  else
    texts = '✖┇خطاء'
  ...
  send(msg.chat_id_, msg.id_, 1, '✖┇لا تستطيع طرد \n🔘┇(مدراء،ادمنيه،اعضاء مميزين)البوت', 1, 'md')
    ...
    local hash =  'i8_88E:'..bot_id..'owners:'..msg.chat_id_
    if database:sismember(hash, result.sender_user_id_) then
    ...
    if result.id_ then
      database:sadd('i8_88E:'..bot_id..'owners:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apow[2] or 'i8_88Eteam')..')\n☑┇تم رفع مدير في البوت'
    ...
    local apow = {string.match(text, "^(رفع مدير) (%d+)$")}
      database:sadd('i8_88E:'..bot_id..'owners:'..msg.chat_id_, apow[2])
  ...
  -----------------------------------------------
f (text and text == 'قفل الفشار' and is_mod(msg)) then 
send(msg.chat_id_, msg.id_, 1,"☑️┇بالتأكيد تم قفل  الفشار", 1, 'html')
database:set("i8_88:bantext"..bot_id..msg.chat_id_,"i8_88")
end
if (text and text == 'فتح الفشار' and is_mod(msg)) then
send(msg.chat_id_, msg.id_, 1,"☑️┇تم فتح الفشار",1, 'html')
database:del("i8_88:bantext"..bot_id..msg.chat_id_)
end
local ii8_88 = database:get("i8_88:bantext"..bot_id..msg.chat_id_)
if (ii8_88 and ii8_88 == 'i8_88') then
if (not is_mod(msg) and text) then 
local i8_88 = { -- the List By : i8_88ETEAM
        'عير',
        'كس',
        'كحبه', -- the List By : i8_88ETEAM
        'كساسه',
        'مناويج',
        'تنيجون',
        'سكسي',
        ...
        'پسر کونی',
        'کیرم تو مادرت',
        'کیرم تو خانوادت',-- the List By : i8_88ETEAM
        'پدر سگ',
        'پدر کونی',
        'خواهرت گاییدم',
        'مادرت گاییدم'
} -- the List By : i8_88ETEAM
function delete_msg(chatid,mid)
    tdcli_function ({
    ID="DeleteMessages",
    chat_id_=chatid,
...
delete_msg(msg.chat_id_,{[0] = msg.id_})
  ...
  if text:match("^جلب صوره (%d+)$") and msg.reply_to_message_id_ == 0 and not database:get('i8_88E:'..bot_id..'get:photo'..msg.chat_id_) then
...
local ph = pronumb[2] - 1
  ...
  database:set('i8_88E:'..bot_id..'flood:max:'..msg.chat_id_,floodmax[2])
  ...
  database:set('i8_88E:'..bot_id..'flood:time:'..msg.chat_id_,floodt[2])
    ...
    local welcome = {string.match(text, "^(وضع ترحيب) (.*)$")}
     send(msg.chat_id_, msg.id_, 1, '☑┇تم وضع ترحيب\n📜┇~⪼('..welcome[2]..')', 1, 'md')
       database:set('i8_88E:'..bot_id..'welcome:'..msg.chat_id_,welcome[2])
    end
    ...
    local wel = database:get('i8_88E:'..bot_id..'welcome:'..msg.chat_id_)
    if wel then
  ...
  database:hset('i8_88E:'..bot_id..'filters:'..msg.chat_id_, name, 'filtered')
  ...
  database:hdel('i8_88E:'..bot_id..'filters:'..msg.chat_id_, rws[2])
  ...
  if (text:match("^كشف البوتات$") or text:match("^البوتات$")) and is_mod(msg) then
   ...
   n = (n + 1)
     local user_info = database:hgetall('i8_88E:'..bot_id..'user:'..v.user_id_)
  if user_info and user_info.username then
   local username = user_info.username
...
text = ""
  ...
  local user_msgs = database:get('i8_88E:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
   if not database:get('i8_88E:'..bot_id..'id:mute'..msg.chat_id_) then
...
add = (tonumber(database:get('i8_88E:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)) or 0)
  send(msg.chat_id_, msg.id_, 1, "📨┇عدد اضافه جهاتك ~⪼ *{"..add.."}*\n📨┇سيتم حذف العدد بعد هذه الرساله", 1, 'md')
database:del('i8_88E:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)
  end
  if text:match("^(عدد السحكات)$") or text:match("^(سحكاتي)$") then
  local edit = database:get('i8_88E:'..bot_id..'user:editmsg'..msg.chat_id_..':'..msg.sender_user_id_) or 0
  ...
  database:del('i8_88E:'..bot_id..'banned:'..msg.chat_id_)
  ...
  database:del('i8_88E:'..bot_id..'creator:'..msg.chat_id_)
...
text = ""
  ...
  send(msg.chat_id_, msg.id_, 1, moody, 1, 'html')
   ...
   bot.channel_get_admins(msg.chat_id_,cb)
    ...
    if database:get("lock_media:i8_88E"..msg.chat_id_..bot_id) then
       mute_all = '✔┇'
       else
       mute_all = '✖┇'
       end
       ...
       ------------
       if not database:get('i8_88E:'..bot_id..'flood:max:'..msg.chat_id_) then
       flood_m = 10
       else
       flood_m = database:get('i8_88E:'..bot_id..'flood:max:'..msg.chat_id_)
     end
       if not database:get( 'i8_88E:'..bot_id..'flood:time:'..msg.chat_id_) then
       flood_t = 10
       else
       flood_t = database:get( 'i8_88E:'..bot_id..'flood:time:'..msg.chat_id_)
     end
       ------------
       if database:get("lock_audeo:i8_88E"..msg.chat_id_..bot_id) then
       mute_music = '✔┇'
       ...
       ------------
       if database:get("lock_inline:i8_88E"..msg.chat_id_..bot_id) then
       mute_in = '✔┇'
       else
       mute_in = '✖┇'
       end
       ...
       ------------
       if database:get("lock_edit:i8_88E"..msg.chat_id_..bot_id) then
       mute_edit = '✔┇'
       else
       mute_edit = '✖┇'
       end
   ...
   ------------
       if database:get("lock_pin:i8_88E"..msg.chat_id_..bot_id) then
       lock_pin = '✔┇'
       else
       lock_pin = '✖┇'
     end
  ...
  if database:get("lock_contact:i8_88E"..msg.chat_id_..bot_id) then
   ...
   if database:get('i8_88E:'..bot_id..'rep:mute'..msg.chat_id_) then
       lock_rep = '✔┇'
       else
       lock_rep = '✖┇'
       end
       ------------
   if database:get('i8_88E:'..bot_id..'repsudo:mute'..msg.chat_id_) then
       lock_repsudo = '✔┇'
       else
       lock_repsudo = '✖┇'
       end
       ------------
   if database:get('i8_88E:'..bot_id..'repowner:mute'..msg.chat_id_) then
       lock_repowner = '✔┇'
       else
       lock_repowner = '✖┇'
       end
       ------------
   if database:get('i8_88E:'..bot_id..'id:mute'..msg.chat_id_) then
       lock_id = '✔┇'
       else
       lock_id = '✖┇'
       end
       ------------
   if database:get('i8_88E:'..bot_id..'pin:mute'..msg.chat_id_) then
       lock_pind = '✔┇'
       else
       lock_pind = '✖┇'
       end
   ...
   if database:get('i8_88E:'..bot_id..'id:mute'..msg.chat_id_) then
       ...
       ------------
if database:get( 'i8_88E:'..bot_id.."welcome"..msg.chat_id_) then
send_welcome = '✔┇'
else
send_welcome = '✖┇'
end
    ------------
local ex = database:ttl( 'i8_88E:'..bot_id.."charge:"..msg.chat_id_)
  if ex == -1 then
  exp_dat = 'لا نهائي'
  else
  ...
  if (text and text == 'تفعيل اطردني') and is_owner(msg) then
     if not database:get('i8_88E:'..bot_id..'kickme:mute'..msg.chat_id_) then
   send(msg.chat_id_, msg.id_, 1, '☑┇امر اطردني بالفعل تم تفعيله', 1, 'md')
     else
   send(msg.chat_id_, msg.id_, 1, '☑┇تم تفعيل امر اطردني', 1, 'md')
    database:del('i8_88E:'..bot_id..'kickme:mute'..msg.chat_id_)
   end
   end
   if (text and text == 'تعطيل اطردني') and is_owner(msg) then
     if database:get('i8_88E:'..bot_id..'kickme:mute'..msg.chat_id_) then
   send(msg.chat_id_, msg.id_, 1, '☑┇امر اطردني بالفعل تم تعطيله', 1, 'md')
   else
   send(msg.chat_id_, msg.id_, 1, '☑┇تم تعطيل امر اطردني', 1, 'md')
     database:set('i8_88E:'..bot_id..'kickme:mute'..msg.chat_id_,true)
   end
     end

  if text:match("^اطردني$") then
  if not database:get('i8_88E:'..bot_id..'kickme:mute'..msg.chat_id_) then
  redis:set('i8_88E:'..bot_id..'kickyess'..msg.sender_user_id_..'', 'kickyes')
  redis:set('i8_88E:'..bot_id..'kicknoo'..msg.sender_user_id_..'', 'kickno')
  send(msg.chat_id_, msg.id_, 1, '🚷┇ ارسل ؛ نعم ، ليتم طردك\n🔘┇ارسل ؛ لا ، لالغاء الامر', 1, 'md')
  else
  send(msg.chat_id_, msg.id_, 1, '⚠┇تم تعطيل امر اطردني', 1, 'md')
  end
  end
  local yess = redis:get('i8_88E:'..bot_id..'kickyess'..msg.sender_user_id_..'')
  ...
  if yess == 'kickyes' then
  chat_kick(msg.chat_id_, msg.sender_user_id_)
  redis:del('i8_88E:'..bot_id..'kickyess'..msg.sender_user_id_..'', 'kickyes')
  redis:del('i8_88E:'..bot_id..'kicknoo'..msg.sender_user_id_..'', 'kickno')
  send(msg.chat_id_, msg.id_, 1, '✅┇تم طردك من المجموعه', 1, 'md')
    end
    end
  end
  if text:match("^لا$") then
  local noo = redis:get('i8_88E:'..bot_id..'kicknoo'..msg.sender_user_id_..'')
  if noo == 'kickno' then
  redis:del('i8_88E:'..bot_id..'kickyess'..msg.sender_user_id_..'', 'kickyes')
  redis:del('i8_88E:'..bot_id..'kicknoo'..msg.sender_user_id_..'', 'kickno')
  ...
  redis:set('i8_88E:'..bot_id..'texts'..msg.sender_user_id_..'', 'msg')
    return false end
  if text:match("^(.*)$") then
  local i8_882 = redis:get('i8_88E:'..bot_id..'texts'..msg.sender_user_id_..'')
  if i8_882 == 'msg' then
  send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
  redis:set('i8_88E:'..bot_id..'texts'..msg.sender_user_id_..'', 'no')
  redis:set('i8_88E:'..bot_id..'text_sudo', text)
  send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
    return false end
   end
  if  (text and text == 'مسح امر المطور بالكليشه') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
   redis:del('i8_88E:'..bot_id..'text_sudo', text)
  ...
  if text:match("^[Dd][Ee][Vv]$")or text:match("^مطور بوت$") or text:match("^مطورين$") or text:match("^مطور البوت$") or text:match("^مطور$") or text:match("^المطور$") and msg.reply_to_message_id_ == 0 then
    local text_sudo = redis:get('i8_88E:'..bot_id..'text_sudo')
  local ni8_88 = redis:get('i8_88E:'..bot_id..'nmi8_88')
  local nai8_88 = redis:get('i8_88E:'..bot_id..'nai8_88')
    if text_sudo then
    send(msg.chat_id_, msg.id_, 1, text_sudo, 1, 'md')
    else
    sendContact(msg.chat_id_, msg.id_, 0, 1, nil, (ni8_88 or 9647819831695), (nai8_88 or "i8_88E TEAM"), "", bot_id)
  end
   end
    for k,v in pairs(sudo_users) do
  ...
  redis:set('i8_88E:'..bot_id..'ni8_88'..msg.sender_user_id_..'', 'msg')
    return false end
  end
  if text:match("^+(.*)$") then
  local i8_88o = redis:get('i8_88E:'..bot_id..'sudoo'..text..'')
  local i8_882 = redis:get('i8_88E:'..bot_id..'ni8_88'..msg.sender_user_id_..'')
  if i8_882 == 'msg' then
  send(msg.chat_id_, msg.id_, 1, '• `الان يمكنك ارسال الاسم الذي تريده` 🏷', 1, 'md')
  redis:set('i8_88E:'..bot_id..'nmi8_88', text)
  redis:set('i8_88E:'..bot_id..'ni8_88'..msg.sender_user_id_..'', 'mmsg')
    return false end
  end
  if text:match("^(.*)$") then
  local i8_882 = redis:get('i8_88E:'..bot_id..'ni8_88'..msg.sender_user_id_..'')
  ...
  redis:set('i8_88E:'..bot_id..'ni8_88'..msg.sender_user_id_..'', 'no')
  redis:set('i8_88E:'..bot_id..'nai8_88', text)
  local nmi8_88 = redis:get('i8_88E:'..bot_id..'nmi8_88')
  sendContact(msg.chat_id_, msg.id_, 0, 1, nil, nmi8_88, text , "", bot_id)
    return false end
  end

    if text:match("^اضف مطور$")  and tonumber(msg.sender_user_id_) == tonumber(sudo_add) and msg.reply_to_message_id_ then
    function promote_by_reply(extra, result, success)
    if redis:sismember('i8_88E:'..bot_id..'dev', result.sender_user_id_) then
	tsX000("prore",msg,'☑┇بالفعل تم رفعه مطور')
  else
    redis:set('i8_88E:'..bot_id..'sudoo'..result.sender_user_id_..'', 'yes')
    redis:sadd('i8_88E:'..bot_id..'dev', result.sender_user_id_)
    ...
    function promote_by_username(extra, result, success)
    if result.id_ then
    redis:set('i8_88E:'..bot_id..'sudoo'..result.id_..'', 'yes')
    redis:sadd('i8_88E:'..bot_id..'dev', result.id_)
      texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'i8_88Eteam')..')\n\n☑┇تم رفعه مطور'
  else
    texts = '✖┇خطاء'
  end
    ...
    redis:set('i8_88E:'..bot_id..'sudoo'..apmd[2]..'', 'yes')
    redis:sadd('i8_88E:'..bot_id..'dev', apmd[2])
    ...
    function demote_by_reply(extra, result, success)
    if not redis:sismember('i8_88E:'..bot_id..'dev', result.sender_user_id_) then
	tsX000("prore",msg,'☑┇ بالفعل تم تنزيله من المطورين')
    else
    redis:del('i8_88E:'..bot_id..'sudoo'..result.sender_user_id_..'', 'no')
    redis:srem('i8_88E:'..bot_id..'dev', result.sender_user_id_)
    ...
    function demote_by_username(extra, result, success)
    if result.id_ then
    redis:del('i8_88E:'..bot_id..'sudoo'..result.id_..'', 'no')
    redis:srem('i8_88E:'..bot_id..'dev', result.id_)
    ...
    redis:del('i8_88E:'..bot_id..'sudoo'..apmd[2]..'', 'no')
    redis:srem('i8_88E:'..bot_id..'dev', apmd[2])
  ...
  redis:set('i8_88E:'..bot_id..'i8_881'..msg.sender_user_id_..''..msg.chat_id_..'', 'msg')
    return false end
  if text:match("^(.*)$") then
  if not database:get('i8_88E:'..bot_id..'repowner:mute'..msg.chat_id_) then
  local i8_88 = redis:get('i8_88E:'..bot_id..'i8_88'..text..''..msg.chat_id_..'')
  send(msg.chat_id_, msg.id_, 1, i8_88, 1, 'md')
  end
  local i8_881 = redis:get('i8_88E:'..bot_id..'i8_881'..msg.sender_user_id_..''..msg.chat_id_..'')
  if i8_881 == 'msg' then
  send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل الرد الذي تريد اضافته', 1, 'md')
  redis:set('i8_88E:'..bot_id..'i8_881'..msg.sender_user_id_..''..msg.chat_id_..'', 're')
  redis:set('i8_88E:'..bot_id..'msg'..msg.sender_user_id_..''..msg.chat_id_..'', text)
  redis:sadd('i8_88E:'..bot_id..'repowner'..msg.sender_user_id_..''..msg.chat_id_..'', text)
    return false end
  if i8_881 == 're' then
  local i8_882 = redis:get('i8_88E:'..bot_id..'msg'..msg.sender_user_id_..''..msg.chat_id_..'')
  redis:set('i8_88E:'..bot_id..'i8_88'..i8_882..''..msg.chat_id_..'', text)
  redis:sadd('i8_88E:'..bot_id..'i8_88re'..msg.chat_id_..'', i8_882)
  send(msg.chat_id_, msg.id_, 1, "☑┇تم حفظ الرد", 1, 'md')
  redis:set('i8_88E:'..bot_id..'i8_881'..msg.sender_user_id_..''..msg.chat_id_..'', 'no')
  end
  end
    if text:match("^حذف رد$") and is_owner(msg) then
  send(msg.chat_id_, msg.id_, 1, '📥┇ارسل الكلمه التي تريد حذفها', 1, 'md')
  redis:set('i8_88E:'..bot_id..'i8_881'..msg.sender_user_id_..''..msg.chat_id_..'', 'nomsg')
    return false end
  if text:match("^(.*)$") then
  local i8_881 = redis:get('i8_88E:'..bot_id..'i8_881'..msg.sender_user_id_..''..msg.chat_id_..'')
  if i8_881 == 'nomsg' then
  send(msg.chat_id_, msg.id_, 1, '☑┇تم حذف الرد', 1, 'md')
  redis:set('i8_88E:'..bot_id..'i8_881'..msg.sender_user_id_..''..msg.chat_id_..'', 'no')
  redis:set('i8_88E:'..bot_id..'i8_88'..text..''..msg.chat_id_..'', " ")
 redis:srem('i8_88E:'..bot_id..'i8_88re'..msg.chat_id_..'', text)
   end
  end
  ...
  redis:set('i8_88E:'..bot_id..'i8_881'..msg.sender_user_id_..'', 'msg')
    return false end
  if text:match("^(.*)$") then
  if not database:get('i8_88E:'..bot_id..'repsudo:mute'..msg.chat_id_) then
  local i8_88 = redis:get('i8_88E:'..bot_id..'i8_88'..text..'')
  send(msg.chat_id_, msg.id_, 1, i8_88, 1, 'md')
  end
  local i8_881 = redis:get('i8_88E:'..bot_id..'i8_881'..msg.sender_user_id_..'')
  if i8_881 == 'msg' then
  send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل الرد الذي تريد اضافته', 1, 'md')
  redis:set('i8_88E:'..bot_id..'i8_881'..msg.sender_user_id_..'', 're')
  redis:set('i8_88E:'..bot_id..'msg'..msg.sender_user_id_..'', text)
    return false end
  if i8_881 == 're' then
  local i8_882 = redis:get('i8_88E:'..bot_id..'msg'..msg.sender_user_id_..'')
  redis:set('i8_88E:'..bot_id..'i8_88'..i8_882..'', text)
  redis:sadd('i8_88E:'..bot_id..'i8_88resudo', i8_882)
  send(msg.chat_id_, msg.id_, 1, "☑┇تم حفظ الرد", 1, 'md')
  redis:set('i8_88E:'..bot_id..'i8_881'..msg.sender_user_id_..'', 'no')
  end
  end

  if text:match("^حذف رد للكل$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add)  then
  send(msg.chat_id_, msg.id_, 1, '📥┇ارسل الكلمه التي تريد حذفها' , 1, 'md')
  redis:set('i8_88E:'..bot_id..'i8_881'..msg.sender_user_id_..'', 'nomsg')
    return false end
  if text:match("^(.*)$") then
  local i8_881 = redis:get('i8_88E:'..bot_id..'i8_881'..msg.sender_user_id_..'')
  if i8_881 == 'nomsg' then
  send(msg.chat_id_, msg.id_, 1, '☑┇تم حذف الرد', 1, 'md')
  redis:set('i8_88E:'..bot_id..'i8_881'..msg.sender_user_id_..'', 'no')
   redis:set('i8_88E:'..bot_id..'i8_88'..text..'', " ")
   redis:srem('i8_88E:'..bot_id..'i8_88resudo', text)
   end
  end

  if text:match("^مسح المطورين$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
    local list = redis:smembers('i8_88E:'..bot_id..'dev')
    for k,v in pairs(list) do
  redis:del('i8_88E:'..bot_id..'dev', text)
  redis:del('i8_88E:'..bot_id..'sudoo'..v..'', 'no')
  end
    send(msg.chat_id_, msg.id_, 1, "☑┇تم مسح مطورين البوت", 1, 'md')
    end
    if text:match("^مسح ردود المدير$") and is_owner(msg) then
    local list = redis:smembers('i8_88E:'..bot_id..'i8_88re'..msg.chat_id_..'')
    for k,v in pairs(list) do
  redis:del('i8_88E:'..bot_id..'i8_88re'..msg.chat_id_..'', text)
  redis:set('i8_88E:'..bot_id..'i8_88'..v..''..msg.chat_id_..'', " ")
  end
    ...
    local list = redis:smembers('i8_88E:'..bot_id..'i8_88resudo')
    for k,v in pairs(list) do
  redis:del('i8_88E:'..bot_id..'i8_88resudo', text)
  redis:set('i8_88E:'..bot_id..'i8_88'..v..'', " ")
  end
  ...
  text = "❕┇لا يوجد ردود للمدير"
    ...
    database:set('i8_88E:'..bot_id..'rules'..msg.chat_id_, txt[2])
     send(msg.chat_id_, msg.id_, 1, "✔┇تم وضع القوانين للمجموعه", 1, 'md')
    end
    
      if  text:match("^القوانين$") then
    local rules = database:get('i8_88E:'..bot_id..'rules'..msg.chat_id_)
    if rules then
  ...
  database:set('i8_88E:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_,true)
  ...
  ----------------------------------------------------------------------------------------------
 if not database:get('i8_88E:'..bot_id..'rep:mute'..msg.chat_id_) then 
if text == 'هلو' then 
...
if  (text and text == 'تفعيل ردود البوت') and is_owner(msg) then
  ...
  send(msg.chat_id_, msg.id_, 1, '☑┇تم تفعيل ردود البوت', 1, 'md')
   database:del('i8_88E :'..bot_id..'rep:mute'..msg.chat_id_)
  end
  end
...
if (text and text == 'تفعيل جلب صوره') and is_owner(msg) then
  ...
  if database:sismember('i8_88E:'..bot_id..'pro:groups',msg.chat_id_) then
    ...
    send(msg.chat_id_, msg.id_, 1, '☑┇تم اضافه المجموعه الى الوضع المدفوع', 1, 'md')
     database:sadd('i8_88E:'..bot_id..'pro:groups',msg.chat_id_)
  end
  end
  if   (text and text == 'تعطيل وضع المدفوع') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
  if not database:sismember('i8_88E:'..bot_id..'pro:groups',msg.chat_id_) then
  ...
  database:srem('i8_88E:'..bot_id..'pro:groups',msg.chat_id_)
  end
  end
  ...
  send(msg.chat_id_, msg.id_, 1, '☑┇تم تفعيل ردود المطور', 1, 'md')
   database:del('i8_88E:'..bot_id..'repsudo:mute'..msg.chat_id_)
  end
  end
  ...
  send(msg.chat_id_, msg.id_, 1, 'تم تعطيل ردود المطور', 1, 'md')
    database:set('i8_88E:'..bot_id..'repsudo:mute'..msg.chat_id_,true)
  end
    end
  ...
  send(msg.chat_id_, msg.id_, 1, '☑┇تم تفعيل ردود المدير', 1, 'md')
   database:del('i8_88E:'..bot_id..'repowner:mute'..msg.chat_id_)
  end
  end
  ...
  send(msg.chat_id_, msg.id_, 1, '☑┇تم تعطيل ردود المدير', 1, 'md')
    database:set('i8_88E:'..bot_id..'repowner:mute'..msg.chat_id_,true)
  end
    end
  ...
  send(msg.chat_id_, msg.id_, 1, '☑┇تم تفعيل الايدي', 1, 'md')
   database:del('i8_88E:'..bot_id..'id:mute'..msg.chat_id_)
  end
  end
  ...
  send(msg.chat_id_, msg.id_, 1, '☑┇تم تعطيل الايدي', 1, 'md')
    database:set('i8_88E:'..bot_id..'id:mute'..msg.chat_id_,true)
  end
  end
  ...
  database:set("i8_88E"..bot_id..":sudo:gr",msg.chat_id_)
  end
  ...
  database:del("i8_88E"..bot_id..":sudo:gr")
  end
  if text:match("^ايدي$") and msg.reply_to_message_id_ == 0 then
  local i8_88_info = nil
  function i8_88333(extra,result,success)
  ...
  if not database:get('i8_88E:'..bot_id..'id:mute'..msg.chat_id_) then
  if not database:get('i8_88E:'..bot_id..'id:photo'..msg.chat_id_) then
  if not database:get("i8_88E:gr:id:text:"..bot_id..msg.chat_id_) then 
  local i8_88_text = {
 ' ياحلو مين الله جابك🤧',
  " غير صورتك 😪 ",
  " تخليني♥️ ",
...
i8_883 = math.random(#i8_88_text)
  sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, result.photos_[0].sizes_[1].photo_.persistent_id_," 📷┇ "..i8_88_text[i8_883].."\n🎫┇ايديك ~⪼ ("..msg.sender_user_id_..")\n📜┇معرفك ~⪼ "..i8_88_info.."\n📡┇موقعك ~⪼ "..t.."\n📨┇رسائلك ~⪼ {"..user_msgs.."}\n📧┇السحكات ~⪼ {"..edit.."}\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ",msg.id_,msg.id_.."")
  else 
  local i8_88E_new_text = database:get("i8_88E:gr:id:text:"..bot_id..msg.chat_id_)
  local i8_88E_new_text = i8_88E_new_text:gsub('#username',(i8_88_info or 'لا يوجد'))
  local i8_88E_new_text = i8_88E_new_text:gsub('#id',(msg.sender_user_id_ or 'لا يوجد'))
  local i8_88E_new_text = i8_88E_new_text:gsub('#edit',(edit or 'لا يوجد'))
  local i8_88E_new_text = i8_88E_new_text:gsub('#msgs',(user_msgs or 'لا يوجد'))
  local i8_88E_new_text = i8_88E_new_text:gsub('#stast',(t or 'لا يوجد'))
  sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, result.photos_[0].sizes_[1].photo_.persistent_id_,i8_88E_new_text,msg.id_,msg.id_.."")
  end
  else
  if is_sudo(msg) then
  t = 'مطور البوت'
  ...
  local i8_88E_new_text = i8_88E_new_text:gsub('#msgs',(user_msgs or 'لا يوجد'))
  local i8_88E_new_text = i8_88E_new_text:gsub('#stast',(t or 'لا يوجد'))   
  send(msg.chat_id_, msg.id_, 1, i8_88E_new_text, 1, 'html')
  end   
  end
  else
  end
  ...
  if  (text and text == 'تفعيل التثبيت') and is_owner(msg) then
     if not database:get('i8_88E:'..bot_id..'pin:mute'..msg.chat_id_) then
   send(msg.chat_id_, msg.id_, 1, '☑┇التثبيت بالفعل تم تفعيله', 1, 'md')
     else
   send(msg.chat_id_, msg.id_, 1, '☑┇تم تفعيل التثبيت', 1, 'md')
    database:del('i8_88E:'..bot_id..'pin:mute'..msg.chat_id_)
   end
   end
   ...
   send(msg.chat_id_, msg.id_, 1, '☑┇تم تعطيل التثبيت', 1, 'md')
     database:set('i8_88E:'..bot_id..'pin:mute'..msg.chat_id_,true)
   end
     end
   ...
   pin(msg.chat_id_,msg.reply_to_message_id_,0)
       database:set('i8_88E:'..bot_id..'pinnedmsg'..msg.chat_id_,msg.reply_to_message_id_)
    ...
    database:set('i8_88E:'..bot_id..'viewget'..msg.sender_user_id_,true)
  ...
  redis:del('i8_88E:'..bot_id..'help', text)
  redis:del('i8_88E:'..bot_id..'h1', text)
  redis:del('i8_88E:'..bot_id..'h2', text)
  redis:del('i8_88E:'..bot_id..'h3', text)
  redis:del('i8_88E:'..bot_id..'h4', text)
  redis:del('i8_88E:'..bot_id..'h5', text)
  redis:del('i8_88E:'..bot_id..'h6', text)
  ...
  send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
  redis:set('i8_88E:'..bot_id..'hhh'..msg.sender_user_id_..'', 'msg')
    return false end
  if text:match("^(.*)$") then
  local i8_882 = redis:get('i8_88E:'..bot_id..'hhh'..msg.sender_user_id_..'')
  if i8_882 == 'msg' then
  send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
  redis:set('i8_88E:'..bot_id..'hhh'..msg.sender_user_id_..'', 'no')
  redis:set('i8_88E:'..bot_id..'help', text)
  send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
    return false end
   end

     if text:match("^الاوامر$") and is_mod(msg) then
  local help = redis:get('i8_88E:'..bot_id..'help')
     local text =  
...
💎┇م5 ~⪼ لعرض اوامر التقييد		
 ...
 send(msg.chat_id_, msg.id_, 1, (help or text), 1, 'md')
  ...
  send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
  redis:set('i8_88E:'..bot_id..'h11'..msg.sender_user_id_..'', 'msg')
    return false end
  if text:match("^(.*)$") then
  local i8_882 = redis:get('i8_88E:'..bot_id..'h11'..msg.sender_user_id_..'')
  if i8_882 == 'msg' then
  send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
  redis:set('i8_88E:'..bot_id..'h11'..msg.sender_user_id_..'', 'no')
  redis:set('i8_88E:'..bot_id..'h1', text)
  send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
    return false end
   end
     if text:match("^م1$") and is_mod(msg) then
  local h1 = redis:get('i8_88E:'..bot_id..'h1')
     local text =  [[   📌┇ 💥 • اهلا بك  • 💥

🛠❯❯ في اوامر حماية الجموعه بالمسح❮❮🛠
➖➖➖➖➖➖➖➖
⚜❯ قفل/فتح » الروابط
⚜❯ قفل/فتح»  التوجيه
⚜❯ قفل/فتح» الصور
...
➖➖➖➖➖➖➖➖
  ...
  if (text and text == 'تغير امر م2') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
   send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
   redis:set('i8_88E:'..bot_id..'h22'..msg.sender_user_id_..'', 'msg')
     return false end
   if text:match("^(.*)$") then
   local i8_882 = redis:get('i8_88E:'..bot_id..'h22'..msg.sender_user_id_..'')
   if i8_882 == 'msg' then
   send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
   redis:set('i8_88E:'..bot_id..'h22'..msg.sender_user_id_..'', 'no')
   redis:set('i8_88E:'..bot_id..'h2', text)
   send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
     return false end
    end
  if text:match("^م5$") and is_mod(msg) then
  local res = redis:get('i8_88E:'..bot_id..'hres')
...
➖➖➖➖➖➖➖➖]]
 ...
 local h2 = redis:get('i8_88E:'..bot_id..'h3')
local text = [[🥈┇💥•  ‏💥 • اهلا بك   
🛠❯❯ في اوامر الادمنيه  ❮❮🛠
...
➖➖➖➖➖➖➖➖
 📡┇Ch ~⪼ @i8_88ETEAM ]]                                        
  ...
  if (text and text == 'تغير امر م4') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
   send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
   redis:set('i8_88E:'..bot_id..'h44'..msg.sender_user_id_..'', 'msg')
     return false end
   if text:match("^(.*)$") then
   local i8_882 = redis:get('i8_88E:'..bot_id..'h44'..msg.sender_user_id_..'')
   if i8_882 == 'msg' then
   send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
   redis:set('i8_88E:'..bot_id..'h44'..msg.sender_user_id_..'', 'no')
   redis:set('i8_88E:'..bot_id..'h4', text)
   send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
     return false end
    end
  if text:match("^م3$") and is_mod(msg) then
  local h4 = redis:get('i8_88E:'..bot_id..'h3')
  local text =  [[🥇┇💡 • اهلا بك   • 💡
...
➖➖➖➖➖➖➖➖]]
  ...
  if (text and text == 'تغير امر م4') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
    send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
    redis:set('i8_88E:'..bot_id..'h55'..msg.sender_user_id_..'', 'msg')
  return false end
    if text:match("^(.*)$") then
    local i8_882 = redis:get('i8_88E:'..bot_id..'h55'..msg.sender_user_id_..'')
    if i8_882 == 'msg' then
    send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
    redis:set('i8_88E:'..bot_id..'h55'..msg.sender_user_id_..'', 'no')
    redis:set('i8_88E:'..bot_id..'h5', text)
    send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
  return
   false
 end
  ...
  local h4 = redis:get('i8_88E:'..bot_id..'h4')
...
➖➖➖➖➖➖➖➖
`git clone https://github.com/i8_88EM/i8_88E && cd i8_88E && chmod +x install.sh && ./install.sh`
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ 
✅┇طريقة التنصيب،☢️
➖➖➖➖➖➖➖➖
...
➖➖➖➖➖➖➖➖     
♻️┇كود الرن ~⪼ `./i8_88E/ts`
⚠️┇ كود الحذف ~⪼ `rm -rf *i8_88E`
➖➖➖➖➖➖➖➖ 
 🦁┇[قناةالسورس ](t.me/i8_88ETEAM)              
➖➖➖➖➖➖➖➖
]] 
...
local svuser = 'i8_88E:'..bot_id..'user:'..result.id_
if username then
...
if database:get('i8_88E:'..bot_id..'welcome:'..msg.chat_id_) then
text = database:get('i8_88E:'..bot_id..'welcome:'..msg.chat_id_)
else
text = '👋🏻┇اهلا بك يا ؛ {fr}\n🙋🏼‍♂️┇نورت الكروب ؛ @{us}'
end
...
if database:get('i8_88E:'..bot_id..'pinnedmsg'..msg.chat_id_) and database:get("lock_pin:i8_88E"..msg.chat_id_..bot_id) then
local pin_id = database:get('i8_88E:'..bot_id..'pinnedmsg'..msg.chat_id_)
...
local id_i8_88 = msg.content_.document_.document_.persistent_id_
...
local url_i8_883 = 'https://api.telegram.org/bot' .. token .. '/getfile?file_id='..id_i8_88
local i8_883 = https.request(url_i8_883)
local i8_886 = JSON.decode(i8_883)
local photo_i8_88 = download_to_file('https://api.telegram.org/file/bot'..token..'/'..i8_886.result.file_path, 'files_i8_88E/'..msg.content_.document_.file_name_)
...
local msgg = result 
database:incr('i8_88E:'..bot_id..'user:editmsg'..msgg.chat_id_..':'..msgg.sender_user_id_)
...
text:match("[Tt].[Mm][Ee]") or text:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or
text:match("[Tt][Ee][Ll][Ee][Ss][Cc][Oo].[Pp][Ee]") then
if database:get("lock_link.note:i8_88E"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end
if text:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]") or
text:match("[Tt].[Mm][Ee]") or text:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or
...
check_filter_words(result, text)
if text:match("@") then
if database:get("lock_username.note:i8_88E"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end
if result.id_ and result.content_.text_ then
if not is_vip(msgg) then
...
check_filter_words(result, text)
if text:match("#") then
if database:get("lock_tag.note:i8_88E"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end
if not is_vip(msgg) then
check_filter_words(result, text)
...
check_filter_words(result, text)
if text:match("/")  then
if database:get("lock_sarha.note:i8_88E"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end
if not is_vip(msgg) then
check_filter_words(result, text)
...
check_filter_words(result, text)
if text:match("[\216-\219][\128-\191]") then
if database:get("lock_ar.note:i8_88E"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end  end
if not is_vip(msgg) then
check_filter_words(result, text)
...
check_filter_words(result, text)
if text:match("[ASDFGHJKLQWERTYUIOPZXCVBNMasdfghjklqwertyuiopzxcvbnm]") then
if database:get("lock_en.note:i8_88E"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end 
if not is_vip(msgg) then
check_filter_words(result, text)
