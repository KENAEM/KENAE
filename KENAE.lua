...
tsX000("prore",msg,"☑┊بالفعل تم رفعه عضو مميز") 
 ...
 chat_kick(result.chat_id_, result.sender_user_id_)
  ...
  send(msg.chat_id_, msg.id_, 1, '❕┊لا تستطيع حظر \n🔘┊(مدراء،ادمنيه،اعضاء مميزين)البوت', 1, 'md')
   ...
   local hash =  'KENAE:'..bot_id..'banned:'..msg.chat_id_
   if not database:sismember(hash, result.sender_user_id_) then
    tsX000("prore",msg,"☑┊بالفعل تم فك الحظره من البوت")
   else
     database:srem(hash, result.sender_user_id_)
       tsX000("prore",msg,"☑┊تم فك الحظره من البوت")
   end
  end
  ...
  texts = '👤┊العضو ~⪼ ['..result.title_..'](t.me/'..(apba[2] or 'KENAEteam')..')\n☑┊تم فك الحظره من البوت' 
   ...
   local apba = {string.match(text, "^(فك الحظر) (%d+)$")}
     database:srem('KENAE:'..bot_id..'banned:'..msg.chat_id_, apba[2])
      tsX000(apba[2],msg,"☑┊تم فك الحظره من البوت") 
    end
  ...
  function ungban_by_reply(extra, result, success)
    local hash =  'KENAE:'..bot_id..'gbanned:'
     tsX000("prore",msg,"🚫┊تم فك الحظره من المجموعات البوت")
    database:srem(hash, result.sender_user_id_)
  end
  ...
  local hash =  'KENAE:'..bot_id..'gbanned:'
    database:srem(hash, apbll[2])
	  tsX000(apbll[2],msg,"🚫┊تم فك الحظره من مجموعات البوت")
  end
...
if text == 'الاسرع' and database:get("gamee:KENAE"..msg.chat_id_..bot_id) then
...
if text == ''..(database:get('KENAE:'..bot_id..'klmo'..msg.chat_id_) or 'لفاتع')..'' and not database:get('tshake:'..bot_id..'l:id'..msg.chat_id_) then
...
if text == 'سمايلات' and database:get("gamee:KENAE"..msg.chat_id_..bot_id) then
...
if text == ''..(database:get('KENAE:'..bot_id..'klmos'..msg.chat_id_) or 'لفاتع')..'' and not database:get('tshake:'..bot_id..'l:ids'..msg.chat_id_) then
...
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md') 
else taha = '*💎¦ عدد نقودك الحاليه ('..(database:get('KENAE:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_))..')*' 
taha1 = '*💎¦ مجموع نقودك ('..(database:get('KENAE:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_))..')*' 
send(msg.chat_id_, msg.id_, 1,''..taha..'\n'..taha1..'', 1, 'md') 
end 
end

if text:match('^رسائل النقود (%d+)$') and is_creator(msg) then 
local kara = {string.match(text, "^(رسائل النقود) (%d+)$")} 
...
database:set('KENAE:'..bot_id..'gamepoint' .. msg.chat_id_, kara[2]) 
...
taha = tonumber((database:get('KENAE:'..bot_id..'gamepoint' .. msg.chat_id_) or 0)) 
send(msg.chat_id_, msg.id_, 1,'💎¦ تم بيع جواهرك كل نقد يساوي '..taha..' رساله', 'md') 
end 
end
