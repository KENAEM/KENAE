...
chats = {}
day = 86400
bot_ikali = {string.match(token, "^(%d+)(:)(.*)")}
bot_id = tonumber(bot_ikali[1])
sudo_users = {sudo_add,bot_id}
URL33 = require('socket.url')
---------- {Show Files} -----------
red = '\27[31m' reset = '\27[m' Blue = "\27[34m" Green = "\27[32m"
local files_kaliE = database:smembers("files"..bot_id) 
print(Green.."\nFiles Now Started : \n "..reset)
for i,v in pairs(files_kaliE) do
print(Blue..i..red..' - \27[10;33m'..v..',\27[m')  end
print(Green.."\nThes all Files.\n\n\n"..reset)
io.popen("mkdir files_kaliE")
...
local kali_add_sudo = redis:get('kaliE:'..bot_id..'sudoo'..msg.sender_user_id_..'')
...
local var = false 
local admin = database:sismember('kaliE:'..bot_id..'admins:', user_id)
if admin then var = true end
for k,v in pairs(sudo_users) do
if user_id == v then var = true end
end
local kali_add_sudo = redis:get('kaliE:'..bot_id..'sudoo'..user_id..'')
...
local var = false
local creator = database:sismember('kaliE:'..bot_id..'creator:'..chat_id, user_id) 
local admin = database:sismember('kaliE:'..bot_id..'admins:', user_id)
...
chat_id = msg.chat_id_
local var = false
local mod = database:sismember('kaliE:'..bot_id..'mods:'..chat_id, user_id)  
local admin = database:sismember('kaliE:'..bot_id..'admins:', user_id)  
local owner = database:sismember('kaliE:'..bot_id..'owners:'..chat_id, user_id)
local creator = database:sismember('kaliE:'..bot_id..'creator:'..chat_id, user_id)  
local vip = database:sismember('kaliE:'..bot_id..'vipgp:'..chat_id, user_id)
...
local var = false
local banned = database:sismember('kaliE:'..bot_id..'banned:'..chat_id, user_id)
...
local var = false
local banned = database:sismember('kaliE:'..bot_id..'gbanned:', user_id)
if banned then var = true end
return var
end
--         »»                 is_muted                         ««              --
...
local var = false
local banned = database:sismember('kaliE:'..bot_id..'muted:'..chat_id, user_id)
if banned then var = true end
return var
end
--         »»                 is_gmuted                         ««              --
...
local var = false 
local banned = database:sismember('kaliE:'..bot_id..'gmuted:', user_id)
if banned then var = true end
return var
end
--         »»                 getMessage                         ««              --
...
local hash =  'kaliE:'..bot_id..'filters:'..msg.chat_id_
if hash then
...
local function send(chat_id, reply_to_message_id, disable_notification, text, disable_web_page_preview, parse_mode)
  ...
  {
  {text = ''..(channel_ts or "kaliE")..'', url=''..(channel_user_ts or 't.me/kaliE')..''},
  },
  }
  local kali = "https://api.telegram.org/bot" ..token.. '/sendMessage?chat_id=' .. chat_id
  if reply_to_message_id ~= 0 then
  kali = kali .. '&reply_to_message_id=' .. reply_to_message_id/2097152/0.5 -- جميع الحقوق محفوظه لفريق ;كيناي لايمكنك نسخ او استخدام هذه السطر بدون موافقه الفريق
  end
...
info = '💬┇بواسطه ~⪼ ['..result.first_name_..'](t.me/'..(result.username_ or 'kaliEteam')..')\n'..text
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
 end
getUser(msg.sender_user_id_, kali333)
end
if value == "prore" then
function get_kaliEX(kaliEx1,kaliEx2,kaliEx3)
local id_kaliEx = kaliEx2.sender_user_id_
function kali333(extra,result,success)
info = '👤┇العضو ~⪼ ['..result.first_name_..'](t.me/'..(result.username_ or 'kaliEteam')..')\n'..text
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
 end
getUser(id_kaliEx, kali333)
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,get_kaliEX)
end
if value ~= "prore" and value~= "lock"  then
function kali333(extra,result,success)
info = '👤┇العضو ~⪼ ['..(result.first_name_ or value)..'](t.me/'..(result.username_ or 'kaliEteam')..')\n'..text
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
...
if not database:get( 'kaliE:'..bot_id.."charge:"..msg.chat_id_) then
function kaliE_info(k1,k2)
send(msg.chat_id_, msg.id_, 1, "❕┇المجموعه {"..(k2.title_ or "").."} معطله سابقا", 1, 'md')
end
openChat(msg.chat_id_,kaliE_info)
end
if database:get( 'kaliE:'..bot_id.."charge:"..msg.chat_id_) then
database:del( 'kaliE:'..bot_id.."charge:"..msg.chat_id_)
function kaliE_info(k1,k2)
send(msg.chat_id_, msg.id_, 1, info.."☑️┇تم تعطيل المجموعه {"..k2.title_.."}", 1, 'md')
end
openChat(msg.chat_id_,kaliE_info)
database:srem("kaliE:gog"..bot_id, msg.chat_id_)
database:del( 'kaliE:'..bot_id.."charge:"..msg.chat_id_)
function kaliE_info2(k1,k2)
local getlink = 'https://api.telegram.org/bot'..token..'/exportChatInviteLink?chat_id='..msg.chat_id_
local req = https.request(getlink)
local link = json:decode(req)
if link.ok == true then 
database:set('kaliE:'..bot_id.."group:link"..msg.chat_id_ ,link.result) 
link = link.result 
else 
link = "(لا يوجد)"
end
send(tostring((database:get("kaliE"..bot_id..":sudo:gr") or sudo_add)), 0, 1, "🔘┇قام بتعطيل مجموعه \n🎫┇ايدي المطور ~⪼ ("..msg.sender_user_id_..")\n📜┇معرف المطور ~⪼ @"..(result.username_ or "لا يوجد").."\n🌐┇معلومات المجموعه \n\n🎫┇ايدي المجموعه ~⪼ ("..msg.chat_id_..")\nⓂ️┇اسم المجموعه ~⪼ ("..k2.title_..")\n📎┇رابط المجموعه ~⪼ ["..link.."]" , 1, 'html')
end
openChat(msg.chat_id_,kaliE_info2) 
--
end
else
...
if database:get("lock_link.note:kaliE"..msg.chat_id_..bot_id) then
...
database:sadd('kaliE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
end
end
...
if database:get("lock_username.note:kaliE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
  database:sadd('kaliE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('kaliE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_chat.note:kaliE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
  database:sadd('kaliE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('kaliE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if text and text:match("(.*)(/)(.*)")  then
if database:get("lock_sarha.note:kaliE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
  database:sadd('kaliE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('kaliE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if text and text:match("(.*)(#)(.*)")  then
if database:get("lock_tag.note:kaliE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
  database:sadd('kaliE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('kaliE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_fwd.note:kaliE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
  database:sadd('kaliE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('kaliE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_stecker.note:kaliE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
  database:sadd('kaliE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('kaliE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_note.note:kaliE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
  database:sadd('kaliE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('kaliE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_photo.note:kaliE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
  database:sadd('kaliE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('kaliE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_audeo.note:kaliE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
  database:sadd('kaliE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false")
database:sadd('kaliE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_voice.note:kaliE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
  database:sadd('kaliE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('kaliE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_video.note:kaliE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
  database:sadd('kaliE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false")
database:sadd('kaliE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_gif.note:kaliE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
  database:sadd('kaliE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('kaliE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_contect.note:kaliE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
  database:sadd('kaliE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false")
database:sadd('kaliE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_ar.note:kaliE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
  database:sadd('kaliE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('kaliE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_files.note:kaliE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
  database:sadd('kaliE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('kaliE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_en.note:kaliE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
  database:sadd('kaliE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('kaliE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_mark.note:kaliE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('kaliE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
end
...
if not database:get( 'kaliE:'..bot_id.."charge:"..msg.chat_id_) then
database:set( 'kaliE:'..bot_id.."charge:"..msg.chat_id_,true)
function kaliE_info(k1,k2)
send(msg.chat_id_, msg.id_, 1, info.."☑️┇تم تفعيل المجموعه {"..(k2.title_ or "").."}", 1, 'md')
end
openChat(msg.chat_id_,kaliE_info)
database:sadd("kaliE:gog"..bot_id, msg.chat_id_)
function kaliE_info2(k1,k2)
local getlink = 'https://api.telegram.org/bot'..token..'/exportChatInviteLink?chat_id='..msg.chat_id_
local req = https.request(getlink)
local link = json:decode(req)
...
send(tostring((database:get("kaliE"..bot_id..":sudo:gr") or sudo_add)), 0, 1, "🔘┇قام بتفعيل مجموعه \n🎫┇ايدي المطور ~⪼ ("..msg.sender_user_id_..")\n📜┇معرف المطور ~⪼ @"..(result.username_ or "لا يوجد").."\n🌐┇معلومات المجموعه \n\n🎫┇ايدي المجموعه ~⪼ ("..msg.chat_id_..")\nⓂ️┇اسم المجموعه ~⪼ ("..k2.title_..")\n📎┇رابط المجموعه ~⪼ ["..link.."]" , 1, 'html')
end
openChat(msg.chat_id_, kaliE_info2) 
--
if data.result.can_promote_members  then
database:sadd('kaliE:'..bot_id..'owners:'..msg.chat_id_,msg.sender_user_id_)
end
database:set( 'kaliE:'..bot_id.."enable:"..msg.chat_id_,true)
if data.result.status == 'creator' then
database:sadd('kaliE:'..bot_id..'creator:'..msg.chat_id_, msg.sender_user_id_)
end
end
else
...
if (text == "قفل التاك") then
  local tsX_o = database:get("lock_tag:kaliE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  التاك")
  else
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  التاك")
  database:set("lock_tag:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الشارحه") then
  local tsX_o = database:get("lock_sarha:kaliE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  الشارحه")
  else
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  الشارحه")
  database:set("lock_sarha:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل التعديل") then
  local tsX_o = database:get("lock_edit:kaliE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  التعديل")
  else
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  التعديل")
  database:set("lock_edit:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل التكرار") then
  local tsX_o = database:get("lock_lllll:kaliE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  التكرار")
  else
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  التكرار")
  database:set("lock_lllll:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل المتحركه") then
  local tsX_o = database:get("lock_gif:kaliE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  المتحركه")
  else
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  المتحركه")
  database:set("lock_gif:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الملفات") then
  local tsX_o = database:get("lock_files:kaliE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  الملفات")
  else
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  الملفات")
  database:set("lock_files:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الماركدون") then
  local tsX_o = database:get("lock_mark:kaliE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  الماركدون")
  else
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  الماركدون")
  database:set("lock_mark:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الصور") then
  local tsX_o = database:get("lock_photo:kaliE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  الصور")
  else
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  الصور")
  database:set("lock_photo:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الملصقات") then
  local tsX_o = database:get("lock_stecker:kaliE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  الملصقات")
  else
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  الملصقات")
  database:set("lock_stecker:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الفيديو") then
  local tsX_o = database:get("lock_video:kaliE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  الفيديو")
  else
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  الفيديو")
  database:set("lock_video:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الانلاين") then
  local tsX_o = database:get("lock_inline:kaliE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  الانلاين")
  else
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  الانلاين")
  database:set("lock_inline:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الدردشه") then
  local tsX_o = database:get("lock_chat:kaliE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  الدردشه")
  else
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  الدردشه")
  database:set("lock_chat:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل التوجيه") then
  local tsX_o = database:get("lock_fwd:kaliE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  التوجيه")
  else
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  التوجيه")
  database:set("lock_fwd:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل التثبيت") then
  local tsX_o = database:get("lock_pin:kaliE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  التثبيت")
  else
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  التثبيت")
  database:set("lock_pin:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الاغاني") then
  local tsX_o = database:get("lock_audeo:kaliE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  الاغاني")
  else
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  الاغاني")
  database:set("lock_audeo:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الصوت") then
  local tsX_o = database:get("lock_voice:kaliE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  الصوت")
  else
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  الصوت")
  database:set("lock_voice:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الجهات") then
  local tsX_o = database:get("lock_contact:kaliE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  الجهات")
  else
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  الجهات")
  database:set("lock_contact:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل العربيه") then
  local tsX_o = database:get("lock_ar:kaliE"..msg.chat_id_..bot_id)
  if tsX_o then
    tsX000("lock",msg,"☑┇بالتأكيد تم قفل  العربيه")
  else
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  العربيه")
  database:set("lock_ar:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الانكليزيه") then
  local tsX_o = database:get("lock_en:kaliE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  الانكليزيه")
  else
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  الانكليزيه")
  database:set("lock_en:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الميديا") then
  local tsX_o = database:get("lock_media:kaliE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  الميديا")
  else
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  الميديا")
  database:set("lock_media:kaliE"..msg.chat_id_..bot_id,"ok")
database:set("lock_audeo:kaliE"..msg.chat_id_..bot_id,"ok")
database:set("lock_video:kaliE"..msg.chat_id_..bot_id,"ok")
database:set("lock_photo:kaliE"..msg.chat_id_..bot_id,"ok")
database:set("lock_stecker:kaliE"..msg.chat_id_..bot_id,"ok")
database:set("lock_voice:kaliE"..msg.chat_id_..bot_id,"ok")
database:set("lock_gif:kaliE"..msg.chat_id_..bot_id,"ok")
database:set("lock_note:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الروابط") then
  local tsX_o = database:get("lock_link:kaliE"..msg.chat_id_..bot_id)
  if tsX_o then
    tsX000("lock",msg,"☑┇بالتأكيد تم قفل  الروابط")
  else
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  الروابط")
  database:set("lock_link:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل المعرف") then
  local tsX_o = database:get("lock_username:kaliE"..msg.chat_id_..bot_id)
  if tsX_o then
   tsX000("lock",msg,"☑┇بالتأكيد تم قفل  المعرف")
  else
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  المعرف")
  database:set("lock_username:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الاشعارات") then
  local tsX_o = database:get("lock_new:kaliE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  الاشعارات")
  else
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  الاشعارات")
  database:set("lock_new:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل البوتات بالطرد") then
  local tsX_o = database:get("lock_botAndBan:kaliE"..msg.chat_id_..bot_id)
  if tsX_o then
   tsX000("lock",msg,"☑┇بالتأكيد تم قفل  البوتات بالطرد")
  else
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  البوتات بالطرد")
  database:set("lock_botAndBan:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل البوتات") then
  local tsX_o = database:get("lock_bot:kaliE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  البوتات")
  else
  tsX000("lock",msg,"☑┇بالتأكيد تم قفل  البوتات")
  database:set("lock_bot:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل بصمه الفيديو") then
  local tsX_o = database:get("lock_note:kaliE"..msg.chat_id_..bot_id)
  if tsX_o then
  ...
  database:del("lock_new:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_tag:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_sarha:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_lllll:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_edit:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_gif:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_files:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_mark:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_photo:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_stecker:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_video:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_inline:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_chat:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_fwd:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_pin:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_audeo:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_voice:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_botAndBan:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_contact:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_ar:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_en:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_media:kaliE"..msg.chat_id_..bot_id,"ok")
database:del("lock_audeo:kaliE"..msg.chat_id_..bot_id,"ok")
database:del("lock_video:kaliE"..msg.chat_id_..bot_id,"ok")
database:del("lock_photo:kaliE"..msg.chat_id_..bot_id,"ok")
database:del("lock_stecker:kaliE"..msg.chat_id_..bot_id,"ok")
database:del("lock_voice:kaliE"..msg.chat_id_..bot_id,"ok")
database:del("lock_gif:kaliE"..msg.chat_id_..bot_id,"ok")
database:del("lock_note:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_username:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_link:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_bot:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end  
if is_mod(msg) then
if (text == "قفل التاك بالتقيد") then
  local tsX_o = database:get("lock_tag.note:kaliE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ التاك بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل  ⌁ التاك بالتقيد ✓")
  database:set("lock_tag.note:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الشارحه بالتقيد") then
  local tsX_o = database:get("lock_sarha.note:kaliE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ الشارحه بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل  ⌁ الشارحه بالتقيد ✓")
  database:set("lock_sarha.note:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل المتحركه بالتقيد") then
  local tsX_o = database:get("lock_gif.note:kaliE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ المتحركه بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل  ⌁ المتحركه بالتقيد ✓")
  database:set("lock_gif.note:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الملفات بالتقيد") then
  local tsX_o = database:get("lock_files.note:kaliE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ الملفات بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل  ⌁ الملفات بالتقيد ✓")
  database:set("lock_files.note:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الماركدون بالتقيد") then
  local tsX_o = database:get("lock_mark.note:kaliE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ الماركدون بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل  ⌁ الماركدون بالتقيد ✓")
  database:set("lock_mark.note:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الصور بالتقيد") then
  local tsX_o = database:get("lock_photo.note:kaliE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ الصور بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل  ⌁ الصور بالتقيد ✓")
  database:set("lock_photo.note:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الملصقات بالتقيد") then
  local tsX_o = database:get("lock_stecker.note:kaliE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ الملصقات بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل  ⌁ الملصقات بالتقيد ✓")
  database:set("lock_stecker.note:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الفيديو بالتقيد") then
  local tsX_o = database:get("lock_video.note:kaliE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ الفيديو بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل  ⌁ الفيديو بالتقيد ✓")
  database:set("lock_video.note:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الانلاين بالتقيد") then
  local tsX_o = database:get("lock_inline.note:kaliE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ الانلاين بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل  ⌁ الانلاين بالتقيد ✓")
  database:set("lock_inline.note:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الدردشه بالتقيد") then
  local tsX_o = database:get("lock_chat.note:kaliE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ الدردشه بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل  ⌁ الدردشه بالتقيد ✓")
  database:set("lock_chat.note:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل التوجيه بالتقيد") then
  local tsX_o = database:get("lock_fwd.note:kaliE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ التوجيه بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل  ⌁ التوجيه بالتقيد ✓")
  database:set("lock_fwd.note:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الاغاني بالتقيد") then
  local tsX_o = database:get("lock_audeo.note:kaliE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ الاغاني بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل  ⌁ الاغاني بالتقيد ✓")
  database:set("lock_audeo.note:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الصوت بالتقيد") then
  local tsX_o = database:get("lock_voice.note:kaliE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ الصوت بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل  ⌁ الصوت بالتقيد ✓")
  database:set("lock_voice.note:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الجهات بالتقيد") then
  local tsX_o = database:get("lock_contact.note:kaliE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ الجهات بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل  ⌁ الجهات بالتقيد ✓")
  database:set("lock_contact.note:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل العربيه بالتقيد") then
  local tsX_o = database:get("lock_ar.note:kaliE"..msg.chat_id_..bot_id)
  if tsX_o then
    tsX000("lock",msg,"👁‍🗨 ⫶ العربيه بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل  ⌁ العربيه بالتقيد ✓")
  database:set("lock_ar.note:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الانكليزيه بالتقيد") then
  local tsX_o = database:get("lock_en.note:kaliE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ الانكليزيه بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل  ⌁ الانكليزيه بالتقيد ✓")
  database:set("lock_en.note:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الروابط بالتقيد") then
  local tsX_o = database:get("lock_link.note:kaliE"..msg.chat_id_..bot_id)
  if tsX_o then
    tsX000("lock",msg,"👁‍🗨 ⫶ الروابط بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل  ⌁ الروابط بالتقيد ✓")
  database:set("lock_link.note:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل المعرف بالتقيد") then
  local tsX_o = database:get("lock_username.note:kaliE"..msg.chat_id_..bot_id)
  if tsX_o then
   tsX000("lock",msg,"👁‍🗨 ⫶ المعرف بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل  ⌁ المعرف بالتقيد ✓")
  database:set("lock_username.note:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل بصمه الفيديو بالتقيد") then
  local tsX_o = database:get("lock_note.note:kaliE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ بصمه الفيديو بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل  ⌁ بصمه الفيديو بالتقيد ✓")
  database:set("lock_note.note:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
end
if is_mod(msg) then
  ...
  database:del("lock_tag.note:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_sarha.note:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_gif.note:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_files.note:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_mark.note:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_photo.note:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_stecker.note:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_video.note:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_inline.note:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_chat.note:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_fwd.note:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_audeo.note:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_voice.note:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_contact.note:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_ar.note:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_en.note:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_username.note:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_note.note:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_link.note:kaliE"..msg.chat_id_..bot_id,"ok")
  end
  end
  end 
--         »»               End kaliE unlock                       ««              --
...
local text = msg.content_.text_
if text and (text:match("[Hh][Tt][Tt][Pp][Ss]://") or text:match("[Hh][Tt][Tt][Pp]://") or text:match(".[Ii][Rr]") or text:match(".[Cc][Oo][Mm]") or text:match(".[Oo][Rr][Gg]") or text:match(".[Ii][Nn][Ff][Oo]") or text:match("[Ww][Ww][Ww].") or text:match(".[Tt][Kk]")) then
if database:get("lock_link:kaliE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.caption_ then
...
text = msg.content_.caption_
if text and text:match("(.*)(@)(.*)")  then
if database:get("lock_username:kaliE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
end
...
if database:get("lock_chat:kaliE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "kaliE"
end
if text and text:match("(.*)(/)(.*)")  then
if database:get("lock_sarha:kaliE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if text and text:match("(.*)(#)(.*)")  then
if database:get("lock_tag:kaliE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.ID == "MessageChatAddMembers" then
if msg.content_.members_[0].type_.ID == 'UserTypeBot' then
if database:get("lock_bot:kaliE"..msg.chat_id_..bot_id) then
changeChatMemberStatus(msg.chat_id_, msg.content_.members_[0].id_, "Kicked")
end
if database:get("lock_botAndBan:kaliE"..msg.chat_id_..bot_id) then
changeChatMemberStatus(msg.chat_id_, msg.content_.members_[0].id_, "Kicked")
...
if msg.forward_info_ then
if msg.forward_info_.ID == "MessageForwardedFromUser" or msg.forward_info_.ID == "MessageForwardedPost" then
if database:get("lock_fwd:kaliE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
end
if msg.content_.ID == "MessageSticker" then
if database:get("lock_stecker:kaliE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.ID == "MessageChatJoinByLink" or msg.content_.ID == "MessageChatAddMembers" then
if database:get("lock_new:kaliE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end

if msg.content_.ID == "MessageChatAddMembers" then
database:incr('kaliE:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)
end
if msg.content_.ID == "MessageUnsupported" then
if database:get("lock_note:kaliE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.ID == "MessagePhoto" then
if database:get("lock_photo:kaliE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.ID == "MessageAudio" then
if database:get("lock_audeo:kaliE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.ID == "MessageVoice" then
if database:get("lock_voice:kaliE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.ID == "MessageVideo" then
if database:get("lock_video:kaliE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.ID == "MessageAnimation" then
if database:get("lock_gif:kaliE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.ID == "MessageContact" then
if database:get("lock_contect:kaliE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if text and text:match("[\216-\219][\128-\191]") then
if database:get("lock_ar:kaliE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.ID == "MessageDocument" then
if database:get("lock_files:kaliE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if text and text:match("[ASDFGHJKLQWERTYUIOPZXCVBNMasdfghjklqwertyuiopzxcvbnm]") then
if database:get("lock_en:kaliE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
...
if msg.content_.entities_[0] then
if msg.content_.entities_[0] and msg.content_.entities_[0].ID == "MessageEntityUrl" or msg.content_.entities_[0].ID == "MessageEntityTextUrl" then
if database:get("lock_mark:kaliE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
end
...
if database:get("lock_lllll:kaliE"..msg.chat_id_..bot_id) then
...
local hash = 'kaliE:'..bot_id..'flood:time:'..msg.chat_id_
if not database:get(hash) then
floodTime = 1
else
...
database:sadd('kaliE:'..bot_id..'muted:'..msg.chat_id_, msg.sender_user_id_)
end
end
...
if not database:get('kaliE:'..bot_id..'leave:groups') then
...
local hash =  'kaliE:'..bot_id..'creator:'..msg.chat_id_
if database:sismember(hash, result.sender_user_id_) then
tsX000("prore",msg,"☑┇بالتأكيد بالتأكيد تم رضع منشئ في البوت")
else
database:sadd(hash, result.sender_user_id_)
tsX000("prore",msg,"☑┇بالتأكيد تم رضع منشئ في البوت")
end
end
...
if result.id_ then
database:sadd('kaliE:'..bot_id..'creator:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apow[2] or 'kaliEteam')..')\n☑┇بالتأكيد تم رضع منشئ في البوت'
else
texts = '✖┇خطاء'
end
...
database:sadd('kaliE:'..bot_id..'creator:'..msg.chat_id_, apow[2])
  tsX000(apow[2],msg,"☑┇بالتأكيد تم رضع منشئ في البوت")
end
...
local user_info = database:hgetall('kaliE:'..bot_id..'user:'..v)
...
if not database:get('kaliE:'..bot_id..'bc:groups') then
local gps = database:scard( 'kaliE:'..bot_id.."groups") or 0
local gpss = database:smembers( 'kaliE:'..bot_id.."groups") or 0
for i=1, #gpss do
if not database:sismember('kaliE:'..bot_id..'pro:groups', gpss[i]) then
send(gpss[i], 0, 1, ssss[2], 1, 'html')		
end					
end
local pro = database:scard('kaliE:'..bot_id..'pro:groups') or 0
...
local gps = database:scard('kaliE:'..bot_id.."groups") or 0
local user = database:scard('kaliE:'..bot_id.."userss") or 0
local gps2 = database:scard("kaliE:gog"..bot_id) or 0
local gps3 = database:scard('kaliE:'..bot_id..'pro:groups') or 0
...
local gps = database:smembers('kaliE:'..bot_id.."groups") or 0
local gps2 = database:smembers("kaliE:gog"..bot_id) or 0
for i=1,#gps do
database:sadd("kaliE:gog"..bot_id, gps[i])
database:set('kaliE:'..bot_id.."enable:"..gps[i],true)
database:set( 'kaliE:'..bot_id.."charge:"..gps[i],true)
end
...
local gps2 = database:smembers("kaliE:gog"..bot_id) or 0
for i=1,#gps do
database:del("kaliE:gog"..bot_id)
database:del('kaliE:'..bot_id.."enable:"..gps[i])
database:del( 'kaliE:'..bot_id.."charge:"..gps[i])
end
...
database:del("kaliE:gog"..bot_id)
database:del('kaliE:'..bot_id.."groups")
end
end --
if text:match("^روابط الكروبات$") then
local gpss = database:smembers("kaliE:"..bot_id.."groups") or 0
local gps = database:scard('kaliE:'..bot_id.."groups")
text = '📊┇روابط الكروبات\n\n'
for i=1, #gpss do
local link = database:get('kaliE:'..bot_id.."group:link"..gpss[i])
text = text.."|"..i.."| ~⪼ "..gpss[i].."\n ~⪼ "..(link or  "لا يوجد رابط").."\n"
end
  local f = io.open('kaliE.txt', 'w')
 f:write(text)
 f:close()
 local kaliEe = 'https://api.telegram.org/bot' .. token .. '/sendDocument'
 local curl = 'curl "' .. kaliEe .. '" -F "chat_id=' .. msg.chat_id_ .. '" -F "document=@' .. 'kaliE.txt' .. '"'
 io.popen(curl)
 end
if text:match("^تحديث$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
send(msg.chat_id_, msg.id_, 1, '☑┇تم التحديث', 1, 'md')
os.execute('rm -rf ./libs/utils.lua')
os.execute('cd libs && wget https://raw.githubusercontent.com/kaliEM/kaliE/master/libs/utils.lua')
os.execute('rm -rf kaliE.lua')
os.execute('wget https://raw.githubusercontent.com/kaliEM/kaliE/master/kaliE.lua')
os.exit()
return false
end
...
local time = a[2] * day
database:setex( 'kaliE:'..bot_id.."charge:"..msg.chat_id_,time,true)
database:set( 'kaliE:'..bot_id.."enable:"..msg.chat_id_,true)
...
local ex = database:ttl( 'kaliE:'..bot_id.."charge:"..txt[2])
if ex == -1 then
...
database:del("kaliE:gog"..bot_id,txt[2])
...
database:setex( 'kaliE:'..bot_id.."charge:"..txt[2],timeplan1,true)
...
database:set( 'kaliE:'..bot_id.."enable:"..txt[2],true)
end
getUser(msg.sender_user_id_, kali333)
end
...
database:setex( 'kaliE:'..bot_id.."charge:"..txt[2],timeplan2,true)
...
function kali333(extra,result,success)
kali_info = '@'..(result.username_ or 'لا يوجد')..''
database:set( 'kaliE:'..bot_id.."charge:"..txt[2],true)
...
local files_kaliE = database:smembers("files"..bot_id)
local kali = io.popen('cd files_kaliE && ls'):read("*all")
local files_kaliE2 = ''
for i=1,#files_kaliE do
files_kaliE2 = files_kaliE2..'{'..files_kaliE[i]..'}\n'
end
...
local name_t = {string.match(text, "^(حذف ملف) (.*)(.lua)$")}
io.popen("rm -fr files_kaliE/"..name_t[2]..'.lua')
database:srem("files"..bot_id,name_t[2]..'.lua')
...
local kaliEe = 'https://api.telegram.org/bot' .. token .. '/sendDocument'
local curl = 'curl "' .. kaliEe .. '" -F "chat_id=' .. msg.chat_id_ .. '" -F "document=@' .. 'files_kaliE/'..name_t[2]..'.lua' .. '"'
io.popen(curl)
end
end
end -- end if sudo
...
function kaliE_run_file(data)
local files_kaliE = database:smembers("files"..bot_id)
for i=1,#files_kaliE do
local kaliEee = dofile("files_kaliE/"..files_kaliE[i])
local kt = kaliEee.kali_kaliE(data)
if kt == 'end' then
return false
end
end
...
local photo = database:get('kaliE:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_)
if photo then
local idPh = nil
...
database:del('kaliE:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_)
end end
...
return false end
  ...
  if not database:get( 'kaliE:'..bot_id.."charge:"..msg.chat_id_) then
  if database:get( 'kaliE:'..bot_id.."enable:"..msg.chat_id_) then
  database:del( 'kaliE:'..bot_id.."enable:"..msg.chat_id_)
  for k,v in pairs(sudo_users) do
  end
  end
  end
    ...
    if not database:sismember( 'kaliE:'..bot_id.."groups",msg.chat_id_) then
    database:sadd( 'kaliE:'..bot_id.."groups",msg.chat_id_)
    end
    elseif id:match('^(%d+)') then
    database:sadd('kaliE:'..bot_id.."userss",msg.chat_id_)
    else
    if not database:sismember( 'kaliE:'..bot_id.."groups",msg.chat_id_) then
    database:sadd( 'kaliE:'..bot_id.."groups",msg.chat_id_)
    end
  ...
  database:incr('kaliE:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
  if database:get('kaliE:'..bot_id..'viewget'..msg.sender_user_id_) then
  if not msg.forward_info_ then
  send(msg.chat_id_, msg.id_, 1, '❕┇قم بارسال المنشور من القناة', 1, 'md')
  database:del('kaliE:'..bot_id..'viewget'..msg.sender_user_id_)
  else
  ...
  function res_by_reply(extra, result, success)
    local hash =  'kaliE:'..bot_id..'res:'..msg.chat_id_
...
function res_by_username(extra, result, success)
  ...
  database:sadd('kaliE:'..bot_id..'res:'..msg.chat_id_, result.id_)
  ...
  database:sadd('kaliE:'..bot_id..'res:'..msg.chat_id_, res[2])
 ...
 local hash =  'kaliE:'..bot_id..'res:'..msg.chat_id_
  ...
  local hash =  'kaliE:'..bot_id..'mods:'..msg.chat_id_
  if database:sismember(hash, result.sender_user_id_) then
  ...
  if result.id_ then
  database:sadd('kaliE:'..bot_id..'mods:'..msg.chat_id_, result.id_)
  texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'kaliEteam')..')\n☑┇تم رفعه ادمن'
  else
  texts = '✖┇خطاء'
  end
  send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
  ...
  database:sadd('kaliE:'..bot_id..'mods:'..msg.chat_id_, apmd[2])
  ...
  function promote_by_reply(extra, result, success)
  local hash =  'kaliE:'..bot_id..'vipgp:'..msg.chat_id_
  if database:sismember(hash, result.sender_user_id_) then
tsX000("prore",msg,"☑┇بالتأكيد تم رفعه عضو مميز") 
  ...
  function promote_by_username(extra, result, success)
  if result.id_ then
  database:sadd('kaliE:'..bot_id..'vipgp:'..msg.chat_id_, result.id_)
  texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'kaliEteam')..')\n☑┇تم رفعه عضو مميز'
  else
  texts = '✖┇خطاء'
  end
  ...
  database:sadd('kaliE:'..bot_id..'vipgp:'..msg.chat_id_, apmd[2])
  ...
  database:set("kaliE:set:id:"..bot_id..msg.chat_id_..msg.sender_user_id_,'kaliE')
  return "kaliE"
  end
  if text and is_owner(msg) and database:get("kaliE:set:id:"..bot_id..msg.chat_id_..msg.sender_user_id_) then 
  database:del("kaliE:set:id:"..bot_id..msg.chat_id_..msg.sender_user_id_)
  send(msg.chat_id_, msg.id_, 1,  '☑┇ تم الحفض بنجاح', 1, 'md')
  database:set("kaliE:gr:id:text:"..bot_id..msg.chat_id_,text)
  end
  if text and text == "مسح الايدي" and is_owner(msg) then
  send(msg.chat_id_, msg.id_, 1,  '✖┇ تم المسح بنجاح', 1, 'md')
  database:del("kaliE:gr:id:text:"..bot_id..msg.chat_id_)
  end
  ...
  tsX000(apmd[2],msg,"☑┇تم تنزيله من اعضاء الممزين البوت")
    ...
    local hash =  'kaliE:'..bot_id..'banned:'..msg.chat_id_
    if ck_mod(result.sender_user_id_, msg.chat_id_) then
 ...
 chat_kick(result.chat_id_, result.sender_user_id_)
  ...
  send(msg.chat_id_, msg.id_, 1, '❕┇لا تستطيع حظر \n🔘┇(مدراء،ادمنيه،اعضاء مميزين)البوت', 1, 'md')
      ...
      database:sadd('kaliE:'..bot_id..'banned:'..msg.chat_id_, result.id_)
      texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apba[2] or 'kaliEteam')..')\n☑┇تم حظره من المجموعه'
  ...
  send(msg.chat_id_, msg.id_, 1, '❕┇لا تستطيع حظر \n🔘┇(مدراء،ادمنيه،اعضاء مميزين)البوت', 1, 'md')
      ...
      database:sadd('kaliE:'..bot_id..'banned:'..msg.chat_id_, apba[2])
  ...
  if database:get('kaliE:'..bot_id.."group:link"..msg.chat_id_) == 'Waiting For Link!\nPls Send Group Link' and is_mod(msg) then 
  ...
  local hash = 'kaliE:'..bot_id.."group:link"..msg.chat_id_ 
  ...
  send(msg.chat_id_, 0, 1, '↙️┇رابط المجموعه الجديد\n'..glink, 1, 'html')
   ...
   if result.id_ then
     database:srem('kaliE:'..bot_id..'banned:'..msg.chat_id_, result.id_)
  texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apba[2] or 'kaliEteam')..')\n☑┇تم الغاء حظره من البوت' 
   ...
   local apba = {string.match(text, "^(الغاء حظر) (%d+)$")}
     database:srem('kaliE:'..bot_id..'banned:'..msg.chat_id_, apba[2])
  ...
  function gban_by_reply(extra, result, success)
    local hash =  'kaliE:'..bot_id..'gbanned:'
  ...
  send(msg.chat_id_, msg.id_, 1, '❕┇لا تستطيع حظر عام \n🔘┇(مدراء،ادمنيه،اعضاء مميزين)البوت', 1, 'md')
    ...
    local hash =  'kaliE:'..bot_id..'gbanned:'
  texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apbll[2] or 'kaliEteam')..')\n🚫┇تم حظره من المجموعات البوت'
  ...
  function ungban_by_username(extra, result, success)
    local hash =  'kaliE:'..bot_id..'gbanned:'
    if result.id_ then
  texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apid[2] or 'kaliEteam')..')\n🚫┇تم الغاء حظره من المجموعات البوت'
  database:srem(hash, result.id_)
    else
    texts = '✖┇خطاء'
    end
  ...
  function gmute_by_reply(extra, result, success)
    local hash =  'kaliE:'..bot_id..'gmuted:'
    if is_admin(result) then
...
tsX000("prore",msg,"🚫┇تم الغاء كتمه من المجموعات البوت")
  ...
  tsX000(apbll[2],msg,"🚫┇تم الغاء كتمه من المجموعات البوت")
    ...
    local hash =  'kaliE:'..bot_id..'muted:'..msg.chat_id_
    if ck_mod(result.sender_user_id_, msg.chat_id_) then
...
tsX000("prore",msg,"🚫┇بالتأكيد تم كتمه")
  ...
  send(msg.chat_id_, msg.id_, 1, '✖┇لا تستطيع كتم \n🔘┇(مدراء،ادمنيه،اعضاء مميزين)البوت', 1, 'md')
      ...
      database:sadd('kaliE:'..bot_id..'muted:'..msg.chat_id_, result.id_)
    texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apsi[2] or 'kaliEteam')..')\n🚫┇تم كتمه من البوت'
    end
  else
    texts = '✖┇خطاء'
  end
...
database:sadd('kaliE:'..bot_id..'muted:'..msg.chat_id_, apsi[2])
tsX000(apsi[2],msg,"🚫┇تم كتمه من البوت")
    ...
    if result.id_ then
     database:srem('kaliE:'..bot_id..'muted:'..msg.chat_id_, result.id_)
   texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apsi[2] or 'kaliEteam')..')\n🚫┇تم الغاء كتمه من البوت'
    ...
    local apsi = {string.match(text, "^(الغاء كتم) (%d+)$")}
      database:srem('kaliE:'..bot_id..'muted:'..msg.chat_id_, apsi[2])
  ...
  send(msg.chat_id_, msg.id_, 1, '✖┇لا تستطيع طرد \n🔘┇(مدراء،ادمنيه،اعضاء مميزين)البوت', 1, 'md')
    ...
    texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apki[2] or 'kaliEteam')..')\n🚫┇تم طرده من المجموعه'
       chat_kick(msg.chat_id_, result.id_)
    end
  else
    texts = '✖┇خطاء'
  ...
  send(msg.chat_id_, msg.id_, 1, '✖┇لا تستطيع طرد \n🔘┇(مدراء،ادمنيه،اعضاء مميزين)البوت', 1, 'md')
    ...
    local hash =  'kaliE:'..bot_id..'owners:'..msg.chat_id_
    if database:sismember(hash, result.sender_user_id_) then
	tsX000("prore",msg,"☑┇بالتأكيد بالتأكيد تم رضع مدير في البوت")
    else
     database:sadd(hash, result.sender_user_id_)
	 tsX000("prore",msg,"☑┇بالتأكيد تم رضع مدير في البوت")
    end
  end
    ...
    if result.id_ then
      database:sadd('kaliE:'..bot_id..'owners:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apow[2] or 'kaliEteam')..')\n☑┇بالتأكيد تم رضع مدير في البوت'
  else
    texts = '✖┇خطاء'
  end
       send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
    ...
    local apow = {string.match(text, "^(رفع مدير) (%d+)$")}
      database:sadd('kaliE:'..bot_id..'owners:'..msg.chat_id_, apow[2])
	    tsX000(apow[2],msg,"☑┇بالتأكيد تم رضع مدير في البوت")
  end
  ...
  local hash =   'kaliE:'..bot_id..'res:'..msg.chat_id_
    local list = database:smembers(hash) 
    for k,v in pairs(list) do database:del('kaliE:'..bot_id..'res:'..msg.chat_id_) 
...
local kali = { -- the List By : kaliETEAM
        'عير',
        'كس',
        'كحبه', -- the List By : kaliETEAM
        'كساسه',
        'مناويج',
        'تنيجون',
        'سكسي',
        ...
        'پسر کونی',
        'کیرم تو مادرت',
        'کیرم تو خانوادت',-- the List By : kaliETEAM
        'پدر سگ',
        'پدر کونی',
        'خواهرت گاییدم',
        'مادرت گاییدم'
} -- the List By : kaliETEAM
...
delete_msg(msg.chat_id_,{[0] = msg.id_})
  ...
  if text:match("^جلب صوره (%d+)$") and msg.reply_to_message_id_ == 0 and not database:get('kaliE:'..bot_id..'get:photo'..msg.chat_id_) then
...
local ph = pronumb[2] - 1
 ...
 send(msg.chat_id_, msg.id_, 1, "❕┇لا تملك صوره رقم <b>{"..pronumb[2].."}</b> في حسابك", 1, 'html')
  ...
  database:set('kaliE:'..bot_id..'flood:max:'..msg.chat_id_,floodmax[2])
  ...
  database:set('kaliE:'..bot_id..'flood:time:'..msg.chat_id_,floodt[2])
    ...
    local welcome = {string.match(text, "^(وضع ترحيب) (.*)$")}
     send(msg.chat_id_, msg.id_, 1, '☑┇تم وضع ترحيب\n📜┇~⪼('..welcome[2]..')', 1, 'md')
       database:set('kaliE:'..bot_id..'welcome:'..msg.chat_id_,welcome[2])
    end
    ...
    local wel = database:get('kaliE:'..bot_id..'welcome:'..msg.chat_id_)
    if wel then
  ...
  database:hset('kaliE:'..bot_id..'filters:'..msg.chat_id_, name, 'filtered')
  ...
  database:hdel('kaliE:'..bot_id..'filters:'..msg.chat_id_, rws[2])
  ...
  if (text:match("^كشف البوتات$") or text:match("^البوتات$")) and is_mod(msg) then
   ...
   n = (n + 1)
     local user_info = database:hgetall('kaliE:'..bot_id..'user:'..v.user_id_)
  if user_info and user_info.username then
   local username = user_info.username
...
text = ""
  ...
  local user_msgs = database:get('kaliE:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
   if not database:get('kaliE:'..bot_id..'id:mute'..msg.chat_id_) then
...
add = (tonumber(database:get('kaliE:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)) or 0)
  send(msg.chat_id_, msg.id_, 1, "📨┇عدد اضافه جهاتك ~⪼ *{"..add.."}*\n📨┇سيتم حذف العدد بعد هذه الرساله", 1, 'md')
database:del('kaliE:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)
  end
  if text:match("^(عدد السحكات)$") or text:match("^(سحكاتي)$") then
  local edit = database:get('kaliE:'..bot_id..'user:editmsg'..msg.chat_id_..':'..msg.sender_user_id_) or 0
  ...
  database:del('kaliE:'..bot_id..'banned:'..msg.chat_id_)
  ...
  database:del('kaliE:'..bot_id..'creator:'..msg.chat_id_)
...
text = ""
   ...
   send(msg.chat_id_, msg.id_, 1, text, 1, 'html')
    ...
    local function cb(extra,result,success)
    local list = result.members_
  moody = '📊┇ بالتأكيد تم رضع ادمنيه المجموعه في البوت\n'
  local n = 0
  for k,v in pairs(list) do
  n = (n + 1)
  local hash =  'kaliE:'..bot_id..'mods:'..msg.chat_id_
  ...
  send(msg.chat_id_, msg.id_, 1, moody, 1, 'html')
   ...
   bot.channel_get_admins(msg.chat_id_,cb)
    ...
    if database:get("lock_media:kaliE"..msg.chat_id_..bot_id) then 
       ...
       ------------
       if not database:get('kaliE:'..bot_id..'flood:max:'..msg.chat_id_) then
       flood_m = 10
       else
       flood_m = database:get('kaliE:'..bot_id..'flood:max:'..msg.chat_id_)
     end
       if not database:get( 'kaliE:'..bot_id..'flood:time:'..msg.chat_id_) then
       flood_t = 10
       else
       flood_t = database:get( 'kaliE:'..bot_id..'flood:time:'..msg.chat_id_)
     end
       ------------
       if database:get("lock_audeo:kaliE"..msg.chat_id_..bot_id) then
       mute_music = '✔┇'
       ...
       ------------
       if database:get("lock_inline:kaliE"..msg.chat_id_..bot_id) then
       mute_in = '✔┇'
       else
       mute_in = '✖┇'
       end
       ...
       ------------
       if database:get("lock_edit:kaliE"..msg.chat_id_..bot_id) then
       mute_edit = '✔┇'
       else
       mute_edit = '✖┇'
       end
   ...
   ------------
       if database:get("lock_pin:kaliE"..msg.chat_id_..bot_id) then
       lock_pin = '✔┇'
       else
       lock_pin = '✖┇'
     end
   ...
   ------------
       if database:get("lock_kaliEr:kaliE"..msg.chat_id_..bot_id) then
       lock_sticker = '✔┇'
       else
       lock_sticker = '✖┇'
       end
  ...
  if database:get("lock_sarha:kaliE"..msg.chat_id_..bot_id) then
   ...
   if database:get("lock_username:kaliE"..msg.chat_id_..bot_id) then
       ...
       ------------
  if database:get("lock_contact:kaliE"..msg.chat_id_..bot_id) then
       lock_contact = '✔┇'
       else
       lock_contact = '✖┇'
       end
   ...
   if database:get('kaliE:'..bot_id..'rep:mute'..msg.chat_id_) then
       lock_rep = '✔┇'
       else
       lock_rep = '✖┇'
       end
       ------------
   if database:get('kaliE:'..bot_id..'repsudo:mute'..msg.chat_id_) then
       lock_repsudo = '✔┇'
       else
       lock_repsudo = '✖┇'
       end
       ------------
   if database:get('kaliE:'..bot_id..'repowner:mute'..msg.chat_id_) then
       lock_repowner = '✔┇'
       else
       lock_repowner = '✖┇'
       end
       ------------
   if database:get('kaliE:'..bot_id..'id:mute'..msg.chat_id_) then
       lock_id = '✔┇'
       else
       lock_id = '✖┇'
       end
       ------------
   if database:get('kaliE:'..bot_id..'pin:mute'..msg.chat_id_) then
       lock_pind = '✔┇'
       else
       lock_pind = '✖┇'
       end
   ...
   if database:get('kaliE:'..bot_id..'id:mute'..msg.chat_id_) then
       ...
       ------------
if database:get( 'kaliE:'..bot_id.."welcome"..msg.chat_id_) then
send_welcome = '✔┇'
else
send_welcome = '✖┇'
end
    ------------
       ...
       ------------
       if database:get("lock_inline.note:kaliE"..msg.chat_id_..bot_id) then
       mute_in = '✔┇'
       else
       mute_in = '✖┇'
       end
  ...
  if database:get("lock_sarha.note:kaliE"..msg.chat_id_..bot_id) then
   ...
   if database:get("lock_username.note:kaliE"..msg.chat_id_..bot_id) then
       ...
       ------------
  if database:get("lock_contact.note:kaliE"..msg.chat_id_..bot_id) then
       lock_contact = '✔┇'
       else
       lock_contact = '✖┇'
       end
...
local ex = database:ttl( 'kaliE:'..bot_id.."charge:"..msg.chat_id_)
  if ex == -1 then
  exp_dat = 'لا نهائي'
  else
  ...
  if (text and text == 'تفعيل اطردني') and is_owner(msg) then
     if not database:get('kaliE:'..bot_id..'kickme:mute'..msg.chat_id_) then
   send(msg.chat_id_, msg.id_, 1, '☑┇امر اطردني بالتأكيد تم تفعيله', 1, 'md')
     else
   send(msg.chat_id_, msg.id_, 1, '☑┇تم تفعيل امر اطردني', 1, 'md')
    database:del('kaliE:'..bot_id..'kickme:mute'..msg.chat_id_)
   end
   end
   if (text and text == 'تعطيل اطردني') and is_owner(msg) then
     if database:get('kaliE:'..bot_id..'kickme:mute'..msg.chat_id_) then
   send(msg.chat_id_, msg.id_, 1, '☑┇امر اطردني بالتأكيد تم تعطيله', 1, 'md')
   else
   send(msg.chat_id_, msg.id_, 1, '☑┇تم تعطيل امر اطردني', 1, 'md')
     database:set('kaliE:'..bot_id..'kickme:mute'..msg.chat_id_,true)
   end
     end

  if text:match("^اطردني$") then
  if not database:get('kaliE:'..bot_id..'kickme:mute'..msg.chat_id_) then
  redis:set('kaliE:'..bot_id..'kickyess'..msg.sender_user_id_..'', 'kickyes')
  redis:set('kaliE:'..bot_id..'kicknoo'..msg.sender_user_id_..'', 'kickno')
  send(msg.chat_id_, msg.id_, 1, '🚷┇ ارسل ؛ نعم ، ليتم طردك\n🔘┇ارسل ؛ لا ، لالغاء الامر', 1, 'md')
  else
  send(msg.chat_id_, msg.id_, 1, '⚠┇تم تعطيل امر اطردني', 1, 'md')
  end
  end
  local yess = redis:get('kaliE:'..bot_id..'kickyess'..msg.sender_user_id_..'')
  ...
  if yess == 'kickyes' then
  chat_kick(msg.chat_id_, msg.sender_user_id_)
  redis:del('kaliE:'..bot_id..'kickyess'..msg.sender_user_id_..'', 'kickyes')
  redis:del('kaliE:'..bot_id..'kicknoo'..msg.sender_user_id_..'', 'kickno')
  send(msg.chat_id_, msg.id_, 1, '✅┇تم طردك من المجموعه', 1, 'md')
    end
    end
  end
  if text:match("^لا$") then
  local noo = redis:get('kaliE:'..bot_id..'kicknoo'..msg.sender_user_id_..'')
  if noo == 'kickno' then
  redis:del('kaliE:'..bot_id..'kickyess'..msg.sender_user_id_..'', 'kickyes')
  redis:del('kaliE:'..bot_id..'kicknoo'..msg.sender_user_id_..'', 'kickno')
  ...
  redis:set('kaliE:'..bot_id..'texts'..msg.sender_user_id_..'', 'msg')
    return false end
  if text:match("^(.*)$") then
  local kali2 = redis:get('kaliE:'..bot_id..'texts'..msg.sender_user_id_..'')
  if kali2 == 'msg' then
  send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
  redis:set('kaliE:'..bot_id..'texts'..msg.sender_user_id_..'', 'no')
  redis:set('kaliE:'..bot_id..'text_sudo', text)
  send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
    return false end
   end
  if  (text and text == 'مسح امر المطور بالكليشه') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
   redis:del('kaliE:'..bot_id..'text_sudo', text)
  ...
  if text:match("^[Dd][Ee][Vv]$")or text:match("^مطور بوت$") or text:match("^مطورين$") or text:match("^مبرمج البوت$") or text:match("^مطور$") or text:match("^المطور$") and msg.reply_to_message_id_ == 0 then
    local text_sudo = redis:get('kaliE:'..bot_id..'text_sudo')
  local nkali = redis:get('kaliE:'..bot_id..'nmkali')
  local nakali = redis:get('kaliE:'..bot_id..'nakali')
    if text_sudo then
    send(msg.chat_id_, msg.id_, 1, text_sudo, 1, 'md')
    else
   sendContact(msg.chat_id_, msg.id_, 0, 1, nil, (nkali or 9647819831695), (nakali or "kaliE"), "", bot_id)
  end
   end
    for k,v in pairs(sudo_users) do
  ...
  redis:set('kaliE:'..bot_id..'nkali'..msg.sender_user_id_..'', 'msg')
    return false end
  end
  if text:match("^+(.*)$") then
  local kalio = redis:get('kaliE:'..bot_id..'sudoo'..text..'')
  local kali2 = redis:get('kaliE:'..bot_id..'nkali'..msg.sender_user_id_..'')
  if kali2 == 'msg' then
  send(msg.chat_id_, msg.id_, 1, '• `الان يمكنك ارسال الاسم الذي تريده` 🏷', 1, 'md')
  redis:set('kaliE:'..bot_id..'nmkali', text)
  redis:set('kaliE:'..bot_id..'nkali'..msg.sender_user_id_..'', 'mmsg')
    return false end
  end
  if text:match("^(.*)$") then
  local kali2 = redis:get('kaliE:'..bot_id..'nkali'..msg.sender_user_id_..'')
  ...
  redis:set('kaliE:'..bot_id..'nkali'..msg.sender_user_id_..'', 'no')
  redis:set('kaliE:'..bot_id..'nakali', text)
  local nmkali = redis:get('kaliE:'..bot_id..'nmkali')
  sendContact(msg.chat_id_, msg.id_, 0, 1, nil, nmkali, text , "", bot_id)
    ...
    function promote_by_reply(extra, result, success)
    if redis:sismember('kaliE:'..bot_id..'dev', result.sender_user_id_) then
	tsX000("prore",msg,'☑┇بالتأكيد تم رفعه مطور')
  else
    redis:set('kaliE:'..bot_id..'sudoo'..result.sender_user_id_..'', 'yes')
    redis:sadd('kaliE:'..bot_id..'dev', result.sender_user_id_)
    ...
    function promote_by_username(extra, result, success)
    if result.id_ then
    redis:set('kaliE:'..bot_id..'sudoo'..result.id_..'', 'yes')
    redis:sadd('kaliE:'..bot_id..'dev', result.id_)
      texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'kaliEteam')..')\n\n☑┇تم رفعه مطور'
  else
    texts = '✖┇خطاء'
  end
    ...
    redis:set('kaliE:'..bot_id..'sudoo'..apmd[2]..'', 'yes')
    redis:sadd('kaliE:'..bot_id..'dev', apmd[2])
    ...
    function demote_by_reply(extra, result, success)
    if not redis:sismember('kaliE:'..bot_id..'dev', result.sender_user_id_) then
	tsX000("prore",msg,'☑┇ بالتأكيد تم تنزيله من المطورين')
    else
    redis:del('kaliE:'..bot_id..'sudoo'..result.sender_user_id_..'', 'no')
    redis:srem('kaliE:'..bot_id..'dev', result.sender_user_id_)
    ...
    function demote_by_username(extra, result, success)
    if result.id_ then
    redis:del('kaliE:'..bot_id..'sudoo'..result.id_..'', 'no')
    redis:srem('kaliE:'..bot_id..'dev', result.id_)
    ...
    redis:del('kaliE:'..bot_id..'sudoo'..apmd[2]..'', 'no')
    redis:srem('kaliE:'..bot_id..'dev', apmd[2])
  ...
  redis:set('kaliE:'..bot_id..'kali1'..msg.sender_user_id_..''..msg.chat_id_..'', 'msg')
    return false end
  if text:match("^(.*)$") then
  if not database:get('kaliE:'..bot_id..'repowner:mute'..msg.chat_id_) then
  local kali = redis:get('kaliE:'..bot_id..'kali'..text..''..msg.chat_id_..'')
  send(msg.chat_id_, msg.id_, 1, kali, 1, 'md')
  end
  local kali1 = redis:get('kaliE:'..bot_id..'kali1'..msg.sender_user_id_..''..msg.chat_id_..'')
  if kali1 == 'msg' then
  send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل الرد الذي تريد اضافته', 1, 'md')
  redis:set('kaliE:'..bot_id..'kali1'..msg.sender_user_id_..''..msg.chat_id_..'', 're')
  redis:set('kaliE:'..bot_id..'msg'..msg.sender_user_id_..''..msg.chat_id_..'', text)
  redis:sadd('kaliE:'..bot_id..'repowner'..msg.sender_user_id_..''..msg.chat_id_..'', text)
    return false end
  if kali1 == 're' then
  local kali2 = redis:get('kaliE:'..bot_id..'msg'..msg.sender_user_id_..''..msg.chat_id_..'')
  redis:set('kaliE:'..bot_id..'kali'..kali2..''..msg.chat_id_..'', text)
  redis:sadd('kaliE:'..bot_id..'kalire'..msg.chat_id_..'', kali2)
  send(msg.chat_id_, msg.id_, 1, "☑┇تم حفظ الرد", 1, 'md')
  redis:set('kaliE:'..bot_id..'kali1'..msg.sender_user_id_..''..msg.chat_id_..'', 'no')
  end
  end
    if text:match("^حذف رد$") and is_owner(msg) then
  send(msg.chat_id_, msg.id_, 1, '📥┇ارسل الكلمه التي تريد حذفها', 1, 'md')
  redis:set('kaliE:'..bot_id..'kali1'..msg.sender_user_id_..''..msg.chat_id_..'', 'nomsg')
    return false end
  if text:match("^(.*)$") then
  local kali1 = redis:get('kaliE:'..bot_id..'kali1'..msg.sender_user_id_..''..msg.chat_id_..'')
  if kali1 == 'nomsg' then
  ...
  redis:set('kaliE:'..bot_id..'kali1'..msg.sender_user_id_..''..msg.chat_id_..'', 'no')
  redis:set('kaliE:'..bot_id..'kali'..text..''..msg.chat_id_..'', " ")
 redis:srem('kaliE:'..bot_id..'kalire'..msg.chat_id_..'', text)
   end
  end
  ...
  redis:set('kaliE:'..bot_id..'kali1'..msg.sender_user_id_..'', 'msg')
    return false end
  if text:match("^(.*)$") then
  if not database:get('kaliE:'..bot_id..'repsudo:mute'..msg.chat_id_) then
  local kali = redis:get('kaliE:'..bot_id..'kali'..text..'')
  send(msg.chat_id_, msg.id_, 1, kali, 1, 'md')
  end
  local kali1 = redis:get('kaliE:'..bot_id..'kali1'..msg.sender_user_id_..'')
  if kali1 == 'msg' then
  send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل الرد الذي تريد اضافته', 1, 'md')
  redis:set('kaliE:'..bot_id..'kali1'..msg.sender_user_id_..'', 're')
  redis:set('kaliE:'..bot_id..'msg'..msg.sender_user_id_..'', text)
    return false end
  if kali1 == 're' then
  local kali2 = redis:get('kaliE:'..bot_id..'msg'..msg.sender_user_id_..'')
  redis:set('kaliE:'..bot_id..'kali'..kali2..'', text)
  redis:sadd('kaliE:'..bot_id..'kaliresudo', kali2)
  send(msg.chat_id_, msg.id_, 1, "☑┇تم حفظ الرد", 1, 'md')
  redis:set('kaliE:'..bot_id..'kali1'..msg.sender_user_id_..'', 'no')
  end
  end

  if text:match("^حذف رد للكل$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add)  then
  send(msg.chat_id_, msg.id_, 1, '📥┇ارسل الكلمه التي تريد حذفها' , 1, 'md')
  redis:set('kaliE:'..bot_id..'kali1'..msg.sender_user_id_..'', 'nomsg')
    return false end
  if text:match("^(.*)$") then
  local kali1 = redis:get('kaliE:'..bot_id..'kali1'..msg.sender_user_id_..'')
  if kali1 == 'nomsg' then
  send(msg.chat_id_, msg.id_, 1, '☑┇تم حذف الرد', 1, 'md')
  redis:set('kaliE:'..bot_id..'kali1'..msg.sender_user_id_..'', 'no')
   redis:set('kaliE:'..bot_id..'kali'..text..'', " ")
   redis:srem('kaliE:'..bot_id..'kaliresudo', text)
   end
  end

  if text:match("^مسح المطورين$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
    local list = redis:smembers('kaliE:'..bot_id..'dev')
    for k,v in pairs(list) do
  redis:del('kaliE:'..bot_id..'dev', text)
  redis:del('kaliE:'..bot_id..'sudoo'..v..'', 'no')
  end
    send(msg.chat_id_, msg.id_, 1, "☑┇تم مسح مطورين البوت", 1, 'md')
    end
    if text:match("^مسح ردود المدير$") and is_owner(msg) then
    local list = redis:smembers('kaliE:'..bot_id..'kalire'..msg.chat_id_..'')
    for k,v in pairs(list) do
  redis:del('kaliE:'..bot_id..'kalire'..msg.chat_id_..'', text)
  redis:set('kaliE:'..bot_id..'kali'..v..''..msg.chat_id_..'', " ")
  end
    ...
    local list = redis:smembers('kaliE:'..bot_id..'kaliresudo')
    for k,v in pairs(list) do
  redis:del('kaliE:'..bot_id..'kaliresudo', text)
  redis:set('kaliE:'..bot_id..'kali'..v..'', " ")
  end
  ...
  text = "❕┇لا يوجد ردود للمدير"
    ...
    database:set('kaliE:'..bot_id..'rules'..msg.chat_id_, txt[2])
     send(msg.chat_id_, msg.id_, 1, "✔┇تم وضع القوانين للمجموعه", 1, 'md')
    end
    
      if  text:match("^القوانين$") then
    local rules = database:get('kaliE:'..bot_id..'rules'..msg.chat_id_)
    if rules then
  ...
  database:set('kaliE:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_,true)
  ...
  ----------------------------------------------------------------------------------------------
 if not database:get('kaliE:'..bot_id..'rep:mute'..msg.chat_id_) then 
if text == 'هلو' then 
...
if  (text and text == 'تفعيل ردود البوت') and is_owner(msg) then
  ...
  send(msg.chat_id_, msg.id_, 1, '☑┇تم تفعيل ردود البوت', 1, 'md')
   database:del('kaliE :'..bot_id..'rep:mute'..msg.chat_id_)
  end
  end
  ...
  send(msg.chat_id_, msg.id_, 1, '☑┇تم تعطيل ردود البوت', 1, 'md')
    database:set('kaliE :'..bot_id..'rep:mute'..msg.chat_id_,true)
  end
    end
  
  if (text and text == 'تفعيل الايدي بالصوره') and is_owner(msg) then
  if not database:get('kaliE:'..bot_id..'id:photo'..msg.chat_id_) then
    ...
    send(msg.chat_id_, msg.id_, 1, '☑┇تم تفعيل الايدي بالصوره', 1, 'md')
     database:del('kaliE:'..bot_id..'id:photo'..msg.chat_id_)
  end
  end
  if (text and text == 'تعطيل الايدي بالصوره') and is_owner(msg) then
  if database:get('kaliE:'..bot_id..'id:photo'..msg.chat_id_) then
  ...
  database:set('kaliE:'..bot_id..'id:photo'..msg.chat_id_,true)
  end
  end
  if  (text and text == 'تفعيل وضع المدفوع') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
  if database:sismember('kaliE:'..bot_id..'pro:groups',msg.chat_id_) then
    ...
    send(msg.chat_id_, msg.id_, 1, '☑┇تم اضافه المجموعه الى الوضع المدفوع', 1, 'md')
     database:sadd('kaliE:'..bot_id..'pro:groups',msg.chat_id_)
  end
  end
  if   (text and text == 'تعطيل وضع المدفوع') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
  if not database:sismember('kaliE:'..bot_id..'pro:groups',msg.chat_id_) then
  ...
  database:srem('kaliE:'..bot_id..'pro:groups',msg.chat_id_)
  end
  end
  ...
  send(msg.chat_id_, msg.id_, 1, '☑┇تم تفعيل ردود المطور', 1, 'md')
   database:del('kaliE:'..bot_id..'repsudo:mute'..msg.chat_id_)
  end
  end
  ...
  send(msg.chat_id_, msg.id_, 1, 'تم تعطيل ردود المطور', 1, 'md')
    database:set('kaliE:'..bot_id..'repsudo:mute'..msg.chat_id_,true)
  end
    end
  ...
  send(msg.chat_id_, msg.id_, 1, '☑┇تم تفعيل ردود المدير', 1, 'md')
   database:del('kaliE:'..bot_id..'repowner:mute'..msg.chat_id_)
  end
  end
  ...
  send(msg.chat_id_, msg.id_, 1, '☑┇تم تعطيل ردود المدير', 1, 'md')
    database:set('kaliE:'..bot_id..'repowner:mute'..msg.chat_id_,true)
  end
    end
  ...
  send(msg.chat_id_, msg.id_, 1, '☑┇تم تفعيل الايدي', 1, 'md')
   database:del('kaliE:'..bot_id..'id:mute'..msg.chat_id_)
  end
  end
  ...
  send(msg.chat_id_, msg.id_, 1, '☑┇تم تعطيل الايدي', 1, 'md')
    database:set('kaliE:'..bot_id..'id:mute'..msg.chat_id_,true)
  end
  end
  ...
  database:set("kaliE"..bot_id..":sudo:gr",msg.chat_id_)
  end
  ...
  database:del("kaliE"..bot_id..":sudo:gr")
  end
  if text:match("^ايدي$") and msg.reply_to_message_id_ == 0 then
  local kali_info = nil
  function kali333(extra,result,success)
  ...
  if not database:get('kaliE:'..bot_id..'id:mute'..msg.chat_id_) then
  if not database:get('kaliE:'..bot_id..'id:photo'..msg.chat_id_) then
  if not database:get("kaliE:gr:id:text:"..bot_id..msg.chat_id_) then 
  local kali_text = {
 ' ياحلو مين الله جابك🤧',
  " غير صورتك 😪 ",
  " هم انته ♥️ ",
  ...
  sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, result.photos_[0].sizes_[1].photo_.persistent_id_," 📷┇ "..kali_text[kali3].."\n🎫┇ايديك ~⪼ ("..msg.sender_user_id_..")\n📜┇معرفك ~⪼ "..kali_info.."\n📡┇موقعك ~⪼ "..t.."\n📨┇رسائلك ~⪼ {"..user_msgs.."}\n📧┇السحكات ~⪼ {"..edit.."}\n____________________________ ",msg.id_,msg.id_.."")
  else 
  local kaliE_new_text = database:get("kaliE:gr:id:text:"..bot_id..msg.chat_id_)
  local kaliE_new_text = kaliE_new_text:gsub('#username',(kali_info or 'لا يوجد'))
  local kaliE_new_text = kaliE_new_text:gsub('#id',(msg.sender_user_id_ or 'لا يوجد'))
  local kaliE_new_text = kaliE_new_text:gsub('#edit',(edit or 'لا يوجد'))
  local kaliE_new_text = kaliE_new_text:gsub('#msgs',(user_msgs or 'لا يوجد'))
  local kaliE_new_text = kaliE_new_text:gsub('#stast',(t or 'لا يوجد'))
  sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, result.photos_[0].sizes_[1].photo_.persistent_id_,kaliE_new_text,msg.id_,msg.id_.."")
  end
  else
  if is_sudo(msg) then
  t = 'مبرمج البوت'
  ...
  local kaliE_new_text = kaliE_new_text:gsub('#msgs',(user_msgs or 'لا يوجد'))
  local kaliE_new_text = kaliE_new_text:gsub('#stast',(t or 'لا يوجد'))   
  send(msg.chat_id_, msg.id_, 1, kaliE_new_text, 1, 'html')
  end   
  end
  else
  end
  ...
  if  (text and text == 'تفعيل التثبيت') and is_owner(msg) then
   ...
   send(msg.chat_id_, msg.id_, 1, '☑┇تم تعطيل التثبيت', 1, 'md')
     database:set('kaliE:'..bot_id..'pin:mute'..msg.chat_id_,true)
   end
     end
   ...
   pin(msg.chat_id_,msg.reply_to_message_id_,0)
       database:set('kaliE:'..bot_id..'pinnedmsg'..msg.chat_id_,msg.reply_to_message_id_)
    ...
    database:set('kaliE:'..bot_id..'viewget'..msg.sender_user_id_,true)
  ...
  redis:del('kaliE:'..bot_id..'help', text)
  redis:del('kaliE:'..bot_id..'h1', text)
  redis:del('kaliE:'..bot_id..'h2', text)
  redis:del('kaliE:'..bot_id..'h3', text)
  redis:del('kaliE:'..bot_id..'h4', text)
  redis:del('kaliE:'..bot_id..'h5', text)
  redis:del('kaliE:'..bot_id..'h6', text)
  ...
  send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
  redis:set('kaliE:'..bot_id..'hhh'..msg.sender_user_id_..'', 'msg')
    return false end
  if text:match("^(.*)$") then
  local kali2 = redis:get('kaliE:'..bot_id..'hhh'..msg.sender_user_id_..'')
  if kali2 == 'msg' then
  send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
  redis:set('kaliE:'..bot_id..'hhh'..msg.sender_user_id_..'', 'no')
  redis:set('kaliE:'..bot_id..'help', text)
  send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
    return false end
   end

     if text:match("^الاوامر$") and is_mod(msg) then
  local help = redis:get('kaliE:'..bot_id..'help')
     local text =  
...
💎┇م5 ~⪼ لعرض اوامر التقييد		
 ...
 send(msg.chat_id_, msg.id_, 1, (help or text), 1, 'md')
  ...
  send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
  redis:set('kaliE:'..bot_id..'h11'..msg.sender_user_id_..'', 'msg')
    return false end
  if text:match("^(.*)$") then
  local kali2 = redis:get('kaliE:'..bot_id..'h11'..msg.sender_user_id_..'')
  if kali2 == 'msg' then
  send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
  redis:set('kaliE:'..bot_id..'h11'..msg.sender_user_id_..'', 'no')
  redis:set('kaliE:'..bot_id..'h1', text)
  send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
    return false end
   end
     if text:match("^م1$") and is_mod(msg) then
  local h1 = redis:get('kaliE:'..bot_id..'h1')
     local text =  [[   📌┇ 💥 • اهلا بك  • 💥
...
___________________________
  ...
  if (text and text == 'تغير امر م2') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
   send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
   redis:set('kaliE:'..bot_id..'h22'..msg.sender_user_id_..'', 'msg')
     return false end
   if text:match("^(.*)$") then
   local kali2 = redis:get('kaliE:'..bot_id..'h22'..msg.sender_user_id_..'')
   if kali2 == 'msg' then
   send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
   redis:set('kaliE:'..bot_id..'h22'..msg.sender_user_id_..'', 'no')
   redis:set('kaliE:'..bot_id..'h2', text)
   send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
     return false end
    end
  if text:match("^م5$") and is_mod(msg) then
  local res = redis:get('kaliE:'..bot_id..'hres')
...
___________________________]]
 ...
 local h2 = redis:get('kaliE:'..bot_id..'h3')
local text = [[🥈┇💥•  ‏💥 • اهلا بك   
🛠❯❯ في اوامر الادمنيه  ❮❮🛠
...
___________________________
 📡┇Ch ~⪼ @kaliETEAM ]]                                        
  ...
  if (text and text == 'تغير امر م4') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
   send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
   redis:set('kaliE:'..bot_id..'h44'..msg.sender_user_id_..'', 'msg')
     return false end
   if text:match("^(.*)$") then
   local kali2 = redis:get('kaliE:'..bot_id..'h44'..msg.sender_user_id_..'')
   if kali2 == 'msg' then
   send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
   redis:set('kaliE:'..bot_id..'h44'..msg.sender_user_id_..'', 'no')
   redis:set('kaliE:'..bot_id..'h4', text)
   send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
     return false end
    end
  if text:match("^م3$") and is_mod(msg) then
  local h4 = redis:get('kaliE:'..bot_id..'h3')
  local text =  [[🥇┇💡 • اهلا بك   • 💡
...
___________________________]]
  ...
  if (text and text == 'تغير امر م4') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
    send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
    redis:set('kaliE:'..bot_id..'h55'..msg.sender_user_id_..'', 'msg')
  return false end
    if text:match("^(.*)$") then
    local kali2 = redis:get('kaliE:'..bot_id..'h55'..msg.sender_user_id_..'')
    if kali2 == 'msg' then
    send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
    redis:set('kaliE:'..bot_id..'h55'..msg.sender_user_id_..'', 'no')
    redis:set('kaliE:'..bot_id..'h5', text)
    send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
  return
   false
 end
  ...
  local h4 = redis:get('kaliE:'..bot_id..'h4')
...
___________________________
`git clone https://github.com/kaliEM/kaliE && cd kaliE && chmod +x install.sh && ./install.sh`
____________________________ ┉ ┉ 
✅┇طريقة التنصيب،☢️
___________________________
...
___________________________     
♻️┇كود الرن ~⪼ `./kaliE/ts`
⚠️┇ كود الحذف ~⪼ `rm -rf *kaliE`
___________________________ 
 🦁┇[قناةالسورس ](t.me/kaliETEAM)              
___________________________
]] 
...
local svuser = 'kaliE:'..bot_id..'user:'..result.id_
if username then
...
if database:get('kaliE:'..bot_id..'welcome:'..msg.chat_id_) then
text = database:get('kaliE:'..bot_id..'welcome:'..msg.chat_id_)
else
text = '👋🏻┇اهلا بك يا ؛ {fr}\n🙋🏼‍♂️┇نورت الكروب ؛ @{us}'
end
...
if database:get('kaliE:'..bot_id..'pinnedmsg'..msg.chat_id_) and database:get("lock_pin:kaliE"..msg.chat_id_..bot_id) then
local pin_id = database:get('kaliE:'..bot_id..'pinnedmsg'..msg.chat_id_)
...
local photo_kali = download_to_file('https://api.telegram.org/file/bot'..token..'/'..kali6.result.file_path, 'files_kaliE/'..msg.content_.document_.file_name_)
send(msg.chat_id_, msg.id_, 1, "✔┇بالتأكيد تم رضع الملف بنجاح", 1, 'html')
...
local msgg = result 
database:incr('kaliE:'..bot_id..'user:editmsg'..msgg.chat_id_..':'..msgg.sender_user_id_)
